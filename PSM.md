...
*Distilled from bookmarked INI-CET / NEET-PG MCQs. Only high-yield, revisable learning points were kept; duplicate pearls have been merged and each pearl states a point once. Date-sensitive programme content has been updated to the position as of 2026, with footnoted sources listed at the end of the file.*
...
- [Epidemiology — Measures and Study Designs](#epidemiology--measures-and-study-designs) — 6 pearls
- [Biostatistics](#biostatistics) — 5 pearls
- [Screening and Surveillance](#screening-and-surveillance) — 2 pearls
- [Communicable Disease Epidemiology](#communicable-disease-epidemiology) — 7 pearls
- [Immunisation and Vaccines](#immunisation-and-vaccines) — 10 pearls
- [Disinfection, Sterilisation and Infection Control](#disinfection-sterilisation-and-infection-control) — 3 pearls
...
- [National Health Programmes](#national-health-programmes) — 5 pearls
- [Health Planning, Management and Administration](#health-planning-management-and-administration) — 7 pearls
- [Demography and Vital Statistics](#demography-and-vital-statistics) — 2 pearls
- [Health Legislation and Miscellaneous](#health-legislation-and-miscellaneous) — 3 pearls
...
- **Incidence FALLS with effective PRIMARY prevention; prevalence falls with cure or with death.** **CFR is most useful in ACUTE, short-duration conditions (outbreaks, food poisoning); the FIVE-YEAR SURVIVAL RATE is the cancer standard, and can be inflated by LEAD-TIME and LENGTH-TIME BIAS from screening.**
- **ATTACK RATE is an incidence rate used in epidemics over a short period; the SECONDARY ATTACK RATE (new cases among contacts of a primary case within the incubation period ÷ susceptible contacts) measures INFECTIVITY.**
- **CRUDE DEATH RATE (deaths ÷ mid-year population × 1000) is affected by age structure; STANDARDISED rates permit comparison — DIRECT standardisation applies study rates to a standard population, INDIRECT gives the STANDARDISED MORTALITY RATIO. PROPORTIONAL MORTALITY RATE is the proportion of all deaths due to one cause.**
- **INFANT MORTALITY RATE (deaths under 1 year per 1000 LIVE BIRTHS) is the single most sensitive indicator of the availability, utilisation and effectiveness of health care and of socio-economic conditions; the UNDER-5 MORTALITY RATE is the best indicator of SOCIO-ECONOMIC DEVELOPMENT.**
- **NEONATAL (<28 days) deaths reflect ENDOGENOUS factors (prematurity, birth asphyxia, congenital anomalies); POST-NEONATAL (28 days–1 year) deaths reflect EXOGENOUS ones (infection, diarrhoea, malnutrition) — so the ratio tells a district which interventions it needs.**
...
**PEARL:** **DESCRIPTIVE studies are used for a NEW disease or one of UNKNOWN AETIOLOGY** — they define the problem by TIME, PLACE and PERSON and generate hypotheses. **LONGITUDINAL (cohort) studies establish the NATURAL HISTORY of disease, incidence and risk factors.** **RANDOMISED CONTROLLED TRIALS are the STRONGEST design for PROVING CAUSATION.** **HIERARCHY OF EVIDENCE: META-ANALYSIS/SYSTEMATIC REVIEW > RCT > COHORT > CASE-CONTROL > CASE SERIES/REPORTS > expert opinion.**

**UNDERSTAND:** The hierarchy is ordered by **how effectively each design eliminates BIAS AND CONFOUNDING.** Only **RANDOMISATION distributes both known AND UNKNOWN confounders equally** — which is why an RCT alone can support a causal claim. Observational designs can adjust only for confounders the investigator thought to measure. Systematic reviews rank higher still because they combine several RCTs, increasing precision.
...
- **ECOLOGICAL (correlational) studies compare POPULATIONS rather than individuals and are the WEAKEST design — vulnerable to the ECOLOGICAL FALLACY.**
...
**PEARL:** **COMMON-SOURCE, SINGLE-EXPOSURE (POINT-SOURCE) — all cases occur WITHIN ONE INCUBATION PERIOD, giving a sharp rise and rapid fall (food poisoning). COMMON-SOURCE, CONTINUOUS/MULTIPLE EXPOSURE — cases span MORE THAN ONE INCUBATION PERIOD from the same source. PROPAGATED (progressive) — PERSON-TO-PERSON spread, producing progressively taller peaks ONE INCUBATION PERIOD APART, with a slow rise and slow fall.**

**UNDERSTAND:** The shape of the curve is a **direct readout of the mode of transmission**, which is why drawing it is the first analytical step in any outbreak. **The distance between successive peaks equals ONE INCUBATION PERIOD** (so the curve itself estimates it), and the **duration of the outbreak relative to that period distinguishes a single exposure from a continuing one**. Counting back one incubation period from the peak identifies the probable time of exposure — the key to finding the source.
...
- **POPULATION ATTRIBUTABLE RISK = (incidence in the total population − incidence in unexposed); PAR% = PAR ÷ incidence in total population × 100. PAR depends on the PREVALENCE OF EXPOSURE**, and must be smaller than AR unless everyone is exposed. **AR speaks about the EXPOSED; PAR speaks about EVERYONE.**
...
**REMEMBER:** *Relative risk is for the individual, attributable risk is for the population — direction from the odds ratio, credibility from the p-value.*
...
**PEARL:** **VALIDITY = ACCURACY (closeness to the true value); RELIABILITY = REPEATABILITY/PRECISION (consistency on repeated testing).** A test whose **readings scatter widely but whose MEAN matches the standard has HIGH VALIDITY AND LOW RELIABILITY.** **INTERNAL validity = is the result true WITHIN the study (free of bias, confounding and chance)? EXTERNAL validity = GENERALISABILITY.**

**UNDERSTAND:** Validity and reliability are **independent properties** — think of a dartboard: darts scattered evenly around the bullseye **average out correctly (valid) but are inconsistent (unreliable)**; darts tightly grouped in one corner are **reliable but not valid**. Internal and external validity, by contrast, are **ordered, not parallel: internal validity is a prerequisite for external validity** — a study biased internally has nothing true to generalise. And the measures that improve internal validity (strict inclusion criteria, highly selected patients, tightly controlled settings) are exactly those that **reduce** external validity — the reason explanatory (efficacy) and pragmatic (effectiveness) trials both exist.
...
- **An RCT has the highest INTERNAL validity; a well-conducted community-based cohort often has better EXTERNAL validity.**

**EXAM CONNECTION:** Definition-matching — **external = generalisation**. In the numerical stem, calculate the mean first.

**REMEMBER:** *Scattered but centred is accurate yet imprecise. Internal = true here; external = true elsewhere.*
...
### 12. Predictive Values and Prevalence
...
### 13. Central Tendency, Dispersion and Skew

**PEARL:** **The MODE is the only measure of central tendency usable for QUALITATIVE (nominal) data; the MEAN is the only one distorted by extreme values.** **POSITIVE (right) skew: MEAN > MEDIAN > MODE. NEGATIVE (left) skew: MEAN < MEDIAN < MODE. Symmetrical: MEAN = MEDIAN = MODE.** **The COEFFICIENT OF VARIATION (CV = SD ÷ mean × 100) is used to COMPARE variability between TWO DIFFERENT DATA SETS.**

**UNDERSTAND:** Each measure demands a different mathematical property. The **mode only needs counting**, so it works for blood groups or religions where arithmetic is meaningless; the **median needs ordering**; the **mean needs addition and division** — and because every value enters the sum, **a single outlier drags the mean while the median only cares about position**. That is also why **the mean chases the tail**: skew is named for **where the long tail points**, not where the bulk of data sits. And a standard deviation cannot be compared across data sets because it **carries the units and scale of its own data** — the CV solves this by expressing variability as a **percentage of the mean, making it dimensionless.**
...
- **SD describes the SPREAD OF THE DATA and does not shrink with a larger sample; the STANDARD ERROR OF THE MEAN (SEM = SD/√n) describes the PRECISION OF THE ESTIMATE and FALLS as sample size rises. CONFIDENCE INTERVALS are built from the SEM (95% CI = mean ± 1.96 SEM). VARIANCE = SD².**
- **The MEDIAN (with IQR) is the appropriate summary for skewed data; the MEAN (with SD) for normal data.** Skewed data are handled by **LOG TRANSFORMATION** or **non-parametric tests**.
...
**UNDERSTAND:** The purpose is **logistical**: you cannot list every student in a state, but you can list every school. Sampling in stages means you only need a **sampling frame for the units you actually select at each level**, which is what makes large national surveys (NFHS, coverage evaluation surveys) feasible.
...
### 16. The Normal Distribution and Probability
...
### 18. Cancer Screening Methods

**PEARL:** **Per-rectal examination is NOT a screening test for colorectal cancer** (it reaches only the distal rectum), and **oral cancer screening requires INSPECTION AND PALPATION of the whole oral cavity, not just external visual inspection.** **Pap smear/HPV testing for cervical cancer** and **mammography for breast cancer** are valid screening tests.

**UNDERSTAND:** A screening test must **reach the site where the disease actually arises** and detect it before symptoms. A finger reaches ~8–10 cm of a 150 cm colon, so most tumours are anatomically out of range — which is why the world moved to **faecal occult blood/FIT and colonoscopy**. Similarly, many oral premalignant lesions sit on the **floor of the mouth, ventral tongue and buccal sulci** and are **palpable before they are visible**.
...
### 22. Secondary Attack Rate and Vaccine Efficacy
...
- **ATTACK RATE is an incidence rate used for a narrow time period during an epidemic, usually a percentage; the FOOD-SPECIFIC ATTACK RATE identifies the offending item by comparing attack rates among those who did and did not eat each dish.**
...
- **In a household or institutional outbreak a HIGH SAR indicates high communicability and person-to-person spread; a LOW SAR with many simultaneous cases points to a COMMON SOURCE.**
...
### 24. Leprosy — Diagnosis, Classification and Treatment

**PEARL:** Diagnose leprosy (and start MDT) on **ANY ONE** of three cardinal signs: **(1) definite LOSS OF SENSATION in a hypopigmented/reddish patch; (2) THICKENED PERIPHERAL NERVE** WITH **sensory loss or muscle weakness; (3) ACID-FAST BACILLI in a slit-skin smear.** **NERVE THICKENING ALONE IS NOT SUFFICIENT.** **RIDLEY–JOPLING classifies on FOUR parameters — CLINICAL, BACTERIOLOGICAL, IMMUNOLOGICAL and HISTOLOGICAL — into the spectrum TT – BT – BB – BL – LL.**

**UNDERSTAND:** Leprosy is fundamentally a **disease of nerves**, so diagnosis requires **evidence of nerve damage (sensory loss or weakness)** or **direct evidence of the organism**. A thickened nerve alone is non-specific — it occurs in amyloidosis, Charcot–Marie–Tooth and other neuropathies. The Ridley–Jopling spectrum, meanwhile, is a **map of cell-mediated immunity**: at the tuberculoid pole strong CMI produces **few, well-defined, bacillus-poor lesions with well-formed granulomas and a positive lepromin test**; at the lepromatous pole absent CMI allows **innumerable, symmetrical, bacillus-rich lesions with foamy macrophages and a negative lepromin test.** All four parameters simply read that same immune gradient differently.
...
- **LOUSE — epidemic typhus, relapsing fever, trench fever. RAT FLEA — bubonic plague, endemic (murine) typhus. MITE (trombiculid) — scrub typhus.**
...
**MUST KNOW — the current schedule:**

- **BIRTH: BCG (0.05 mL intradermal, LEFT upper arm), OPV-0, HEPATITIS B birth dose (within 24 hours).**
...
- **WHO-recommended strain: DANISH 1331. Reconstitute with NORMAL SALINE (distilled water is irritant); DISCARD leftover within 3–4 HOURS; store at 2–8 °C protected from light.** **The FREEZE-DRIED (lyophilised) preparation is MORE stable than liquid** — the statement that it is less stable is the classic false option.
- **A normal individual becomes MANTOUX-POSITIVE at about 8 WEEKS.** **ABSENCE OF A SCAR does not reliably indicate failure and is NOT an automatic indication to revaccinate.**
- **Efficacy against adult pulmonary TB is highly variable (0–80%)** — attributed to **environmental mycobacteria, BCG strain differences and latitude**; the **CHINGLEPUT trial in India showed no protection** against pulmonary TB.
- **Adverse effects: BCG ADENITIS (commonest; usually self-limiting — needle aspiration if suppurative, antitubercular drugs NOT routinely indicated), injection abscess, KELOID, rarely BCG OSTEOMYELITIS, and DISSEMINATED BCG-osis in the immunocompromised.**
...
**UNDERSTAND:** Protection depends on **neutralising antibody against conformational epitopes on the intact capsid** — antibody must bind the virus **before** it enters the basal keratinocyte. Only the **assembled L1 capsid** presents those epitopes, which is why the vaccine uses self-assembling VLPs rather than a linear peptide. **E6 and E7 are intracellular**, expressed only *after* infection (E6 degrades **p53**, E7 inactivates **Rb**), so antibodies against them would be useless prophylactically — they are the target of **therapeutic** vaccines under development. The dose reduction rests on the observation that **adolescent immune systems generate exceptionally robust, durable antibody responses to VLPs** — a premise that fails in immunodeficiency, where seroconversion is lower and antibody wanes faster, so full multi-dose priming is retained precisely in the group at highest risk.
...
- **WHO 90–70–90 ELIMINATION TARGETS BY 2030: 90% of girls vaccinated by age 15; 70% of women screened twice with a high-performance test (HPV DNA) at 35 and 45; 90% of those with disease treated.**

**EXAM CONNECTION:** One-line recall of the antigen — **L1 = vaccine; E6/E7 = oncogenes.** For schedules, the question turns on **immune status**: normal adolescent → 1 dose (or 1–2); **immunocompromised → 2–3 doses.**
...
### 30. The Cold Chain, Vaccine Vial Monitor and Open Vial Policy
...
- **Order of heat sensitivity (most → least): OPV > measles/MR > reconstituted BCG > DPT/hepatitis B/TT.** **FREEZE-SENSITIVE — NEVER FREEZE: HEPATITIS B, DPT, TT, Td, IPV, PCV, ROTAVIRUS, typhoid and ALL DILUENTS.** **The SHAKE TEST detects a previously frozen adsorbed vaccine (sedimentation within 30 minutes = damaged, discard).**
...
**PEARL:** **MMR can cause THROMBOCYTOPENIA** (2–3 weeks after, transient); **TETANUS TOXOID is the classic cause of BRACHIAL NEURITIS**; **IPV may contain trace STREPTOMYCIN, NEOMYCIN and POLYMYXIN B, and MMR contains NEOMYCIN.** For **DPT, ONLY ANAPHYLAXIS to a previous dose and ENCEPHALOPATHY WITHIN 7 DAYS are true CONTRAINDICATIONS** — fever, inconsolable crying and seizures are **PRECAUTIONS**, and a temperature above 37 °C is certainly not a contraindication.

**UNDERSTAND:** The trace antibiotics matter only because they are the basis of a **genuine contraindication** — documented ANAPHYLAXIS to neomycin precludes MMR or IPV. Knowing *why* an excipient is there converts a list into a decision rule, and prevents the far commoner error of withholding vaccines for trivial reasons. The contraindication/precaution split is a **risk–benefit judgement**: a contraindication means the risk of the next dose **exceeds** the benefit; a precaution means the risk is **increased but usually outweighed**, so vaccination proceeds with counselling and monitoring.
...
- **PASSIVE immunity is IMMEDIATE but SHORT-LIVED (weeks to months) with NO memory; ACTIVE immunity is slower but long-lasting with memory.** **COMBINED (passive–active) immunisation is used when protection is needed at once AND for the future — TETANUS (TIG + toxoid), RABIES (RIG + vaccine), HEPATITIS B (HBIG + vaccine at birth) — given at SEPARATE SITES WITH SEPARATE SYRINGES** so the immunoglobulin does not neutralise the vaccine.
- **SPECIFIC (hyperimmune) immunoglobulins come from recently recovered or hyperimmunised donors: TIG, HBIG, RIG, VZIG, anti-D.**
- **NATURAL PASSIVE immunity is transplacental IgG (protecting the infant ~6 months — the reason measles vaccine is deferred to 9 months) and secretory IgA in breast milk.**
...
### 33. Tetanus Prophylaxis in Wound Management

**PEARL:** For a **CLEAN MINOR WOUND** in someone who completed a primary course but whose **last dose was >10 years ago**, give **a single Td BOOSTER — NO immunoglobulin.** TIG is reserved for **tetanus-prone wounds in the INCOMPLETELY immunised.**

**UNDERSTAND: **Two independent questions drive every answer:** how tetanus-prone is the wound?** and **how complete is the immunisation?** Toxoid produces **active** immunity (days to develop, durable); TIG provides **immediate passive** antitoxin for the current wound. A fully primed person mounts a rapid **anamnestic response** to a booster, so passive cover is unnecessary — hence no TIG despite the long interval.
...
- **CATEGORY I — touching/feeding, licks on INTACT skin → NO PEP. CATEGORY II — nibbling of uncovered skin, minor scratches or abrasions WITHOUT bleeding → VACCINE ONLY. CATEGORY III — transdermal bites, scratches with BLEEDING/OOZING, licks on broken skin or mucosa, BAT exposure → VACCINE + RABIES IMMUNOGLOBULIN.** ***AN ABRASION WITH OOZING IS CATEGORY III, not II — bleeding is the discriminator.***
...
- **The yellow fever certificate is the only one still routinely required under the IHR** for international travel (plus meningococcal and polio requirements for the Hajj and specified countries).
...
### 36. Managing a Blood Spill and Needle-Stick Injury
...
### 37. Spaulding Classification and Sterilisation Methods
...
- **AUTOCLAVE (moist heat) — 121 °C at 15 psi for 15–20 minutes — is the most reliable method and the standard for surgical instruments and dressings.** Sterility is monitored with ***GEOBACILLUS STEAROTHERMOPHILUS*** spores.
- **HOT AIR OVEN (dry heat) — 160 °C for 2 HOURS** (alternatives 170 °C/1 h, 180 °C/30 min) — for **glassware, powders, oils, sharp instruments and glass syringes**, i.e. items ruined by moisture. Monitored with ***BACILLUS ATROPHAEUS***.
- **2% GLUTARALDEHYDE sterilises ENDOSCOPES and fibreoptic instruments** (heat-sensitive semi-critical items): it needs **ACTIVATION at alkaline pH**, and takes **20 MINUTES for high-level disinfection but 10 HOURS for sterilisation**. It is irritant to eyes and airways.

**REMEMBER:** *Sterile tissue = sterilise; mucosa = high-level; skin = low-level. Dry heat 160 °C × 2 h; autoclave 121 °C × 15 min at 15 psi.*

---

### 38. Disinfectants — Potency, Spectrum and Testing

**PEARL:** The **RIDEAL–WALKER coefficient measures the power of a disinfectant RELATIVE TO PHENOL** (the phenol coefficient) — the ratio of the dilution of the test disinfectant to the dilution of phenol that kills *Salmonella typhi* in the same time, so **>1 means more potent than phenol.** Its weakness is that it is tested **WITHOUT organic matter**, so it overstates real-life performance; the **CHICK–MARTIN test corrects this by adding organic matter (yeast/faeces)**, giving **more realistic and LOWER** values.

**UNDERSTAND:** Every property of a disinfectant follows from its chemistry and what inactivates it. **Cationic agents (QACs) are neutralised by anionic soaps; oxidising agents (hypochlorite) are consumed by organic matter; iodine needs a carrier to stay soluble and non-irritant.** Knowing the inactivator predicts the failure mode.
...
### 39. Refuse Disposal and Housing Standards

**PEARL:** **CONTROLLED TIPPING (SANITARY LANDFILL) is the most satisfactory method of refuse disposal where land is available — the TRENCH METHOD requires about 1 ACRE OF LAND PER 10,000 POPULATION PER YEAR.** Variants: **TRENCH (level ground), RAMP (sloping terrain) and AREA (filling depressions, quarries and clay pits).** **The DAYLIGHT FACTOR should be 10% IN KITCHENS and 8% in living rooms.**
...
**PEARL:** **YELLOW — human/animal ANATOMICAL waste, SOILED waste (dressings, cotton, plaster casts, blood bags), EXPIRED/discarded medicines, CHEMICAL and CYTOTOXIC waste, MICROBIOLOGY waste, discarded linen → INCINERATION/plasma pyrolysis/deep burial. RED — contaminated RECYCLABLE PLASTICS: IV tubing and sets, catheters, urine bags, SYRINGES WITHOUT NEEDLES, gloves, bottles → AUTOCLAVE/microwave, then SHRED and RECYCLE. WHITE (translucent, puncture-proof) — SHARPS: needles, scalpels, blades → autoclave then SHRED/encapsulate. BLUE — broken/discarded GLASSWARE and METALLIC IMPLANTS → disinfect and RECYCLE.**

**UNDERSTAND:** The colour code is organised by **FINAL DISPOSAL ROUTE, not by degree of contamination**. **Yellow goes to incineration** (things that must be destroyed), **red is autoclaved then shredded and recycled** (plastics that can be reclaimed), **white is disinfected then shredded** (things that can injure), and **blue is disinfected and recycled** (glass and metal, which cannot be incinerated).
...
**PEARL:** The tests used on pasteurised milk are the **PHOSPHATASE test** (checks pasteurisation was adequate), the **STANDARD PLATE COUNT** (total bacterial load) and the **METHYLENE BLUE REDUCTION test** (bacterial activity). **The IODINE test is for STARCH ADULTERATION, not pasteurisation.**

**UNDERSTAND:** The phosphatase test works because **alkaline phosphatase in raw milk is destroyed at a temperature just above that required to kill *Mycobacterium tuberculosis* and *Coxiella burnetii***. So if phosphatase is absent, the milk must have reached a pathogen-killing temperature — a biological thermometer that needs no equipment. A **POSITIVE phosphatase test therefore means UNDER-PASTEURISATION or contamination with raw milk.**
...
### 42. Water — Hardness, Disinfection and Sanitation Numbers
...
**PEARL:** **EPIDEMIC DROPSY = SANGUINARINE from *ARGEMONE MEXICANA* oil adulterating MUSTARD OIL.** It presents with **bilateral pitting leg oedema, diarrhoea, breathlessness, and characteristically GLAUCOMA (hypersecretory)** — with death from **cardiac failure**.

**UNDERSTAND:** Sanguinarine is a **CAPILLARY TOXIN** that increases vascular permeability throughout the body — hence oedema everywhere, including the eye, where excess aqueous production raises intraocular pressure. That same systemic capillary leak explains the cardiac failure and the pulmonary symptoms.
...
- **RELATIVE: anaemia, MENORRHAGIA, previous PID, purulent cervical discharge, a distorted cavity from fibroids, uterine malformations, previous ectopic pregnancy, an unmotivated user.**
...
- **Ideal timing: during or just after menstruation, immediately post-abortion, or POSTPARTUM (PPIUCD — within 48 HOURS or after 6 WEEKS).**
- **The COPPER IUCD is also the MOST EFFECTIVE EMERGENCY CONTRACEPTIVE — within 5 days.**
- **CONTRACEPTIVE PREVALENCE in India has risen to 69.1% (NFHS-6, 2023-24), from 66.7% in NFHS-5; the TOTAL FERTILITY RATE remains at 2.0 — below replacement level.**[^8]
...
**REMEMBER:** *Nikshay for TB, Nikusth for leprosy — and BPaLM turns two years of drug-resistant treatment into six months.*
...
### 49. Malaria — Treatment and Elimination
...
**UNDERSTAND:** The regional split exists because of **documented SULFADOXINE-PYRIMETHAMINE RESISTANCE IN THE NORTH-EAST** — the **partner drug**, not the artemisinin, is the problem. Using a failing partner drug exposes the artemisinin to selection pressure and breeds artemisinin resistance, which is why a different ACT is mandated there.

**MUST KNOW:**

- ***P. VIVAX*: CHLOROQUINE 25 mg/kg over 3 DAYS + PRIMAQUINE 0.25 mg/kg for 14 DAYS (radical cure of hypnozoites). CHECK G6PD before primaquine; CONTRAINDICATED in pregnancy and infants <6 months.**
- ***P. FALCIPARUM*: ACT for 3 days + SINGLE-DOSE PRIMAQUINE 0.75 mg/kg ON DAY 2 (gametocytocidal, to block transmission).** **ACT is contraindicated in the FIRST TRIMESTER and in children <5 kg.**
- ***P. MALARIAE* → treat as falciparum; *P. OVALE* → treat as vivax.**
- **SEVERE MALARIA: parenteral ARTESUNATE (drug of choice), artemether or quinine for at least 24 h, then a full oral ACT course.**
- **CHEMOPROPHYLAXIS: SHORT-TERM (≤6 weeks) DOXYCYCLINE daily, from 1 day before travel to 4 weeks after; LONG-TERM (>6 weeks) MEFLOQUINE weekly, from 2 weeks before to 4 weeks after.**
...
**PEARL:** The **TRANSMISSION ASSESSMENT SURVEY (TAS) belongs to the LYMPHATIC FILARIASIS elimination programme.** It decides **WHEN MASS DRUG ADMINISTRATION (MDA) CAN BE STOPPED** — whether infection has fallen below the threshold at which mosquitoes can sustain transmission — and afterwards serves as the **post-MDA SURVEILLANCE tool**.

**UNDERSTAND:** Filariasis elimination does not require killing every worm — it requires pushing prevalence **below the threshold at which a mosquito is unlikely to pick up and pass on the parasite**, after which transmission collapses on its own. TAS exists because that threshold must be **measured, not assumed**: it is surveyed in **6–7 YEAR-OLD CHILDREN**, since they were born after MDA began, so **any infection in them proves ONGOING transmission** rather than old infection.

**MUST KNOW:**

- **TAS is done after AT LEAST 5 EFFECTIVE ANNUAL ROUNDS of MDA with ≥65% coverage, and requires TWO FURTHER PASSES at 2–3 year intervals before a district can be declared free.**
- **ELIMINATION CRITERIA: (1) children born after the programme began are free of circulating ANTIGENAEMIA, and (2) the MICROFILARIA RATE IS <1%.**
...
- **MusQan: colourful child-friendly wards, play and recreation space, breastfeeding and KMC corners, facilities for parents to stay, privacy and cleanliness, family-centred care to reduce hospitalisation stress.**
- **LaQshya (Labour Room Quality Improvement Initiative, 2017): reduce maternal and neonatal mortality, prevent obstetric complications, ensure RESPECTFUL MATERNITY CARE, promote skin-to-skin contact and early breastfeeding.**
- **Kayakalp (2015): awards for cleanliness, hygiene, biomedical waste management and infection control; SWACHH SWASTH SARVATRA links it with Swachh Bharat.**
...
- **RDA FOR IODINE (ICMR-NIN 2020): 150 µg/day for adults, 250 µg in pregnancy, 280 µg in lactation** (WHO gives 250 µg for both pregnancy and lactation).
- **WEIGHT GAIN IN PREGNANCY (ICMR-NIN, Indian BMI cut-offs): normal BMI 18.5–23 → 10–12 kg; overweight (23–27.5) or obese (>27.5) → 5–9 kg.**
- **MID-DAY MEAL (now PM POSHAN) must supply at least ONE-THIRD of the daily energy and HALF of the daily protein requirement — a SUPPLEMENT, NOT A SUBSTITUTE for the home diet.**
...
### 53. Management Methods — Quantitative versus Behavioural
...
### 57. The Ishikawa (Fishbone) Diagram and Quality Tools
...
- **BAJAJ (1986) — health manpower planning and the Educational Commission for Health Sciences.**
...
- **ABC ANALYSIS classifies items by ANNUAL EXPENDITURE (A = 70% of cost but only 10% of items); VED ANALYSIS by CRITICALITY (Vital, Essential, Desirable). Combining them prioritises AV ITEMS for the closest control.**
- **Related tools: FSN (fast/slow/non-moving), HML (high/medium/low unit cost), and two-bin/expiry-date (FEFO) issue systems.**
...
- **ERRORS IN CERTIFICATION (writing a mode of dying, omitting the underlying cause, or the wrong sequence) are the commonest source of unusable mortality data** — the reason this is examined so persistently.
...
### 61. Demography — Dependency Ratio and Couple Protection Rate
...
**UNDERSTAND:** The denominator is **ONLY the economically productive age band, not the total population** — the distinction that trips up most candidates. The ratio expresses **how many dependants each 100 workers must support**, so a rising figure signals either a youthful, high-fertility population or an ageing one.
...
- **ASHA NEWBORN HOME VISITS: up to DAY 42 — 6 VISITS after institutional delivery (days 3, 7, 14, 21, 28, 42) and 7 after home delivery (adding day 1); more for at-risk newborns.** ASHA: a woman **25–45 years**, resident of the village, educated to about class 10.
...
### 62. Constitutional Articles Protecting Children
...
**UNDERSTAND:** The distinction tested is **whose right is being protected**. Articles 21A, 24 and 39(e)/(f) are framed around **the child**; **Article 42 is framed around the WORKER (and specifically the working mother)** — maternity benefit is a labour right, not a paediatric one.
...
*Footnotes below support the date-sensitive figures, programme names and guideline changes flagged in the pearls above. Where an official source lags behind a change (for example, departmental pages that still list the pre-HPV immunisation schedule), the more recent government announcement has been followed.*
...
[^8]: **National Family Health Survey (NFHS-6), 2023-24 factsheets**, International Institute for Population Sciences — full immunisation coverage 87.1%; rotavirus coverage 85.4%; stunting 29.3%, wasting 5.2%, underweight 31.8% in under-fives; contraceptive prevalence rate 69.1%; total fertility rate 2.0. <https://rchiips.org/nfhs/>

[^9]: **WHO Strategic Advisory Group of Experts (SAGE) on Immunization — updated HPV vaccination recommendations**: a single-dose schedule is an effective off-label alternative for girls aged 9-20 years; single-dose schedules had been adopted by around 89 countries by 2025. WHO position paper on human papillomavirus vaccines. <https://www.who.int/teams/immunization-vaccines-and-biologicals/diseases/human-papillomavirus-vaccines-(HPV)>

[^10]: **India TB Report 2025 and WHO Global TB Report 2025** — treatment coverage 53% (2015) to 92% (2024); TB mortality 28 to 21 per lakh population; incidence down ~21% and deaths ~25% against the 2015 baseline; treatment success rate 90%. The 2025 national elimination target was not achieved. <https://tbcindia.mohfw.gov.in/>

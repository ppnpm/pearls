# Medicine — Bookmark-Derived Pearls
...
## 1. Chronic Myeloid Leukaemia — The Philadelphia Chromosome
...
## 2. Acute Promyelocytic Leukaemia — The Bleeding Emergency
...
**MUST KNOW:**
- **ATRA works by DIFFERENTIATION, not cytotoxicity** — it relieves the PML-RARα block at the promyelocyte stage, allowing maturation. This makes APL the **most curable AML (>90% cure)** — a complete reversal of its historic status as the most lethal.
- **Watch for DIFFERENTIATION (retinoic acid) SYNDROME:** fever, dyspnoea, pulmonary infiltrates, weight gain, hypotension and renal failure within days of starting ATRA → treat with **dexamethasone**.
- **Support aggressively: platelets to keep >50 000, cryoprecipitate/fibrinogen, FFP** — do not wait for the count to fall further.
- **The other AML cytogenetics: t(8;21) (AML with maturation) and inv(16) (myelomonocytic with abnormal eosinophils) are FAVOURABLE-risk; NPM1 mutation without FLT3-ITD is favourable; FLT3-ITD and complex karyotype are adverse.**
- **AUER RODS are specific to AML** (crystallised azurophilic granules, MPO-positive) and are never seen in ALL.
...
## 3. Acute Lymphoblastic Leukaemia in a Child
...
**MUST KNOW:**
- **A child with bone pain + cytopenias + organomegaly must have a marrow examination** — this is the single most important clinical rule, because ALL is frequently first misdiagnosed as juvenile arthritis, ITP or osteomyelitis.
- **DOWN SYNDROME (trisomy 21) markedly increases the risk of both ALL and AML-M7 (acute megakaryoblastic leukaemia)** — a 10–20 fold excess; neonates may also have **transient abnormal myelopoiesis**.
...
## 4. Lymphoma Translocations and Markers
...
**EXAM CONNECTION:** Given a translocation, name the disease and its markers — **t(11;14) → mantle cell → CD5 and cyclin D1**.
...
**PEARL:** A child with **SEBORRHOEIC-LIKE SCALP/FLEXURAL RASH + PUNCHED-OUT LYTIC BONE LESIONS (classically SKULL) + recurrent otitis media/hearing loss ± hepatosplenomegaly and lymphadenopathy**. Biopsy: **histiocytes with BIRBECK GRANULES (tennis-racquet shaped) on electron microscopy**, staining **CD1a, S-100 and CD207 (langerin) positive**.

**UNDERSTAND:** LCH is a **clonal proliferation of Langerhans-type dendritic cells** (most carry a **BRAF V600E** mutation) — so it behaves like a neoplasm but presents with what looks like eczema and ear infection. The bone lesions are **osteolytic because the histiocytes recruit osteoclasts**; skull and mastoid involvement explains the deafness.
...
- **BRAF V600E mutations** make **BRAF inhibitors** effective in refractory disease; localised disease may need only curettage or observation.
...
## 6. Multiple Myeloma — Bortezomib and Hypercalcaemia
...
**MUST KNOW:**
- **Hypercalcaemia sequence: (1) AGGRESSIVE IV SALINE (these patients are profoundly volume-depleted from calcium-induced nephrogenic DI) → (2) IV BISPHOSPHONATE (zoledronic acid) or DENOSUMAB → (3) CALCITONIN for rapid but transient effect → (4) treat the underlying myeloma.** **Loop diuretics are NOT routine** — only after volume repletion, for fluid overload.
...
**PEARL:** Elderly patient, **painless lymphadenopathy, absolute LYMPHOCYTOSIS, and SMUDGE (basket) CELLS** on the smear. **Confirm with FLOW CYTOMETRY OF PERIPHERAL BLOOD** — a marrow biopsy is not needed for diagnosis. Immunophenotype: **CD19, CD20 (dim), CD23 and CD5 POSITIVE with light-chain restriction**.

**UNDERSTAND:** CLL circulates, so **the diagnostic material is already in the peripheral blood** — flow cytometry demonstrating a **monoclonal B-cell population (light chain restricted)** proves clonality and defines the phenotype in one inexpensive test. Marrow, FISH and node biopsy answer different questions (staging, prognosis, transformation).

**MUST KNOW:**
- **Smudge/basket cells are fragile lymphocytes crushed during smear preparation** — a classic morphological clue, though not specific.
...
## 8. Dry Tap on Bone Marrow Aspiration
...
**MUST KNOW:**
- **HAIRY CELL LEUKAEMIA:** middle-aged man, **MASSIVE SPLENOMEGALY WITHOUT lymphadenopathy, PANCYTOPENIA with MONOCYTOPENIA (characteristic)**, and cells with fine cytoplasmic projections that are **TRAP-POSITIVE (tartrate-resistant acid phosphatase)** and **CD11c, CD25, CD103 positive**, with a **BRAF V600E** mutation. **Treatment: CLADRIBINE** (a purine analogue) — dramatically effective.
- **PRIMARY MYELOFIBROSIS:** **JAK2, CALR or MPL** mutation; **massive splenomegaly with EXTRAMEDULLARY HAEMATOPOIESIS**, and a **LEUCOERYTHROBLASTIC blood picture with TEARDROP CELLS (dacrocytes)**. Treat with **ruxolitinib**.
- **AML-M7** is strongly associated with **DOWN SYNDROME** in children.
- **Always send the trephine for reticulin staining** when a dry tap occurs — that is the diagnostic step.
- **Follicular lymphoma infiltrates the marrow (paratrabecular aggregates) but does NOT usually cause fibrosis** — hence it is the exception in the question.
...
## 9. Chloroma (Myeloid Sarcoma)

**PEARL:** **Chloroma = granulocytic/myeloid sarcoma — an extramedullary tumour of immature myeloid cells**, classically causing **BILATERAL PROPTOSIS in a child**. It signifies **acute myeloid leukaemia**, so the **next investigation is BONE MARROW EXAMINATION**.

**UNDERSTAND:** A chloroma is **AML growing outside the marrow** — so finding one is not the end of the diagnostic process but the beginning: you must establish the **underlying systemic disease, the blast percentage, and the cytogenetics** that will determine treatment. Flow cytometry and molecular studies are then performed **on the marrow sample**.
...
**PEARL:** **Progressive weakness with BOTH UPPER MOTOR NEURON signs (hypertonia, hyperreflexia, extensor plantars) AND LOWER MOTOR NEURON signs (wasting, FASCICULATIONS) — with a COMPLETELY NORMAL SENSORY SYSTEM.** That combination in one patient is ALS.

**UNDERSTAND:** ALS is the only common disease that **simultaneously destroys the corticospinal tract and the anterior horn cell**. Hence the paradoxical examination — a **wasted, fasciculating limb that is also spastic with brisk reflexes**. Any sensory loss should make you doubt the diagnosis.
...
## 11. Normal Pressure Hydrocephalus vs Binswanger Disease
...
- **Imaging distinction:** NPH — **ventriculomegaly OUT OF PROPORTION to atrophy, with tight high-convexity sulci (DESH sign) and an increased Evans index >0.3**. Cerebral atrophy — ventriculomegaly **WITH** widened sulci (*hydrocephalus ex vacuo*).
...
## 13. Alzheimer Disease Genetics

**PEARL:** **APP → chromosome 21 · PRESENILIN-1 → chromosome 14 · PRESENILIN-2 → chromosome 1 · APOE → chromosome 19 · LRP1 → chromosome 12.**

**UNDERSTAND:** The three **autosomal dominant, EARLY-ONSET familial** genes (**APP, PSEN1, PSEN2**) all sit in the **amyloid production pathway** — they increase generation of the toxic **Aβ42** peptide. **APOE is different: it is a RISK MODIFIER for LATE-ONSET sporadic disease**, affecting amyloid clearance — not a deterministic mutation.

**MUST KNOW:**
- **APP on chromosome 21 explains why virtually everyone with DOWN SYNDROME develops Alzheimer pathology by age 40** (three copies of APP = lifelong overproduction of amyloid) — the single most testable consequence of this list.
- **PSEN1 (chromosome 14) is the commonest cause of early-onset familial Alzheimer disease**, and typically presents in the 40s.
- **APOE ε4 is the strongest genetic RISK factor for late-onset AD** (one allele ~3×, two alleles ~12×); **APOE ε2 is PROTECTIVE**. **APOE genotyping is NOT recommended for routine diagnosis or screening** — it changes nothing clinically and creates harm.
- **Pathology: extracellular β-amyloid (senile/neuritic) plaques and intracellular NEUROFIBRILLARY TANGLES of hyperphosphorylated tau**, with **hippocampal and medial temporal atrophy first**, and loss of **cholinergic neurons in the nucleus basalis of Meynert** (the rationale for cholinesterase inhibitors).
- **Treatment: donepezil, rivastigmine, galantamine (cholinesterase inhibitors) and MEMANTINE (NMDA antagonist) for moderate–severe disease**; the **anti-amyloid monoclonals (lecanemab, donanemab)** modestly slow decline with a risk of **ARIA (amyloid-related imaging abnormalities)**.
...
## 14. Headache — Cluster and Tension-Type

**PEARL:** **CLUSTER headache — acute attack: SUBCUTANEOUS SUMATRIPTAN (first-line, fastest) or 100% OXYGEN at 10–15 L/min via a non-rebreather mask.** **Prophylaxis: VERAPAMIL** (drug of choice). **TENSION-TYPE headache — acute: paracetamol/NSAIDs and relaxation techniques; AMITRIPTYLINE is for PROPHYLAXIS of CHRONIC tension-type headache only (≥15 days/month for >3 months), not for episodic attacks.**

**UNDERSTAND:** Cluster attacks **peak within minutes and last 15–180 minutes**, so an **oral drug is simply too slow** — this is why the route, not the drug, is the answer. Similarly, amitriptyline takes weeks to work, so it is useless for an acute attack and is reserved for frequent headache.
...
## 15. Rimegepant and the Gepants

**PEARL:** **Rimegepant is an ORAL CGRP RECEPTOR ANTAGONIST used in the ACUTE treatment of migraine** (and, uniquely, also for prevention). It is **NOT an agonist, causes NO VASOCONSTRICTION, and is given ORALLY — not subcutaneously.**

**UNDERSTAND:** **CGRP released from trigeminal nerve endings causes vasodilatation, neurogenic inflammation and pain transmission** in migraine. Gepants block its receptor. **Because they do not constrict blood vessels, they are SAFE IN PATIENTS WITH CARDIOVASCULAR DISEASE** — precisely the group in whom **triptans (5-HT1B/1D agonists, which DO vasoconstrict) are contraindicated**. That is the entire clinical reason the class exists.

**MUST KNOW:**
- **Compare the three anti-CGRP approaches:** **GEPANTS (rimegepant, ubrogepant, atogepant) — small molecules, ORAL, acute ± preventive**; **CGRP MONOCLONAL ANTIBODIES (erenumab targets the receptor; fremanezumab, galcanezumab, eptinezumab target the ligand) — SUBCUTANEOUS/IV, PREVENTIVE only, given monthly/quarterly**; **TRIPTANS — 5-HT1B/1D agonists, acute only, vasoconstrictive**.
- **Triptan contraindications: ischaemic heart disease, uncontrolled hypertension, previous stroke, Prinzmetal angina, hemiplegic and basilar migraine, and within 24 hours of an ergot.**
- **Ditans (lasmiditan)** are the other new acute class — **5-HT1F agonists**, also non-vasoconstrictive, but they cause **sedation and driving restriction**.
...
## 16. WHO Analgesic Ladder

**PEARL:** **Step 1 — non-opioids (paracetamol, NSAIDs) ± adjuvants for MILD pain. Step 2 — WEAK OPIOIDS (tramadol, codeine) PLUS the step-1 drugs, for MODERATE pain. Step 3 — STRONG OPIOIDS (morphine, fentanyl) PLUS step-1 drugs, for SEVERE pain.** Move **ONE step up** when pain is uncontrolled; **do not simply increase the NSAID dose**.

**UNDERSTAND:** Non-opioids have a **CEILING EFFECT** — beyond a certain dose you gain toxicity (GI bleeding, renal injury, hepatotoxicity) without extra analgesia. **Opioids have no analgesic ceiling.** So the correct response to failure at step 1 is to **add a different mechanism**, not to push the same drug harder.
...
## 18. Radical Cure in Malaria

**PEARL:** **Artemisinins, chloroquine and sulfadoxine–pyrimethamine achieve CLINICAL cure by killing the ERYTHROCYTIC (asexual blood) stages.** **PRIMAQUINE does NOT produce clinical cure** — it is the **only drug that eradicates the LIVER HYPNOZOITES of *P. vivax* and *P. ovale*, giving RADICAL cure and preventing relapse.**

**UNDERSTAND:** The distinction the question tests is **clinical cure (relief of the acute attack — a blood-stage action) versus radical cure (preventing relapse — a liver-stage action)**. Primaquine has almost no effect on the blood stages, so it cannot treat the acute illness on its own; conversely, the blood schizonticides cannot touch dormant hypnozoites.
...
## 19. Headache Red Flags — SNOOP

**PEARL:** **SNOOP: Systemic symptoms (fever, weight loss, cancer, HIV) · Neurological signs · Onset sudden (thunderclap) · Older age >50 · Progression/Pattern change (and Papilloedema, Positional, Precipitated by Valsalva).** **TEMPORAL TENDERNESS, AGE >50 and PROGRESSIVE WORSENING are red flags. CHRONIC COCAINE USE ALONE IS NOT.**

**UNDERSTAND:** Red flags are features that make a **SECONDARY (structural, vascular or inflammatory) headache** likely rather than a primary headache. Cocaine can certainly cause headache — and dangerous ones (haemorrhage, RCVS) — but **the history of use alone, without any accompanying feature, does not change the probability of serious pathology**; it needs a red flag beside it.
...
**MUST KNOW:**
- **Commonest aneurysm sites: ANTERIOR COMMUNICATING artery (commonest overall) > posterior communicating > MCA bifurcation.** A **posterior communicating artery aneurysm classically causes a PAINFUL THIRD NERVE PALSY WITH A DILATED PUPIL** (pupillary fibres run superficially and are compressed first) — an aneurysm until proven otherwise.
- **Diagnosis: NCCT within 6 hours is >95% sensitive; beyond that, LP for XANTHOCHROMIA (spectrophotometry) at ≥12 hours.** Then **CT angiography/DSA** to find the aneurysm.
- **Complications, each with its own timing: RE-BLEEDING (highest in the first 24 h — secure the aneurysm early by CLIPPING or ENDOVASCULAR COILING), VASOSPASM and delayed cerebral ischaemia (day 3–14 — prevent with oral NIMODIPINE), HYDROCEPHALUS (early or late — EVD/shunt), HYPONATRAEMIA (cerebral salt wasting or SIADH), and seizures.**
- **NIMODIPINE improves outcome in SAH by reducing delayed cerebral ischaemia** — a guaranteed exam point. **Do not fluid-restrict for the hyponatraemia** (it worsens ischaemia).
- **Risk factors: hypertension, smoking, alcohol, ADPKD, Ehlers–Danlos type IV, coarctation of the aorta, and a family history.**
- **Grading: Hunt and Hess (clinical), WFNS, and the Fisher scale (CT blood load, predicting vasospasm).**

**EXAM CONNECTION:** Young patient + sudden headache + neck stiffness + **hydrocephalus** → SAH (not meningitis, which evolves over hours to days with fever).
...
## 21. Guillain–Barré Syndrome and Miller–Fisher Variant
...
**UNDERSTAND:** **Anti-GQ1b antibodies target ganglioside GQ1b, which is heavily concentrated in the myelin of cranial nerves III, IV and VI** — which is precisely why this variant presents with **ophthalmoplegia** rather than limb weakness. Antibody specificity dictates clinical pattern.
...
- **STEROIDS ARE INEFFECTIVE AND ARE NOT USED in GBS** — a key negative and a classic exam trap (contrast with CIDP, where they work).
...
## 22. Suspected Meningitis — The Order of Actions

**PEARL:** **(1) SECURE IV ACCESS AND STABILISE → (2) FUNDOSCOPY (look for papilloedema/raised ICP) → (3) GUARDED LUMBAR PUNCTURE → (4) START CEFTRIAXONE.** The purpose of the sequence is to **avoid CONING (uncal/tonsillar herniation) by never doing an LP in the presence of raised ICP**.

**UNDERSTAND:** In practice the overriding rule is that **ANTIBIOTICS MUST NEVER BE DELAYED for imaging or LP** — blood cultures are taken and the antibiotic given, because CSF sterilises slowly enough that the diagnosis is not lost. When the exam presents a stable patient with no focal signs, the intended sequence is as above; when the patient is sick or CT is needed, **antibiotics come first**.

**MUST KNOW:**
- **Indications for CT BEFORE LP: focal neurological deficit, new-onset seizure, papilloedema, GCS <10 or altered consciousness, immunocompromise, or a history of CNS disease.** **In all of these, GIVE ANTIBIOTICS FIRST.**
...
## 23. Meningococcal Chemoprophylaxis

**PEARL:** **RIFAMPICIN is the drug of choice for chemoprophylaxis of close contacts** of meningococcal disease; **ciprofloxacin (single dose) and ceftriaxone (single IM dose, preferred in PREGNANCY) are the alternatives.**

**UNDERSTAND:** Prophylaxis works by **eradicating NASOPHARYNGEAL CARRIAGE**, not by treating disease. That is why the drug must **penetrate respiratory secretions** — penicillin, though it treats meningococcal meningitis effectively, **does NOT reliably clear the carrier state**, and so is useless for prophylaxis. It is also why the **index case must receive prophylaxis before discharge** unless treated with ceftriaxone.

**MUST KNOW:**
- **Who gets it: household contacts, those sharing a dormitory/barracks, intimate (kissing) contacts, and healthcare workers with DIRECT exposure to respiratory secretions (mouth-to-mouth, intubation, suctioning WITHOUT a mask)** — not the whole ward or casual contacts.
- **Give it as soon as possible, ideally within 24 hours**; it is of little value after 14 days.
- **Doses: rifampicin 600 mg 12-hourly for 2 days (adults); ciprofloxacin 500 mg single dose; ceftriaxone 250 mg IM single dose.** Warn about **orange discoloration of urine, tears and contact lenses, and the reduced efficacy of oral contraceptives** with rifampicin.
- **Recognise the disease: fever, headache, neck stiffness and a NON-BLANCHING PETECHIAL/PURPURIC RASH.** **Purpura fulminans and WATERHOUSE–FRIDERICHSEN SYNDROME (bilateral adrenal haemorrhage with shock)** are the fulminant forms.
- **Give the first dose of antibiotic (benzylpenicillin or ceftriaxone) IMMEDIATELY in the community** when meningococcal disease is suspected with a petechial rash — do not wait for hospital transfer.
...
## 24. Multiple Sclerosis — Presentation and Ocrelizumab
...
## 25. Compressive Myelopathy and Spinal Shock
...
- **The GATE CONTROL THEORY (Melzack and Wall)** explains why rubbing a painful area helps: **large Aβ fibre activity inhibits transmission of nociceptive input in the dorsal horn** — the physiological basis of **TENS**.
...
**PEARL:** Delirium = **acute, FLUCTUATING disturbance of ATTENTION and AWARENESS** with **IMPAIRED CONSCIOUSNESS, ALTERED SLEEP–WAKE CYCLE (day–night reversal), CARPHOLOGIA (picking at bedclothes), AUTONOMIC DYSFUNCTION**, and **DIFFUSE SLOWING ON EEG — the EEG is NOT normal.**

**UNDERSTAND:** The **EEG is the discriminator from dementia and from psychiatric illness**: delirium is a **global disturbance of cortical function**, so it slows the background rhythm. In **dementia the EEG is normal early**, and in primary psychiatric disease it is normal. **The exception worth knowing is DELIRIUM TREMENS, where the EEG shows FAST activity.**
...
## 28. Acute Ischaemic Stroke — CT Angiography

**PEARL:** In acute stroke with a **clear NCCT**, the next step is **CT ANGIOGRAPHY to identify a LARGE VESSEL OCCLUSION** — performed **concurrently with, not instead of, thrombolysis**. **Thrombolysis (alteplase/tenecteplase) within 4.5 hours; MECHANICAL THROMBECTOMY for large vessel occlusion, ideally within 6 hours and selected cases up to 24 hours.**

**UNDERSTAND:** **Large clots in proximal vessels (ICA terminus, M1) recanalise poorly with lysis alone** — the drug cannot penetrate a large thrombus. So identifying an LVO changes the whole pathway: the patient needs **transfer to a thrombectomy centre**. Thrombolysis is therefore given **while** the angiogram is done, not after.

**MUST KNOW:**
- **NCCT's job is to EXCLUDE HAEMORRHAGE**, not to show the infarct (which is often invisible for hours). **Early signs: hyperdense MCA sign, loss of grey–white differentiation, insular ribbon sign, sulcal effacement.** **ASPECTS score** quantifies early ischaemic change.
...
**MUST KNOW:**
- **The interpretation table:** **Acute infection — HBsAg+, anti-HBc IgM+, HBeAg+.** **Window period — anti-HBc IgM+ ONLY.** **Recovered/immune — anti-HBs+ AND anti-HBc IgG+.** **VACCINATED — anti-HBs+ ONLY (anti-HBc NEGATIVE)** — this is the distinction that is most often tested. **Chronic infection — HBsAg+ for >6 months with anti-HBc IgG+.**
- **HBeAg indicates ACTIVE VIRAL REPLICATION and HIGH INFECTIVITY** — an HBeAg-positive mother has a **~90% transmission risk** to her infant versus ~10–20% if negative.
...
**MUST KNOW:**
- **The DIMORPHIC picture (a population of hypochromic microcytes alongside normal/macrocytic cells) is the clue on the smear** — and is also seen in mixed deficiency and after transfusion.
- **Causes: HEREDITARY (X-linked, ALAS2 — often PYRIDOXINE-RESPONSIVE)** and **ACQUIRED — MYELODYSPLASTIC SYNDROME (MDS-RS, associated with SF3B1 mutations), LEAD poisoning, ALCOHOL, ISONIAZID and other B6 antagonists, chloramphenicol, linezolid, COPPER DEFICIENCY (or zinc excess) and hypothermia.**
- **Always ask about isoniazid and alcohol** — the two commonest reversible causes; **give pyridoxine** in INH-associated and in hereditary forms.
...
## 31. Anteroseptal Myocardial Infarction
...
**MUST KNOW:**
- **The territory map: V1–V2 SEPTAL · V3–V4 ANTERIOR · V1–V4 ANTEROSEPTAL · V5–V6, I, aVL LATERAL · V1–V6 + I + aVL EXTENSIVE ANTERIOR (proximal LAD) · II, III, aVF INFERIOR (RIGHT CORONARY in ~80%, left circumflex in 20%) · tall R with ST DEPRESSION in V1–V2 = POSTERIOR (confirm with V7–V9).**
- **INFERIOR MI: always do RIGHT-SIDED LEADS (V4R) to detect RIGHT VENTRICULAR infarction** — because RV infarction is **preload-dependent**, so **NITRATES AND DIURETICS CAUSE PROFOUND HYPOTENSION and are contraindicated; give FLUIDS instead.** Inferior MI also causes **bradycardia and heart block** (the AV node is RCA-supplied) — usually transient and atropine-responsive.
- **STEMI criteria: ≥1 mm ST elevation in 2 contiguous limb leads, or ≥2 mm in contiguous chest leads (≥2.5 mm in men <40).** **New LBBB with symptoms** is treated as STEMI (Sgarbossa criteria).
- **Reciprocal change strongly supports a true infarct** rather than pericarditis or early repolarisation — **pericarditis gives WIDESPREAD saddle-shaped ST elevation with PR depression and NO reciprocal change**.
- **Management: PRIMARY PCI within 90–120 minutes is the goal**; thrombolysis if PCI is unavailable within that window. Plus **dual antiplatelet therapy, anticoagulation, statin, beta-blocker and ACE inhibitor**.
- **Anterior MI has the WORST prognosis** (largest territory, most pump failure) and is more likely to cause **VSD, LV aneurysm and mural thrombus**.

**EXAM CONNECTION:** ECG image → read which leads are elevated → **V1–V4 = anteroseptal**.
...
## 32. Pneumonia in HIV — Read the CD4 and the Cough

**PEARL:** **STREPTOCOCCUS PNEUMONIAE remains the commonest cause of bacterial pneumonia in HIV, at ANY CD4 count** — a **PRODUCTIVE cough with LOBAR CONSOLIDATION** is bacterial. ***PNEUMOCYSTIS JIROVECII* causes a DRY cough with DIFFUSE INTERSTITIAL infiltrates and a CD4 <200.**

**UNDERSTAND:** The exam always supplies two discriminators: **the character of the cough and the radiographic pattern**. **Consolidation = alveolar = bacterial**; **interstitial/ground-glass = PCP**. A low CD4 count tells you the patient is *at risk* of opportunistic infection, but it does not override the clinical and radiological pattern.

**MUST KNOW:**
- **HIV patients have an increased risk of pneumococcal disease at all CD4 counts** (from impaired B-cell function and opsonisation) — hence **pneumococcal vaccination** is indicated.
- **PCP: DRY cough, exertional DYSPNOEA and DESATURATION ON EXERTION, a normal chest examination, RAISED LDH, and diffuse bilateral perihilar ground-glass change with a normal CXR in up to 25%.** **Diagnosis by induced sputum/BAL with silver or immunofluorescent staining, or PCR.** **Treatment: high-dose CO-TRIMOXAZOLE; ADD STEROIDS if PaO2 <70 mmHg or A-a gradient >35** — the steroid indication is a guaranteed exam point. **Prophylaxis with co-trimoxazole when CD4 <200.**
- **CD4 thresholds worth owning: <200 — PCP; <100 — TOXOPLASMOSIS and cryptococcus; <50 — CMV retinitis and MAC.**
...
**MUST KNOW:**
- **Insulin is the ONLY hypoglycaemic hormone** — glucagon, adrenaline, cortisol and growth hormone all oppose it.
- **Insulin drives K⁺ INTO cells (via Na⁺/K⁺-ATPase)** — the basis of the **insulin–dextrose infusion for HYPERKALAEMIA**, and of the hypokalaemia seen while treating DKA.
- **It ACTIVATES lipoprotein lipase (clearing circulating triglyceride into adipose tissue) while INHIBITING hormone-sensitive lipase** — two lipases with opposite regulation, a classic point of confusion.
- **Insulin ACTIVATES pyruvate dehydrogenase** (generating acetyl-CoA for fat synthesis) — it does not inhibit it.
- **Receptor: a tyrosine kinase (α2β2)** → IRS-1 → **PI3K/Akt (metabolic effects, GLUT-4 translocation)** and **MAPK (growth)**.
...
## 34. Myocarditis — Diagnostic Categories
...
**MUST KNOW:**
- **Endomyocardial biopsy is the gold standard but is used selectively** (sampling error, risk) — reserved for **fulminant, rapidly deteriorating or unexplained heart failure with arrhythmia/heart block, and suspected GIANT CELL myocarditis or eosinophilic myocarditis**, which are treatable with immunosuppression.
- **CARDIAC MRI is the non-invasive investigation of choice** — the **LAKE LOUISE criteria** (oedema on T2, hyperaemia, and **LATE GADOLINIUM ENHANCEMENT in a SUBEPICARDIAL/mid-wall, non-coronary distribution** — as opposed to the subendocardial pattern of infarction).
- **Aetiology: VIRAL is commonest (COXSACKIE B and other enteroviruses, adenovirus, parvovirus B19, HHV-6, COVID-19)**; also **Chagas disease** (a major cause worldwide), diphtheria, Lyme disease, **drugs (anthracyclines, clozapine, IMMUNE CHECKPOINT INHIBITORS — a highly fatal form)**, and autoimmune disease.
- **Myocarditis is a leading cause of SUDDEN CARDIAC DEATH IN YOUNG ATHLETES** — hence the universal advice to **avoid strenuous exercise for 3–6 months** after diagnosis.
...
## 35. Severity Scores in Acute Pancreatitis
...
## 36. Heavy Chain Diseases

**PEARL:** **ALPHA heavy chain disease is the COMMONEST** — also called **SELIGMANN DISEASE, Mediterranean lymphoma, or IMMUNOPROLIFERATIVE SMALL INTESTINAL DISEASE (IPSID)**, and it is a form of **MALT lymphoma of the small bowel**. **Gamma (FRANKLIN) disease** and **mu heavy chain disease** are rarer.

**UNDERSTAND:** In heavy chain disease the clone produces a **truncated immunoglobulin HEAVY chain with NO bound LIGHT chain** — the mirror image of light-chain (Bence Jones) myeloma. Because the alpha isotype is the **mucosal immunoglobulin (IgA)**, alpha HCD is a **disease of the gut mucosa** — which is why it presents with malabsorption rather than bone disease.
...
## 37. Cardiac Tamponade — The ECG Triad

**PEARL:** The classic ECG triad of pericardial effusion with tamponade is **SINUS TACHYCARDIA + LOW-VOLTAGE QRS COMPLEXES + ELECTRICAL ALTERNANS**. A prolonged ST segment is **not** part of it.

**UNDERSTAND:** Each element has a mechanical explanation. **Low voltage** occurs because the surrounding fluid **insulates and short-circuits** the cardiac electrical signal. **Electrical alternans** — beat-to-beat variation in QRS amplitude — occurs because the heart is **literally swinging back and forth within the fluid**, changing its orientation to the electrodes with each beat. **Tachycardia** is the compensatory response to a fixed, low stroke volume.
...
**PEARL:** Treatment is **RAPID REWARMING by immersion in water at 37–40°C for 30–60 minutes**, plus **analgesia (ibuprofen), ANTIBIOTICS where indicated, tetanus prophylaxis, and amputation only in severe established necrosis**. **Antibiotics and analgesics ARE used** — the statement denying them is false.

**UNDERSTAND:** Tissue injury has **two components: direct freezing (ice crystal formation) and progressive vascular stasis with thrombosis and inflammatory mediator release**. Rewarming addresses the first; **ibuprofen matters specifically because it blocks thromboxane**, which drives the vasoconstriction and microvascular thrombosis of the second phase. So the analgesic is also a therapeutic agent.
...
- **Distinguish from TRENCH/IMMERSION FOOT (non-freezing cold injury, prolonged wet cold) and CHILBLAINS/pernio.**
...
## 39. MEN Syndromes — 2A vs 2B

**PEARL:** **MEN 2A (Sipple) = MEDULLARY THYROID CARCINOMA + PHAEOCHROMOCYTOMA + PARATHYROID hyperplasia/adenoma.** **MEN 2B = MTC + PHAEOCHROMOCYTOMA + MUCOSAL NEUROMAS + MARFANOID habitus + MEGACOLON + medullated corneal nerve fibres — but NO parathyroid disease.** Both are **RET proto-oncogene** gain-of-function mutations (10q11.2), autosomal dominant.

**UNDERSTAND:** **The parathyroid is the discriminator.** MEN 2B swaps parathyroid involvement for a **neural/mesenchymal phenotype** (neuromas, marfanoid build, gut ganglioneuromatosis causing megacolon) — reflecting a different RET codon mutation (M918T) with more aggressive neural crest effects.

**MUST KNOW:**
- **MTC in MEN 2B is the most aggressive form** — it can metastasise in infancy, which is why **PROPHYLACTIC TOTAL THYROIDECTOMY is recommended in the FIRST YEAR OF LIFE for MEN 2B and by age 5 for MEN 2A**. This is one of the clearest examples of genotype-directed prophylactic surgery.
- **ALWAYS EXCLUDE AND TREAT THE PHAEOCHROMOCYTOMA FIRST** before any other surgery — operating on an unblocked phaeochromocytoma causes fatal hypertensive crisis. **α-blockade (phenoxybenzamine) before β-blockade**, never the reverse.
- **MTC arises from PARAFOLLICULAR C CELLS and secretes CALCITONIN** (the tumour marker, with CEA); histology shows **amyloid stroma**. **It does NOT take up radioiodine** — so RAI therapy is useless.
- **MEN 1 (Wermer) = the 3 Ps: PITUITARY + PARATHYROID + PANCREAS** (gastrinoma, insulinoma) — **MEN1 gene, a TUMOUR SUPPRESSOR on chromosome 11**. Note the contrast: **MEN 1 is a tumour suppressor loss; MEN 2 is an oncogene gain-of-function.**
...
## 40. SAAG — Serum-Ascites Albumin Gradient

**PEARL:** **SAAG = serum albumin − ascitic fluid albumin.** **SAAG ≥1.1 g/dL = PORTAL HYPERTENSION** (cirrhosis, cardiac failure, Budd–Chiari, massive liver metastases). **SAAG <1.1 g/dL = NON-portal causes** (peritoneal carcinomatosis, tuberculous peritonitis, pancreatic ascites, nephrotic syndrome, serositis).

**UNDERSTAND:** SAAG measures the **oncotic–hydrostatic balance across the sinusoids**. In portal hypertension the high sinusoidal pressure drives out a **protein-POOR ultrafiltrate**, so the gradient between serum and ascites albumin is **wide**. When the peritoneum itself is diseased (cancer, TB), leaky capillaries release **protein-RICH fluid**, so the gradient is **narrow**.

**MUST KNOW:**
- **SAAG has replaced the old transudate/exudate classification** — it is ~97% accurate, whereas total protein is not.
- **Add total protein to refine it: SAAG ≥1.1 WITH ascitic protein ≥2.5 g/dL suggests CARDIAC ascites or Budd–Chiari (the sinusoids are still intact and leak protein); SAAG ≥1.1 with protein <2.5 suggests CIRRHOSIS.**
- **SPONTANEOUS BACTERIAL PERITONITIS: ascitic PMN count ≥250/mm³** — treat with **cefotaxime plus IV ALBUMIN (1.5 g/kg day 1 and 1 g/kg day 3, which reduces hepatorenal syndrome and mortality)**. Usually a **single organism** (*E. coli*, Klebsiella, pneumococcus); a **polymicrobial growth with high protein and low glucose suggests SECONDARY bacterial peritonitis** needing imaging and surgery.
- **Prophylaxis with norfloxacin** after an episode of SBP, with variceal bleeding, or when ascitic protein <1.5 g/dL with organ dysfunction.
- **Management of cirrhotic ascites: SALT RESTRICTION (<2 g/day) + SPIRONOLACTONE ± FUROSEMIDE in a 100:40 ratio** (spironolactone is the key drug — it opposes the hyperaldosteronism); then **large-volume paracentesis with albumin cover (6–8 g per litre removed)**, and **TIPS** for refractory ascites. **Fluid restriction only if Na <125.**
- **Tuberculous peritonitis: high ascitic ADA and lymphocytosis with a LOW SAAG.**

**EXAM CONNECTION:** Compute it — 3.5 − 1.8 = 1.7, which is **≥1.1** → portal hypertension → **cirrhosis**.
...
## 41. Oxford (Bamford) Stroke Classification

**PEARL:** **TOTAL ANTERIOR CIRCULATION STROKE (TACS) requires ALL THREE: (1) higher cortical dysfunction (aphasia or NEGLECT), (2) HOMONYMOUS HEMIANOPIA, and (3) motor and/or sensory deficit of at least 2 of face/arm/leg.** **PACS = 2 of 3 (or isolated cortical dysfunction).** **LACS = a pure motor, pure sensory, sensorimotor or ataxic hemiparesis with NO cortical signs.** **POCS = brainstem/cerebellar/occipital signs.**

**UNDERSTAND:** The classification is **purely clinical and requires no imaging** — yet it predicts **territory, prognosis and mortality** better than most scores. **The presence or absence of CORTICAL signs (aphasia, neglect, hemianopia) is the pivot**: cortex means a large-vessel occlusion; no cortex means a small perforating vessel (lacune).

**MUST KNOW:**
- **TACS has the worst prognosis: ~60% mortality at 1 year and only ~5% functional independence.** **LACS has the best short-term prognosis but the highest recurrence.**
- **LACUNAR strokes arise from LIPOHYALINOSIS of small penetrating arteries** (hypertension, diabetes) in the **internal capsule, basal ganglia, thalamus and pons** — and **never cause aphasia, neglect or visual field loss**, because they spare the cortex. The five classic lacunar syndromes: **pure motor hemiparesis (posterior limb of internal capsule), pure sensory (thalamic VPL), sensorimotor, ataxic hemiparesis, and dysarthria–clumsy hand.**
- **Cortical signs by hemisphere: DOMINANT (left) — APHASIA; NON-DOMINANT (right) — NEGLECT, anosognosia, constructional apraxia, dressing apraxia.**
- **Gaze deviation: in a cortical (MCA) stroke the eyes look TOWARDS the lesion (away from the hemiparesis); in a PONTINE stroke they look AWAY from the lesion (towards the weak side).**
- **Posterior circulation clues: crossed signs, diplopia, vertigo, dysarthria, dysphagia, ataxia, and cortical blindness** — and **posterior circulation strokes are the most often MISSED** in casualty.
- **In haemorrhagic stroke: NO thrombolysis; control BP to <140 systolic; manage oedema; ELECTIVE HYPERVENTILATION is only a brief temporising measure for herniation, not routine therapy.**
...
## 42. Diabetic Ketoacidosis — Grading Severity

**PEARL:** **MILD — pH 7.25–7.30, bicarbonate 15–18, alert.** **MODERATE — pH 7.00–7.24, bicarbonate 10–15, alert or drowsy.** **SEVERE — pH <7.00, bicarbonate <10, stupor or coma.** Anion gap is raised (>10–12) in all.

**UNDERSTAND:** Severity is graded by **acidaemia and mental status, NOT by the blood glucose**. This matters because **EUGLYCAEMIC DKA** exists — with **SGLT-2 inhibitors, pregnancy, starvation and alcohol** — where glucose may be near-normal while the patient is profoundly acidotic. Anchoring on the sugar is the classic clinical error.

**MUST KNOW:**
- **Management priorities in order: FLUIDS FIRST (normal saline — these patients are 5–7 L depleted), then INSULIN infusion (0.1 U/kg/h), and POTASSIUM.**
- **The potassium rule is the most examined point: total body potassium is DEPLETED even when the serum level is normal or high** (acidosis and insulin deficiency shift K⁺ out of cells). **Insulin drives it back in and can cause fatal hypokalaemia — so if K⁺ <3.3 mEq/L, HOLD the insulin and replace potassium first.** Add potassium to fluids once K⁺ <5.2 and urine output is adequate.
...
**EXAM CONNECTION:** Read off the pH (7.1) and bicarbonate (12) → **moderate DKA**.
...
- **Investigation: DaT scan shows reduced dopamine transporter uptake in the basal ganglia** (normal in Alzheimer disease); **MIBG cardiac scintigraphy** shows reduced uptake.
- **DLB is the SECOND commonest degenerative dementia after Alzheimer disease.**
...
## 44. NYHA vs mMRC — Grading Breathlessness

**PEARL:** **NYHA grades CARDIAC functional status: I — no limitation · II — symptoms on ORDINARY activity · III — symptoms on LESS THAN ordinary activity (e.g. walking to the bathroom, dressing) · IV — symptoms AT REST.** **mMRC grades RESPIRATORY (COPD) breathlessness on a 0–4 scale.**

**UNDERSTAND:** The two scales are frequently offered together as distractors because both grade dyspnoea — but **NYHA is for heart failure and mMRC for chronic lung disease**, and **mMRC has NO grade 5** (it runs 0 to 4). Spotting the invalid grade eliminates options instantly.
...
**EXAM CONNECTION:** Breathless on an ordinary daily activity in a cardiac patient → **NYHA III** (and mMRC 5 does not exist).
...
## 45. Stages of Type 1 Diabetes

**PEARL:** **STAGE 1 — autoantibody POSITIVE with NORMOGLYCAEMIA (asymptomatic).** **STAGE 2 — autoantibody positive with DYSGLYCAEMIA (impaired glucose tolerance, still asymptomatic).** **STAGE 3 — autoantibody positive with dysglycaemia AND CLINICAL SYMPTOMS — i.e. overt diabetes.**

**UNDERSTAND:** Type 1 diabetes is now understood as a **continuum beginning years before diagnosis**. Autoantibodies appear first, β-cell mass declines silently, and **symptoms appear only when ~80–90% of β-cell function is lost**. Staging exists because it creates a **window for intervention before clinical disease**.
...
## 46. Serological Workup for Acute Viral Hepatitis

**PEARL:** The standard panel is **IgM ANTI-HAV + HBsAg + IgM ANTI-HBc + ANTI-HCV**. **Hepatitis D and E are NOT tested routinely** — HDV only when hepatitis B is confirmed and deteriorating, and HEV in specific settings (pregnancy, epidemics, endemic exposure).

**UNDERSTAND:** **IgM anti-HBc must be included alongside HBsAg** because of the **WINDOW PERIOD** — a phase where HBsAg has fallen below detection but anti-HBs has not yet appeared, and **IgM anti-HBc is the only positive marker**. Testing HBsAg alone would miss those cases.

**MUST KNOW:**
- **Clinical clue in the stem: an ADULT SMOKER developing an AVERSION TO CIGARETTE SMOKING** is a classic prodromal feature of acute viral hepatitis (along with anorexia, nausea, malaise and low-grade fever).
- **The LFT pattern of hepatocellular injury: transaminases in the HUNDREDS to THOUSANDS with a modest ALP rise** — versus the **cholestatic pattern (ALP and GGT disproportionately raised)**.
- **HEPATITIS E deserves specific attention in India: the commonest cause of acute viral hepatitis and of epidemic waterborne hepatitis, and it carries a MORTALITY OF 20–25% IN PREGNANCY (third trimester)** — so **always test for HEV in a pregnant woman with acute hepatitis**.
- **HEPATITIS D can only occur with hepatitis B** — as **CO-infection** (clinically indistinguishable from acute HBV, usually self-limiting) or **SUPERINFECTION** on chronic HBV (severe, with sudden deterioration and a high risk of fulminant hepatitis and cirrhosis).
- **HEPATITIS A: faeco-oral, self-limiting, never chronic, but can cause cholestatic and relapsing forms; vaccine available.**
- **Markers of ACUTE LIVER FAILURE and the need for transplant referral: ENCEPHALOPATHY and COAGULOPATHY (INR >1.5)** — not the bilirubin level.
...
## 47. Rheumatoid Arthritis — The Cervical Spine

**PEARL:** The **ATLANTOAXIAL JOINT (C1–C2) is the part of the spine most commonly affected in rheumatoid arthritis** — the rest of the spine is characteristically spared. RA also spares the **DISTAL INTERPHALANGEAL joints** — DIP involvement points to **osteoarthritis or psoriatic arthritis**.

**UNDERSTAND:** RA attacks **SYNOVIAL joints**, and the **atlantoaxial joint is the only truly synovial joint in the cervical spine** (between the odontoid peg and the atlas, with the transverse ligament). Pannus erodes the **transverse ligament and the odontoid**, producing **atlantoaxial subluxation** — everywhere else the spine is cartilaginous and therefore spared.

**MUST KNOW:**
- **This is an ANAESTHETIC EMERGENCY WAITING TO HAPPEN: neck extension during intubation can cause CORD COMPRESSION or even sudden death.** **Every RA patient needs cervical spine assessment (flexion–extension X-rays or MRI) before general anaesthesia**, and careful in-line stabilisation. Warning symptoms are **neck pain radiating to the occiput, and paraesthesiae in the hands**.
- **Joints typically involved: MCP, PIP, WRIST and MTP — bilaterally and SYMMETRICALLY, with sparing of the DIP.** Early morning stiffness lasting **>1 hour**.
...
## 48. SIADH vs Cerebral Salt Wasting

**PEARL:** **SIADH: hyponatraemia + LOW serum osmolality (<275) + INAPPROPRIATELY HIGH urine osmolality (>100) + urine sodium >40 + CLINICALLY EUVOLAEMIC + no diuretics + normal thyroid, adrenal and renal function** (Bartter–Schwartz criteria). **CEREBRAL SALT WASTING has the same biochemistry but the patient is HYPOVOLAEMIC.**

**UNDERSTAND:** **VOLUME STATUS IS THE ONLY THING THAT SEPARATES THEM — and it dictates opposite treatments.** SIADH is **water retention** → treat with **FLUID RESTRICTION**. CSW is **primary salt (and hence volume) loss** → treat with **SALT AND FLUID REPLACEMENT**. Fluid-restricting a patient with CSW worsens cerebral perfusion and can precipitate infarction — which is why the distinction is not academic.

**MUST KNOW:**
- **Causes of SIADH — group them: MALIGNANCY (SMALL CELL LUNG CANCER classically — ectopic ADH), CNS disease (stroke, haemorrhage, meningitis, trauma, tumour), PULMONARY disease (pneumonia, TB), and DRUGS (carbamazepine, SSRIs, TCAs, cyclophosphamide, vincristine, THIAZIDES, NSAIDs, ecstasy).**
- **A patient may have MORE THAN ONE cause** — as in the classic stem: **lung cancer with brain metastases AND a thiazide**.
- **Note the trap: THIAZIDE-induced hyponatraemia produces a HYPOVOLAEMIC, dehydrated patient** — so it fails the euvolaemia criterion; and **diuretic use formally excludes the diagnosis of SIADH** by the Bartter–Schwartz criteria.
- **Treatment of SIADH: FLUID RESTRICTION first-line; then salt tablets/urea, DEMECLOCYCLINE, or VAPTANS (tolvaptan — ADH V2 antagonists).** **Treat the underlying cause.**
- **CORRECTION RATE IS THE SAFETY POINT: no more than 8–10 mEq/L in 24 hours** (or 8 in high-risk patients). Over-rapid correction causes **OSMOTIC DEMYELINATION SYNDROME (central pontine myelinolysis)** — locked-in syndrome, quadriparesis, pseudobulbar palsy — the risk being greatest in **alcoholics, malnourished, hypokalaemic and liver disease patients**.
- **Symptomatic severe hyponatraemia with seizures: 3% HYPERTONIC SALINE boluses**, aiming to raise sodium by 4–6 mEq/L quickly, then stop.
- **Always exclude PSEUDOhyponatraemia (normal osmolality — hyperlipidaemia, hyperproteinaemia) and hyperglycaemia-induced dilutional hyponatraemia.**

**EXAM CONNECTION:** The stem gives you osmolalities, urine sodium and **normal BP/euvolaemia** → **SIADH**.
...
## 49. ABCD2 Score for TIA

**PEARL:** **ABCD2: AGE ≥60 (1) · BLOOD PRESSURE ≥140/90 (1) · CLINICAL features — unilateral weakness (2), speech disturbance without weakness (1) · DURATION ≥60 min (2), 10–59 min (1) · DIABETES (1).** Maximum score 7.

**UNDERSTAND:** Every item is a marker of **either large-vessel atherosclerotic disease or an unstable plaque** — which is why **unilateral weakness scores double** (it implies a motor pathway territory infarct rather than a minor sensory event) and why **longer duration scores more** (it implies more substantial ischaemia).
...
## 50. Liver Function Tests — What Each Marker Actually Means
...
- **AST:ALT ratio >2 suggests ALCOHOLIC liver disease; ALT > AST suggests viral hepatitis or NAFLD.**
...
**PEARL:** **Positive IgA tissue transglutaminase (tTG) is the best SCREENING test, but a SMALL-INTESTINAL BIOPSY is required to CONFIRM the diagnosis BEFORE starting a gluten-free diet.** Biopsy shows **VILLOUS ATROPHY, CRYPT HYPERPLASIA and INTRAEPITHELIAL LYMPHOCYTOSIS** (Marsh classification).

**UNDERSTAND:** The biopsy must be taken **while the patient is still eating gluten** — starting the diet first causes the mucosa to heal and makes the diagnosis impossible to establish, condemning the patient to a lifelong restrictive diet on uncertain grounds. Hence the strict sequence: **serology → biopsy → diet**.
...
- **Confirm with PAS staining plus PCR for *T. whipplei*** (PAS-positive macrophages also occur with **MAC infection in HIV**, so PCR distinguishes them).
- **IMMUNE RECONSTITUTION INFLAMMATORY SYNDROME (IRIS)** can occur after starting treatment — treated with steroids.
...
## 53. Small Bowel vs Large Bowel Diarrhoea
...
## 54. Pleural Effusion in the Ventilated Patient

**PEARL:** In an intubated ICU patient with **fever and a chest radiograph showing an effusion**, the next step is **ULTRASOUND-GUIDED PLEURAL ASPIRATION (diagnostic tap)** — take the fluid **before** starting antibiotics where possible. ET suctioning and chest physiotherapy do not remove pleural fluid.

**UNDERSTAND:** The tap answers the question that changes management: **is this a simple parapneumonic effusion, an EMPYEMA needing a chest drain, or a transudate?** No amount of physiotherapy or antibiotics will resolve an infected pleural collection — **infected pleural space requires DRAINAGE**, and the aspirate tells you whether drainage is needed.
...
**EXAM CONNECTION:** Ventilated patient + fever + effusion on X-ray → **ultrasound-guided aspiration**.
...
## 55. Lambert–Eaton Myasthenic Syndrome vs Myasthenia Gravis

**PEARL:** **LEMS shows a DECREMENTAL response at LOW-frequency (2–3 Hz) stimulation — like myasthenia — but an INCREMENTAL response at HIGH frequency (20–50 Hz) or after brief exercise.** Antibodies are against **P/Q-type VOLTAGE-GATED CALCIUM CHANNELS**; **reflexes are DEPRESSED or ABSENT** and **autonomic features** (dry mouth, impotence, constipation) are present.

**UNDERSTAND:** LEMS is a **PRE-synaptic** disorder — antibodies block calcium entry, so **too few acetylcholine vesicles are released**. **Repeated rapid stimulation or exercise allows calcium to ACCUMULATE in the terminal**, temporarily restoring release. Hence the pathognomonic **post-exercise FACILITATION**: strength and reflexes **improve briefly with activity** — the exact opposite of myasthenia gravis.

**MUST KNOW:**
- **The clinical mirror-image table: LEMS — PROXIMAL LOWER LIMB weakness, improves with exercise, ABSENT reflexes that RETURN after exercise, prominent AUTONOMIC symptoms, and ocular/bulbar involvement is MILD.** **MG — ocular and bulbar first (PTOSIS, diplopia), FATIGUES with exercise, NORMAL reflexes, no autonomic features.**
- **~50–60% of LEMS is PARANEOPLASTIC, overwhelmingly SMALL CELL LUNG CANCER** — so a diagnosis of LEMS mandates a **CT chest and cancer surveillance**, and the neurological syndrome often precedes the tumour by months to years.
- **Treatment: 3,4-DIAMINOPYRIDINE (amifampridine)** — it blocks potassium channels, prolonging the action potential and increasing calcium entry; plus treating the underlying tumour, and IVIG/immunosuppression.
- **MG: anti-ACETYLCHOLINE RECEPTOR antibodies (85%), or MuSK; associated with THYMIC HYPERPLASIA and THYMOMA (10–15%) — do a CT chest; treat with pyridostigmine, steroids, azathioprine and thymectomy.** **Myasthenic crisis** needs IVIG/plasma exchange and ventilation.
...
## 56. Ulcerative Colitis — Imaging and Pattern
...
## 57. Septic Shock — Vasopressor of Choice

**PEARL:** After **30 mL/kg of crystalloid**, if the **MAP remains <65 mmHg**, start **NOREPINEPHRINE — the FIRST-LINE vasopressor in septic shock.** Vasopressin is added second (to spare noradrenaline dose); **dopamine is no longer recommended** (more arrhythmias); steroids are reserved for shock refractory to fluids and vasopressors.

**UNDERSTAND:** Septic shock is **distributive** — profound **vasodilatation with a low systemic vascular resistance**. Noradrenaline is a **potent α1 vasoconstrictor with modest β1 activity**, so it restores SVR and perfusion pressure with **less tachyarrhythmia** than dopamine or adrenaline. You are treating the pathophysiology directly.
...
- **Balanced crystalloids are preferred over saline; ALBUMIN in those needing large volumes; AVOID starches.**
...
## 58. Jugular Venous Pressure in Heart Failure vs Tamponade

**PEARL:** In heart failure the JVP is **elevated AND PULSATILE**. A **NON-PULSATILE elevated JVP is NOT a feature of heart failure** — it suggests **cardiac tamponade** (or SVC obstruction).

**UNDERSTAND:** In tamponade, the **pericardial pressure is persistently high and fixed throughout diastole**, so the right atrium cannot empty in early diastole — this **abolishes the Y DESCENT**, leaving a JVP that is high but **flat and non-pulsatile**. In heart failure the chambers still fill and empty, so the normal a and v waves and x and y descents persist.
...
## 59. Systemic Lupus Erythematosus — Commonest Presentation
...
## 60. Which Anaemias Respond to Erythropoietin

**PEARL:** EPO works where **endogenous erythropoietin is INAPPROPRIATELY LOW for the degree of anaemia** — principally **ANAEMIA OF CHRONIC KIDNEY DISEASE**, and also **MYELOFIBROSIS with a serum EPO <125 U/L**, anaemia of inflammation, and chemotherapy-induced anaemia. It does **not** work in **aplastic anaemia** or **nutritional anaemia**.

**UNDERSTAND:** EPO is a **growth factor, not a substrate**. It can only stimulate a marrow that **has the cells and the raw materials to respond**. In **aplastic anaemia there are no progenitors to stimulate**; in **nutritional anaemia the bricks (iron, B12, folate) are missing** — so the correct treatment is replacement, not stimulation.

**MUST KNOW:**
- **ALWAYS ENSURE IRON SUFFICIENCY BEFORE AND DURING EPO THERAPY** — EPO dramatically increases iron demand, and **functional iron deficiency is the commonest cause of EPO resistance**. IV iron is often needed in dialysis patients.
- **Target haemoglobin 10–11.5 g/dL — do NOT normalise it.** Trials (**CHOIR, CREATE, TREAT**) showed that targeting normal haemoglobin **increases stroke, thrombosis, hypertension and mortality**, and in cancer patients may promote tumour progression.
- **Adverse effects: HYPERTENSION (the commonest), thrombosis/vascular access clotting, seizures, and rarely PURE RED CELL APLASIA from anti-EPO antibodies.**
- **Other causes of EPO resistance: infection/inflammation, hyperparathyroidism, aluminium toxicity, blood loss, and malignancy.**
- **In CKD, anaemia is multifactorial: EPO deficiency (dominant), shortened red cell survival, uraemic inhibitors of erythropoiesis, blood loss, and iron deficiency.**
- **Aplastic anaemia treatment: immunosuppression (ATG + ciclosporin) or ALLOGENEIC STEM CELL TRANSPLANT** in the young with a matched donor; plus **eltrombopag**.
...
## 61. Conn's Syndrome and the Aldosterone Escape
...
**MUST KNOW:**
- **Primary aldosteronism is the commonest CURABLE cause of secondary hypertension (5–10% of all hypertensives)** — far commoner than once believed. **Screen: resistant hypertension, hypertension with spontaneous or diuretic-induced hypokalaemia, an adrenal incidentaloma, or hypertension under 40.**
- **Screening test: the ALDOSTERONE:RENIN RATIO (ARR) — HIGH aldosterone with SUPPRESSED renin.** Confirm with a **saline suppression or fludrocortisone suppression test**, then **CT adrenals and ADRENAL VEIN SAMPLING** to distinguish a unilateral adenoma (surgical) from bilateral hyperplasia (medical).
- **Many patients are NORMOKALAEMIC** — waiting for hypokalaemia to trigger screening misses most cases.
- **Treatment: unilateral adenoma → LAPAROSCOPIC ADRENALECTOMY; bilateral hyperplasia → mineralocorticoid receptor antagonist (SPIRONOLACTONE or eplerenone).** Spironolactone causes **gynaecomastia and impotence** — eplerenone does not.
...
## 62. Iatrogenic (Exogenous) Cushing Syndrome
...
**MUST KNOW:**
- **NIV (BiPAP) is indicated for respiratory ACIDOSIS (pH <7.35 with pCO2 >45) despite optimal medical therapy** — it reduces intubation, mortality and length of stay, and is one of the best-evidenced interventions in respiratory medicine.
- **Antibiotics are indicated by the ANTHONISEN criteria — increased DYSPNOEA, increased SPUTUM VOLUME, and increased SPUTUM PURULENCE**: give them for **all three, or for two if one is purulence**, or if ventilatory support is needed. Cover ***H. influenzae*, *S. pneumoniae* and *Moraxella catarrhalis***, adding pseudomonal cover in advanced disease.
- **Steroids: prednisolone 40 mg for 5 days** — oral is as good as IV, and longer courses add harm without benefit.
- **GOLD spirometric grading (post-bronchodilator FEV1/FVC <0.70): GOLD 1 mild ≥80% · 2 moderate 50–79% · 3 SEVERE 30–49% · 4 very severe <30%.** Note the exam trap: **severe is 30–49%, NOT 40–50%.**
- **Only three things alter mortality in COPD: SMOKING CESSATION, LONG-TERM OXYGEN THERAPY (for PaO2 ≤55, or ≤59 with cor pulmonale/polycythaemia, used ≥15 h/day), and lung volume reduction surgery in selected patients.** Vaccination (influenza, pneumococcal) reduces exacerbations.
- **Methylxanthines and mucolytics have marginal roles; ICS is added for frequent exacerbators with EOSINOPHILIA, at the cost of increased pneumonia risk.**

**EXAM CONNECTION:** Multi-select → **antibiotics, steroids and nebulised bronchodilators** (antivirals excluded).
...
**MUST KNOW:**
- **IPF is a diagnosis of EXCLUSION** — you must rule out the known causes of a UIP pattern: **connective tissue disease (especially RHEUMATOID ARTHRITIS and scleroderma), asbestosis, drugs (amiodarone, methotrexate, nitrofurantoin, bleomycin), and chronic hypersensitivity pneumonitis.**
- **A definite UIP pattern on HRCT means BIOPSY IS NOT REQUIRED.**
- **Physiology: RESTRICTIVE — reduced FVC and TLC with a NORMAL OR HIGH FEV1/FVC ratio, and a REDUCED DLCO** with exercise desaturation. **DLCO is the most sensitive early marker.**
- **Treatment: ANTIFIBROTICS — PIRFENIDONE and NINTEDANIB — slow the rate of FVC decline but do not reverse disease.** **CORTICOSTEROIDS AND IMMUNOSUPPRESSION ARE HARMFUL IN IPF** (the PANTHER-IPF trial showed increased mortality with prednisone/azathioprine/NAC) — a crucial negative, and the opposite of the approach in most other ILDs. **Lung transplantation is the only curative option.**
- **Supportive care matters and is examinable: PNEUMOCOCCAL AND INFLUENZA VACCINATION, pulmonary rehabilitation, long-term oxygen, treatment of GERD, and smoking cessation.**
...
**MUST KNOW:**
- **Kinetics make it useful: procalcitonin rises within 4–6 hours, peaks at 12–24 hours, and has a half-life of ~24 hours — so it FALLS RAPIDLY when infection is controlled.** CRP is much slower on both ends.
- **A PCT-guided algorithm safely reduces antibiotic duration and exposure without increasing mortality** — the main evidence base.
- **False positives (non-infectious PCT rise): major surgery, trauma, burns, cardiogenic shock, pancreatitis, heat stroke, prolonged severe organ perfusion failure, medullary thyroid carcinoma and small cell lung cancer, and the first days of life.**
- **Ventilator-associated pneumonia (VAP) is defined as pneumonia occurring >48 hours after intubation** — diagnosis rests on **new/progressive infiltrate PLUS at least two of fever, leucocytosis and purulent secretions**, with **quantitative culture of endotracheal aspirate or BAL**. Neither PCT nor CRP diagnoses it.
...
## 67. Atrial Fibrillation and PSVT — Acute Management

**PEARL:** **AF presenting WITHIN 48 HOURS in a HAEMODYNAMICALLY STABLE patient → VENTRICULAR RATE CONTROL** (and cardioversion may be considered without prior anticoagulation, since thrombus is unlikely). **If unstable → immediate synchronised CARDIOVERSION.** **If >48 hours or unknown duration → anticoagulate for 3 weeks first, or exclude thrombus with TRANSOESOPHAGEAL echo (NOT carotid Doppler) before cardioverting.**

**UNDERSTAND:** The **48-hour rule** exists because organised thrombus takes about that long to form in the fibrillating left atrial appendage. Cardioverting later without anticoagulation risks **ejecting a formed clot** — hence the need for TOE, which is the only echo window that visualises the **left atrial appendage** (a carotid Doppler looks at the wrong vessel entirely).

**MUST KNOW:**
- **Rate control agents: beta-blockers (metoprolol, esmolol) or non-dihydropyridine calcium channel blockers (diltiazem, verapamil); digoxin for the sedentary or those in heart failure. AVOID verapamil/diltiazem in decompensated heart failure and in WPW with AF.**
- **AF WITH PRE-EXCITATION (WPW) is the classic trap: AV nodal blockers — ADENOSINE, verapamil, diltiazem, digoxin and beta-blockers — are CONTRAINDICATED**, because blocking the AV node channels conduction down the accessory pathway and can precipitate **ventricular fibrillation**. Use **procainamide or synchronised cardioversion**.
- **Anticoagulation is decided by CHA2DS2-VASc, not by rhythm strategy** — and rate versus rhythm control makes little difference to mortality (AFFIRM), though early rhythm control is favoured in recent trials.
- **PSVT (narrow, regular, no visible P waves): first VAGAL MANOEUVRES — VALSALVA (the modified Valsalva is most effective) or CAROTID SINUS MASSAGE (contraindicated with a bruit or previous stroke); then IV ADENOSINE (drug of choice); then verapamil/diltiazem or a beta-blocker. If HAEMODYNAMICALLY UNSTABLE → synchronised DC cardioversion.**
- **ORAL VERAPAMIL (or diltiazem/beta-blocker) is used for PROPHYLAXIS of PSVT; adenosine is purely for acute termination** (half-life <10 seconds). **Definitive cure is RADIOFREQUENCY ABLATION.**
- **Adenosine cautions: transient asystole, flushing and chest tightness; AVOID in asthma; effect potentiated by dipyridamole and antagonised by theophylline/caffeine.**

**EXAM CONNECTION:** Stable AF of 4 hours → **rate control**. Stable PSVT → **carotid sinus massage/vagal manoeuvre first**, then adenosine. Prophylaxis of PSVT → **oral verapamil**.
...
## 68. Confirming Acromegaly/Gigantism

**PEARL:** The **confirmatory test is FAILURE OF GH SUPPRESSION after a 75 g ORAL GLUCOSE LOAD** — normally GH falls below 0.4 µg/L; in acromegaly it does not, and may **paradoxically rise**. **Random GH is useless** (pulsatile secretion), and **IGF-1 is the best SCREENING test** but not confirmatory.

**UNDERSTAND:** GH is secreted in **pulses**, so a single random level can be high in a normal person and low in an acromegalic. **Glucose normally suppresses GH** — so removing that suppression demonstrates **autonomous secretion**, which is the definition of a functioning adenoma. **IGF-1 is stable through the day** (it integrates GH exposure over time), which is why it screens well.

**MUST KNOW:**
- **GIGANTISM if GH excess occurs BEFORE epiphyseal fusion (tall stature); ACROMEGALY if AFTER (coarse features, prognathism, macroglossia, increasing shoe/ring size, sweating, skin tags).**
- **The morbidity is systemic and is what kills: HYPERTENSION, CARDIOMYOPATHY and heart failure, DIABETES/insulin resistance, OBSTRUCTIVE SLEEP APNOEA, carpal tunnel syndrome, arthropathy, and an increased risk of COLONIC POLYPS AND COLORECTAL CANCER (hence colonoscopy screening).**
- **After biochemical confirmation, do an MRI PITUITARY** — the sequence is **biochemistry first, imaging second** (incidental pituitary lesions are common, so imaging first misleads).
- **Treatment: TRANS-SPHENOIDAL SURGERY is first-line; then medical therapy — SOMATOSTATIN ANALOGUES (octreotide, lanreotide), the GH receptor antagonist PEGVISOMANT (which normalises IGF-1 but not GH), and cabergoline; radiotherapy as last resort.**
...
**MUST KNOW:**
- **GINA now recommends ICS–FORMOTEROL as the preferred RELIEVER at every step ("anti-inflammatory reliever")**, replacing SABA-only rescue. The same inhaler is used for maintenance and relief (**MART regimen**).
...
- **Add-on options: LAMA (tiotropium), leukotriene receptor antagonist (montelukast — with a warning about neuropsychiatric effects), and for severe disease the biologics: OMALIZUMAB (anti-IgE, allergic), MEPOLIZUMAB/reslizumab/benralizumab (anti-IL-5, eosinophilic), DUPILUMAB (anti-IL-4Rα), and TEZEPELUMAB (anti-TSLP).**
- **Oral corticosteroids are for exacerbations and severe refractory disease only** — chronic use carries unacceptable toxicity.
- **Acute severe asthma: oxygen, nebulised salbutamol + ipratropium, systemic steroids, IV magnesium sulphate in severe cases.** **Warning signs: silent chest, exhaustion, cyanosis, bradycardia, and a NORMAL OR RISING pCO2 (a normal pCO2 in an acutely breathless asthmatic is ominous, not reassuring).**

**EXAM CONNECTION:** Frequent nocturnal symptoms on bronchodilators alone → **add ICS + LABA**.
...
## 70. Malignant MCA Infarction — Decompressive Craniectomy

**PEARL:** A stroke patient who **deteriorates in consciousness on day 2–3** with CT showing **large MCA-territory infarction with oedema and MIDLINE SHIFT** requires **DECOMPRESSIVE HEMICRANIECTOMY** — not more medical therapy.

**UNDERSTAND:** Cerebral oedema peaks at **48–72 hours** and can persist for ~10 days. Once a large infarct swells inside a closed cranial vault, **medical therapy (mannitol, hypertonic saline, hyperventilation) only buys hours** — the pressure has nowhere to go. **Removing part of the skull gives the swelling somewhere to expand**, preventing transtentorial herniation and death.

**MUST KNOW:**
- **The evidence-based selection criteria: AGE ≤60 YEARS, and surgery WITHIN 48 HOURS of onset** (pooled DECIMAL, DESTINY and HAMLET trials) — this **halves mortality**, though many survivors have moderate–severe disability, so the decision requires discussion with the family.
- **Predictors of malignant oedema: hypodensity involving ≥1/3 of the MCA territory on CT within 6 hours, early midline shift, DWI lesion volume ≥80 mL within 6 hours, and NIHSS >15–20.**
- **CEREBELLAR infarction has a lower threshold for surgery** — posterior fossa oedema compresses the brainstem and causes **obstructive hydrocephalus** quickly; treat with **suboccipital craniectomy ± EVD**.
- **Medical measures are temporising: head-up 30°, avoid hypotonic fluids and hyperthermia, osmotherapy (mannitol or hypertonic saline), and brief hyperventilation only as a bridge to surgery.** **CORTICOSTEROIDS ARE NOT USED for cytotoxic oedema in ischaemic stroke** (they help vasogenic oedema from tumours and abscesses).
- **Aspirin/clopidogrel and thrombectomy have no role once the infarct is established and swelling** — the window has closed.
- **Antiplatelets and anticoagulation should be withheld** around the time of craniectomy.
...
## 71. Choosing an Antidiabetic Drug by Comorbidity

**PEARL:** **Metformin is stopped when eGFR <30 mL/min** (and with creatinine 5.6 mg/dL it is absolutely contraindicated) because of **LACTIC ACIDOSIS**. In an **obese patient with previous PANCREATITIS and a family history of BLADDER CANCER who refuses injections**, an **SGLT-2 inhibitor (canagliflozin)** is the right choice.

**UNDERSTAND:** Drug selection in diabetes is now **comorbidity-driven, not glucose-driven**. Each option in the question is excluded by a specific adverse effect: **DPP-4 inhibitors and GLP-1 agonists → PANCREATITIS**; **pioglitazone → BLADDER CANCER, weight gain, fluid retention and heart failure**; **GLP-1 agonists are injectable**. Only the SGLT-2 inhibitor avoids all four constraints and **causes weight loss**.
...
- **Insulin is the answer in pregnancy, severe hyperglycaemia with catabolism, and advanced renal or hepatic failure** — and note that **insulin is NOT contraindicated in renal failure** (only the dose must be reduced, since insulin is renally cleared).
...
## 72. Anion Gap in Metabolic Acidosis
...
## 73. Pneumatocele and Staphylococcal Pneumonia

**PEARL:** **PNEUMATOCELES (thin-walled air-filled cysts) in an INFANT with pneumonia point to *STAPHYLOCOCCUS AUREUS***, which causes a **necrotising pneumonitis** complicated by **pneumothorax and EMPYEMA**.

**UNDERSTAND:** *S. aureus* produces **exotoxins and proteases that necrose lung parenchyma**, creating a **one-way check-valve at a damaged bronchiole** — air enters on inspiration but cannot escape, progressively inflating a thin-walled cyst. This mechanism also explains the tendency to rupture into the pleura, causing **pyopneumothorax**.

**MUST KNOW:**
- **Staphylococcal pneumonia in infants is typically rapidly progressive, with high fever, marked toxicity and rapid radiographic change** — the X-ray can transform within hours. It often follows a **viral illness (classically INFLUENZA or measles)**, which damages the respiratory epithelium.
...
## 74. Thyroid Scan — The Hot Nodule
...
## 75. Corrected Reticulocyte Count and the Sodium Deficit — Two Formulas Worth Memorising

**PEARL:** **CORRECTED RETICULOCYTE COUNT = reticulocyte % × (actual Hb / 15)** — or × (actual PCV / 45). A count of 9% at Hb 5 g/dL is really **9 × 5/15 = 3%**. **SODIUM DEFICIT = (140 − actual Na) × 0.6 × body weight in kg** — so a 60 kg patient with Na 120 needs **(140−120) × 0.6 × 60 = 720 mEq**.

**UNDERSTAND:** The reticulocyte **percentage is a fraction of a shrunken denominator** — in severe anaemia there are far fewer red cells, so an unimpressive absolute output looks like a high percentage. Correcting for haemoglobin restores the true measure of **marrow effort**. Likewise the sodium deficit uses **0.6 × weight because that is TOTAL BODY WATER**, the compartment across which sodium is distributed.

**MUST KNOW:**
- **A second correction gives the RETICULOCYTE PRODUCTION INDEX (RPI) = CRC ÷ maturation factor**, applied when **polychromatophilic macrocytes ("shift cells") appear on the smear** — prematurely released reticulocytes survive longer in blood and inflate the count. Maturation factor: **Hct 45% → 1.0; 35% → 1.5; 25% → 2.0; 15% → 2.5.**
- **Interpretation is the whole point: RPI >2–3 = HYPERPROLIFERATIVE (haemolysis or blood loss — marrow is working). RPI <2 = HYPOPROLIFERATIVE (nutritional deficiency, marrow failure, CKD, anaemia of chronic disease).** This is the first branch point in the classification of anaemia.
- **Sodium correction rate: NO FASTER THAN 8–10 mEq/L in 24 hours** (some say 6 in high-risk patients). Over-rapid correction of chronic hyponatraemia causes **OSMOTIC DEMYELINATION SYNDROME (central pontine myelinolysis)** — a delayed, devastating quadriparesis with pseudobulbar palsy and "locked-in" state.
- **Symptomatic/seizing hyponatraemia is the exception: give 3% HYPERTONIC SALINE boluses to raise sodium by 4–6 mEq/L quickly** to stop the seizure, then slow down.
- **Risk factors for osmotic demyelination: chronic hyponatraemia, alcoholism, malnutrition, hypokalaemia, liver disease.**

**EXAM CONNECTION:** Pure calculation questions — plug in and answer; the numbers are always clean.

**REMEMBER:** *Divide the reticulocyte count by how anaemic they are; multiply the sodium gap by total body water.*

`Q11736, Q11766`

---

## 76. Hereditary Syndromes and Pancreatic Cancer Risk

**PEARL:** **PEUTZ–JEGHERS SYNDROME (STK11/LKB1) carries the HIGHEST relative risk of pancreatic cancer (~132-fold)** — far above hereditary pancreatitis (~53×), cystic fibrosis (~30×), FAMMM/p16 (~20×), BRCA2 (~10×), Lynch syndrome (~8×) and FAP (~4×).

**UNDERSTAND:** STK11 is a **tumour suppressor upstream of the mTOR pathway**; its loss removes a brake on growth signalling in multiple epithelia at once. That is why Peutz–Jeghers is not a colon syndrome but a **pan-gastrointestinal and extraintestinal cancer syndrome**, with the pancreas carrying the steepest multiplier of all.
...
## 77. Unstable Ventricular Tachycardia

**PEARL:** **Wide-complex (>0.12 s) tachycardia with a SINGLE uniform QRS morphology = MONOMORPHIC VT.** With **syncope and SBP <90**, treatment is **immediate synchronised DC CARDIOVERSION** — not lignocaine, not amiodarone. **Stable VT → IV AMIODARONE** (procainamide is an alternative). **Pulseless VT/VF → UNSYNCHRONISED defibrillation.**

**UNDERSTAND:** Drugs take minutes to work and may drop the blood pressure further. Electricity **depolarises the entire myocardium at once and lets the sinus node resume**, which is instantaneous. The rule is universal in tachyarrhythmia management: **instability = electricity, stability = drugs.**

**MUST KNOW:**
- **Signs of instability: hypotension, altered mental status, ischaemic chest pain, acute heart failure, shock.** Any one of them ends the debate.
...
**EXAM CONNECTION:** ECG image of monomorphic VT + hypotension → **DC cardioversion**.
...
## 78. The Substantia Nigra — Image Question
...
## 79. Empirical Therapy of Bacterial Meningitis

**PEARL:** Empirical therapy for suspected community-acquired bacterial meningitis is **THIRD-GENERATION CEPHALOSPORIN (CEFTRIAXONE or cefotaxime) + VANCOMYCIN + DEXAMETHASONE**, with **AMPICILLIN added at the extremes of age or in immunosuppression** (for *Listeria*). **Penicillin G remains the drug of choice for CONFIRMED penicillin-SUSCEPTIBLE meningococcus**, but is inadequate empirically.

**UNDERSTAND:** Vancomycin is added not because it is better, but because **penicillin- and cephalosporin-resistant *S. pneumoniae* now exists** — and pneumococcus is the commonest and deadliest cause. You cannot afford to be wrong for the 12–48 hours until sensitivities return, so you cover the resistant organism until you can safely narrow.

**MUST KNOW:**
- **DEXAMETHASONE must be given BEFORE OR WITH the first dose of antibiotics** — it reduces the inflammatory burst caused by bacterial lysis and **reduces mortality and hearing loss, particularly in pneumococcal meningitis and in *H. influenzae* in children.** Given late, it does nothing.
- **NEVER DELAY ANTIBIOTICS FOR A CT OR AN LP.** CT before LP is indicated for **immunocompromise, a history of CNS disease, new-onset seizure, papilloedema, altered consciousness or a focal neurological deficit** — take blood cultures and give antibiotics first.
- **Age-specific pathogens: NEONATES — group B *Streptococcus*, *E. coli*, *Listeria*. CHILDREN/ADULTS — *S. pneumoniae*, *N. meningitidis*. >50 or IMMUNOCOMPROMISED — add *LISTERIA* (hence ampicillin; cephalosporins do NOT cover it). POST-NEUROSURGERY/SHUNT — staphylococci and Gram-negatives (add ceftazidime/meropenem + vancomycin).**
- **CSF patterns: BACTERIAL — high neutrophils, glucose <40% of blood, high protein, high opening pressure. VIRAL — lymphocytes, NORMAL glucose. TB/FUNGAL — lymphocytes, VERY LOW glucose, VERY HIGH protein, cobweb clot.**
...
## 80. Statin Intensity After Myocardial Infarction
...
**MUST KNOW:**
- **High-intensity statins lower LDL by ≥50%; moderate-intensity by 30–49%.** The four statin benefit groups: **clinical ASCVD, LDL ≥190, diabetes aged 40–75, and a 10-year risk ≥7.5%.**
- **Target LDL in very high-risk ASCVD is <55–70 mg/dL**; if not achieved on maximal statin, add **EZETIMIBE** (IMPROVE-IT), then a **PCSK9 INHIBITOR (evolocumab, alirocumab)** or inclisiran.
- **Combining a statin with a FIBRATE increases the risk of MYOPATHY and RHABDOMYOLYSIS — the risk is highest with GEMFIBROZIL, so FENOFIBRATE is preferred if combination is unavoidable.**
- **Statin adverse effects: myalgia (common), raised transaminases, rhabdomyolysis (check CK), and a small increase in new-onset diabetes** — which does not outweigh the cardiovascular benefit.
- **Interactions matter: statins metabolised by CYP3A4 (simvastatin, atorvastatin) accumulate with macrolides, azoles, protease inhibitors, ciclosporin and grapefruit juice.** Pravastatin, rosuvastatin and pitavastatin are less affected.
- **Statins are CONTRAINDICATED IN PREGNANCY.**
- **Low HDL alone is not a drug target** — no HDL-raising drug (niacin, CETP inhibitors) has improved outcomes.
...
## 81. Non-Invasive Assessment of Hepatic Fibrosis
...
**MUST KNOW:**
- **Falling PLATELETS are the earliest and most useful routine clue to cirrhosis and portal hypertension** (hypersplenism plus reduced thrombopoietin) — hence platelets appear in APRI, FIB-4 and the Forns index.
- **An AST:ALT RATIO >2 suggests ALCOHOLIC liver disease** (pyridoxine deficiency limits ALT synthesis); a ratio **>1 in NAFLD suggests advanced fibrosis**.
- **TRANSIENT ELASTOGRAPHY (FibroScan) is now the practical first-line test** and is excellent at **ruling OUT advanced fibrosis**; it is unreliable in obesity, ascites, acute hepatitis (inflammation falsely raises stiffness), cholestasis and right heart failure. **MR ELASTOGRAPHY is the most accurate but expensive.**
- **LIVER BIOPSY remains the gold standard but has sampling error, inter-observer variation and a small risk of bleeding** — non-invasive tests have largely displaced it for staging.
...
## 82. First Step in the Comatose Patient
...
## 83. Haemophilia — Pattern of Bleeding
...
**MUST KNOW:**
- **X-LINKED RECESSIVE: HAEMOPHILIA A = FACTOR VIII (commoner), HAEMOPHILIA B (Christmas disease) = FACTOR IX. They are clinically INDISTINGUISHABLE — only the factor assay separates them.**
- **Laboratory: PROLONGED aPTT with NORMAL PT, NORMAL platelet count and NORMAL bleeding time.** A **MIXING STUDY CORRECTS** in factor deficiency but **fails to correct with an INHIBITOR** (acquired factor VIII antibodies or lupus anticoagulant).
- **Blood in a joint is chemically destructive: iron-laden synovium → SYNOVITIS → cartilage destruction → CHRONIC HAEMOPHILIC ARTHROPATHY and fixed deformity.** Hence prophylaxis, not on-demand treatment, is the modern standard.
- **Treatment: recombinant factor concentrate; DESMOPRESSIN (DDAVP) works in MILD haemophilia A and von Willebrand disease (releases stored VIII/vWF) but NOT in haemophilia B; tranexamic acid for mucosal bleeds; EMICIZUMAB (a bispecific antibody bridging IXa and X) has transformed prophylaxis, including for those with inhibitors.**
- **INHIBITOR DEVELOPMENT is the major complication of therapy** — suspect it when bleeding stops responding to factor.
...
## 84. Thrombolysis in Acute Ischaemic Stroke

**PEARL:** Acute hemiparesis of **1 hour** with a **NORMAL CT** is an **ischaemic stroke within the window** → give **IV THROMBOLYSIS (ALTEPLASE 0.9 mg/kg, max 90 mg — 10% bolus, the rest over 60 minutes) within 4.5 HOURS of onset.** A normal CT does not exclude stroke — **it excludes HAEMORRHAGE**, which is exactly what you needed to know before thrombolysing.
...
- **MECHANICAL THROMBECTOMY for LARGE VESSEL OCCLUSION (ICA, proximal MCA) is indicated up to 24 HOURS with favourable perfusion imaging (DAWN, DEFUSE-3), and is given IN ADDITION to thrombolysis when eligible** — do not withhold alteplase while arranging it. **Tenecteplase is increasingly preferred (single bolus).**
...
## 85. Relative Bradycardia — Faget's Sign
...
## 86. Cardiac Tamponade vs Tension Pneumothorax

**PEARL:** Both give **hypotension, distended neck veins, muffled heart sounds and tachycardia** — the discriminator is **BREATH SOUNDS: ABSENT in TENSION PNEUMOTHORAX, PRESENT in TAMPONADE**, along with **hyper-resonant percussion and TRACHEAL/mediastinal shift AWAY from the affected side.**

**UNDERSTAND:** Both are **obstructive shock** — external compression prevents venous return, which is why the JVP and heart sounds behave identically. They differ only in **where the compressing thing sits**: air in the pleural space abolishes air entry and pushes the mediastinum; fluid in the pericardial sac does neither. So the examination finding that separates them is necessarily a **lung** finding, not a cardiac one.
...
**PEARL:** **Steatorrhoea + NORMAL D-XYLOSE + NORMAL duodenal biopsy + ABNORMAL SCHILLING test = PANCREATIC INSUFFICIENCY** (or terminal ileal disease). A **NORMAL D-xylose test EXCLUDES generalised small-bowel MUCOSAL disease** such as coeliac disease and lymphangiectasia.

**UNDERSTAND:** Each test isolates one step. **D-XYLOSE is a monosaccharide absorbed PASSIVELY, requiring NO digestion at all** — so it tests **mucosal surface area alone**. If xylose is absorbed normally, the mucosa is intact, and the defect must lie **upstream in DIGESTION (pancreatic enzymes or bile salts)** rather than in absorption. The Schilling test then localises further, because **B12 absorption needs pancreatic proteases to release it from R-binder, intrinsic factor, and an intact TERMINAL ILEUM.**
...
## 88. The EEG in Creutzfeldt–Jakob Disease

**PEARL:** **PERIODIC SHARP-WAVE COMPLEXES (bi- or triphasic, ~1 per second) on EEG, with rapidly progressive dementia and MYOCLONUS, = CREUTZFELDT–JAKOB DISEASE.** CSF is positive for **14-3-3 PROTEIN** (and RT-QuIC, which is far more specific); MRI shows the **CORTICAL RIBBON SIGN** and pulvinar/basal ganglia hyperintensity on DWI.

**UNDERSTAND:** CJD is a **prion disease** — the normal cellular protein **PrP^C misfolds into the protease-resistant PrP^Sc**, which templates further misfolding in a self-propagating chain. The result is **spongiform vacuolation of grey matter with neuronal loss and astrogliosis, but NO inflammation** (there is no foreign nucleic acid to trigger it) — which is why it is uniformly fatal, usually within a year.
...
**EXAM CONNECTION:** EEG image with periodic sharp waves → **CJD**; the differentials are all other EEG patterns.
...
## 90. Massive Transfusion and Mixed Acid–Base Disorders

**PEARL:** **Multiple blood transfusions cause METABOLIC ALKALOSIS** — the **CITRATE anticoagulant is metabolised to BICARBONATE (each unit yields ~23 mEq)**. And whenever the **pH, bicarbonate and pCO2 all look NORMAL but the ANION GAP IS HIGH**, there is a **MIXED disorder** — a high anion gap metabolic acidosis coexisting with a metabolic alkalosis (classically renal failure plus vomiting).

**UNDERSTAND:** The anion gap is the tell-tale because it is **not corrected by any compensation**. Alkalosis can push the bicarbonate and pH back to normal, but it **cannot remove the unmeasured anions already in the blood**. So a raised anion gap with a normal pH is proof of acid that has been masked, not acid that is absent — which is exactly why you calculate the gap on every ABG, even a normal-looking one.

**MUST KNOW:**
- **Use the DELTA-DELTA (Δ anion gap ÷ Δ bicarbonate): ≈1–2 = pure high anion gap acidosis; >2 = a coexisting METABOLIC ALKALOSIS; <1 = a coexisting NORMAL anion gap acidosis.**
...
## 91. Hypopituitarism — What Needs Replacing
...
## 92. Tigroid White Matter — Pelizaeus–Merzbacher Disease
...
**MUST KNOW:**
- **PENDULAR NYSTAGMUS in the first months of life plus head titubation is the classic opening clue** to PMD.
- **The other leukodystrophies worth carrying: METACHROMATIC LEUKODYSTROPHY (arylsulfatase A deficiency, sulfatide accumulation, metachromatic granules, autosomal recessive). KRABBE DISEASE (galactocerebrosidase deficiency, GLOBOID CELLS, irritability and optic atrophy). ADRENOLEUKODYSTROPHY (X-linked, ABCD1, very-long-chain fatty acids, POSTERIOR/parieto-occipital white matter with a contrast-enhancing advancing edge, plus ADRENAL INSUFFICIENCY — the combination of behavioural decline and hyperpigmentation in a boy). ALEXANDER DISEASE (GFAP, macrocephaly, FRONTAL predominance, Rosenthal fibres). CANAVAN DISEASE (aspartoacylase deficiency, macrocephaly, RAISED N-ACETYL ASPARTATE on MR spectroscopy).**
- **A tigroid pattern is also described in metachromatic leukodystrophy and Krabbe disease** — but PMD is the classic single-best answer.
- **MR SPECTROSCOPY is diagnostically useful across this group** (NAA reflects neurons/axons, choline reflects membrane turnover and myelin breakdown).
...
## 93. Pleural Disease in Rheumatoid Arthritis and SLE
...
## 94. Lateral Medullary (Wallenberg) Syndrome
...
## 95. Uveitis — Which Disease Hits Which Part of the Eye
...
## 96. Multiple Myeloma — Treat the Emergency First

**PEARL:** In myeloma with **hypercalcaemia (12 mg/dL), acute kidney injury (creatinine 6, urine output 50 mL/24 h) and severe anaemia**, the immediate step is **AGGRESSIVE IV FLUIDS + DEXAMETHASONE** — not definitive chemotherapy, not transplant, and not reflex dialysis. **HYPERCALCAEMIA is the commonest cause of renal failure in myeloma**, and it is reversible.

**UNDERSTAND:** Hypercalcaemia causes **nephrogenic diabetes insipidus and profound volume depletion**, which then causes **pre-renal failure and concentrates the light chains that form casts** — a self-amplifying loop. **Rehydration breaks the loop at both ends**: it restores perfusion and dilutes the nephrotoxic light chains. **Dexamethasone is both anti-myeloma and anti-hypercalcaemic**, and works within days — so it is the one drug that treats the emergency and the disease simultaneously.
...
## 98. Diabetic Cheiroarthropathy — The Prayer Sign
...
## 100. Tuberculous Meningitis — CSF Findings

**PEARL:** CSF showing a **LYMPHOCYTIC (mononuclear) pleocytosis, HIGH PROTEIN, LOW GLUCOSE and a COBWEB-LIKE CLOT (pellicle) on standing** = **TUBERCULOUS MENINGITIS.** Acid-fast bacilli are best sought **in that pellicle**; **CSF CULTURE remains the gold standard**, and **CBNAAT/Xpert MTB-RIF gives a rapid answer with rifampicin resistance.**

**UNDERSTAND:** TB meningitis is a **thick, gelatinous EXUDATE that settles at the BASE of the brain** — which explains its entire clinical picture. **Basal exudate traps the CRANIAL NERVES (especially CN VI, then III and VII), obstructs CSF flow at the basal cisterns to cause HYDROCEPHALUS, and produces a vasculitis of the perforating vessels of the circle of Willis → INFARCTS in the basal ganglia and internal capsule.** The very high protein content is what allows the fibrin web to form in the tube.

**MUST KNOW:**
- **The CSF table: BACTERIAL — thousands of NEUTROPHILS, very low glucose, high protein. VIRAL/ASEPTIC — lymphocytes, NORMAL glucose, mildly raised protein. TUBERCULOUS — lymphocytes, VERY LOW glucose, VERY HIGH protein (often >100), cobweb clot. FUNGAL (cryptococcal) — lymphocytes, low glucose, positive INDIA INK and CRYPTOCOCCAL ANTIGEN.** **Early TB meningitis may show a NEUTROPHIL predominance** that shifts to lymphocytes — a real diagnostic trap.
- **The presentation is SUBACUTE — a prodrome of weeks with fever, headache, vomiting, apathy and personality change** — quite unlike the hours-to-days course of pyogenic meningitis. **Cranial nerve palsy and hydrocephalus in a subacute meningitis means TB until proven otherwise.**
- **Contrast MRI shows BASAL MENINGEAL ENHANCEMENT, hydrocephalus, TUBERCULOMAS and basal ganglia infarcts.**
- **Treatment: ATT for at least 12 MONTHS (2 months intensive, then 10 months continuation) — note the extended duration — PLUS ADJUNCTIVE CORTICOSTEROIDS (dexamethasone) for 6–8 weeks, which REDUCE MORTALITY.** **ETHAMBUTOL penetrates the CSF poorly and is often replaced by a fluoroquinolone or by streptomycin/linezolid in regimens.**
- **Do NOT delay treatment for microbiological confirmation** — outcome depends almost entirely on the stage at which therapy begins.
- **Hydrocephalus may need a VP shunt or external ventricular drain.** **SIADH is a common complication.**
...
## 101. GOLD Staging of COPD

**PEARL:** Diagnosis requires a **POST-BRONCHODILATOR FEV1/FVC <0.70**. Then stage by **FEV1 % predicted**: **GOLD 1 ≥80% · GOLD 2 = 50–79% · GOLD 3 = 30–49% · GOLD 4 <30%.**

**UNDERSTAND:** Two different numbers do two different jobs: the **RATIO establishes that obstruction exists**; the **FEV1 grades how severe** it is. Both must be post-bronchodilator, because the whole point is to demonstrate **irreversible** airflow limitation — that is what separates COPD from asthma.

**MUST KNOW:**
- **The fixed 0.70 ratio OVER-diagnoses COPD in the elderly** (the ratio falls physiologically with age) and under-diagnoses it in the young — which is why the **lower limit of normal (LLN)** is increasingly preferred.
- **Modern GOLD management is driven by the ABE groups — symptoms (mMRC/CAT) and EXACERBATION history — not by the spirometric grade.** The FEV1 number no longer chooses the inhaler.
- **Significant bronchodilator reversibility** = **FEV1 improvement >12% AND >200 mL** — suggests asthma, though it does not exclude COPD.
- **Other spirometry patterns:** **restrictive** — FEV1 and FVC both reduced with a **normal or high ratio**, and a **reduced TLC** (the defining feature). **DLCO is reduced in emphysema and ILD, but NORMAL or high in asthma and chronic bronchitis.**
- **Preoperative relevance:** COPD is an **independent risk factor for postoperative pulmonary complications**. Optimise with smoking cessation (ideally **≥8 weeks** pre-op), bronchodilators, treating infection, and incentive spirometry — and prefer **regional over general anaesthesia** where possible.
...
## 102. Zollinger–Ellison Syndrome: The Secretin Test

**PEARL:** Refractory ulcers, **ulcers in the second part of the duodenum or jejunum**,
prominent gastric folds and a raised fasting gastrin → **gastrinoma**. Confirm with the
**SECRETIN STIMULATION TEST**: gastrin **paradoxically RISES by >120 pg/mL** in ZES,
whereas it falls or stays flat in every other cause of hypergastrinaemia.

**UNDERSTAND:** Secretin is the physiological **brake** on gastrin — normal antral G cells
have secretin receptors that inhibit release. **Gastrinoma cells express secretin
receptors that are aberrantly coupled to stimulation**, so the brake becomes an
accelerator. That inversion is what makes the test specific: it separates an autonomous
tumour from the far commoner reactive hypergastrinaemia of **PPI use, atrophic gastritis
or H. pylori**, where the normal inhibitory response is preserved.
...
**EXAM CONNECTION:** The question is almost always "next best investigation" after a
raised gastrin — the answer is the **secretin test**, not imaging and not biopsy.
...
## 103. Antibody-Mediated Rejection: C4d

**PEARL:** **C4d staining in the peritubular capillaries** of a transplant biopsy =
**antibody-mediated (humoral) rejection**. Treatment is **plasmapheresis + IVIG +
steroids ± rituximab** — not thymoglobulin, which targets T cells.

**UNDERSTAND:** C4d is a **split product of complement C4 that becomes covalently bound to
the endothelium** at the site where donor-specific antibody has fixed complement. Unlike
the antibody itself, it stays put — so it is a durable "footprint" proving an
antibody-mediated attack occurred on that tissue. Because the effector is a
**circulating antibody produced by plasma cells**, treatment must **remove the antibody
(plasmapheresis), neutralise/modulate it (IVIG) and suppress its source (rituximab against
CD20 B cells)**. T-cell agents cannot touch it.

**MUST KNOW:**
- **T-cell-mediated (cellular) rejection:** tubulitis and interstitial mononuclear infiltrate, **C4d negative** → treat with **pulse methylprednisolone**, and **thymoglobulin (ATG)** for steroid-resistant or vascular (Banff grade II–III) rejection.
- **Antibody-mediated rejection:** peritubular capillaritis, glomerulitis, **C4d positive**, plus **detectable donor-specific antibodies (DSA)** — the Banff triad.
- **Hyperacute** rejection (minutes to hours, preformed antibodies) is prevented by the **cross-match**; a **negative cross-match with low-level DSA** still permits later AMR, exactly as in this scenario.
...
**EXAM CONNECTION:** The biopsy image plus "C4d positive" fixes the answer; the trap is
**thymoglobulin** (right idea, wrong immune arm) or "check DSA" (already implied — the
patient needs treatment, not more tests).
...
## 104. Thyroid Eye Disease: What Actually Treats It

**PEARL:** Treatments with evidence in thyroid eye disease: **glucocorticoids (IV
methylprednisolone first-line for active moderate–severe disease), selenium (mild
disease), teprotumumab (anti-IGF-1R), tocilizumab, orbital radiotherapy and
decompression surgery**. **Rituximab is NOT an established treatment** — trials
conflicted and it is not recommended.

**UNDERSTAND:** The disease is driven by **orbital fibroblasts** expressing both the
**TSH receptor and IGF-1 receptor**; autoantibodies cross-activate them, causing
**glycosaminoglycan deposition, oedema and adipogenesis** in a bony box that cannot
expand. Effective drugs target this fibroblast axis (teprotumumab blocks IGF-1R) or
general inflammation (steroids). B-cell depletion sits too far upstream of the effector
fibroblast to be reliably useful.

**MUST KNOW:**
- **Assess activity (CAS — clinical activity score) and severity separately.** Active + severe → treat; inactive + severe → surgery (decompression → squint → lid, **in that order**).
- **Sight-threatening = dysthyroid optic neuropathy** (reduced acuity, **colour desaturation**, RAPD, disc swelling) or **corneal breakdown** → **urgent high-dose IV methylprednisolone**, then **urgent orbital decompression** if no response in 1–2 weeks.
- **SMOKING is the single strongest modifiable risk factor** — cessation is a treatment.
...
**EXAM CONNECTION:** "Which is NOT a treatment option" → **rituximab**. Also asked as the
indication for **urgent decompression** (optic neuropathy).
...
## 105. Vitamin B12: The Two Enzymes

**PEARL:** Vitamin B12 is a cofactor for exactly **two** human enzymes: **methionine
synthase** (homocysteine + methyl-THF → methionine + THF) and **methylmalonyl-CoA
mutase** (methylmalonyl-CoA → succinyl-CoA).

**UNDERSTAND:** Every clinical and biochemical feature of B12 deficiency falls out of
these two reactions. Blocking **methionine synthase** traps folate as **methyl-THF (the
"methyl-folate trap")**, starving DNA synthesis of THF → **megaloblastic anaemia**, and
raises **homocysteine**. Blocking the **mutase** causes accumulation of **methylmalonic
acid**, which disrupts myelin synthesis → **subacute combined degeneration**. Folate can
rescue the anaemia but not the mutase — which is why **giving folate alone to a B12-deficient
patient corrects the blood picture while the neurology progresses**.

**MUST KNOW:**
- **Homocysteine raised in BOTH B12 and folate deficiency; methylmalonic acid raised ONLY in B12 deficiency** — MMA is the discriminating test.
- Blood picture: **MCV >100 fL, hypersegmented neutrophils (>5 lobes), pancytopenia** with **ineffective erythropoiesis** — raised **LDH and indirect bilirubin**, low reticulocytes.
- **Subacute combined degeneration** — dorsal columns and corticospinal tracts: loss of vibration and proprioception, brisk knee jerks with absent ankle jerks and extensor plantars; peripheral neuropathy; dementia; optic atrophy. **Neurology can occur without anaemia.**
...
**EXAM CONNECTION:** "Which enzyme depends on the deficient vitamin?" — the two-enzyme
list is the whole answer. Folate reductase and the cystathionine enzymes are always the
distractors.
...
## 106. Dextrocardia on the ECG

**PEARL:** ECG of **dextrocardia**: **inverted P, QRS and T in lead I**, **positive
(upright) complexes in aVR**, and **reversed/decreasing R wave progression across the
chest leads (R gets smaller from V1 to V6)**.

**UNDERSTAND:** In dextrocardia the heart's electrical axis points to the **right and
posteriorly**, so every vector that normally travels toward lead I now travels away from
it, inverting the whole complex — and the vector that normally moves away from aVR now
points toward it, making aVR upright. Chest leads placed in the standard left-sided
positions get progressively **further** from the right-sided heart, so voltage falls
instead of rising.

**MUST KNOW:**
- **Limb-lead reversal (left–right arm swap)** produces the same limb-lead picture — but **precordial progression stays NORMAL**. That single feature separates a technical error from true dextrocardia.
- Confirm by repeating the ECG with **right-sided chest leads (V3R–V6R)**, which then look normal.
- **Kartagener syndrome** = **primary ciliary dyskinesia** — situs inversus + bronchiectasis + chronic sinusitis + male infertility (immotile sperm), from a dynein arm defect.
- **Isolated dextrocardia (dextrocardia with situs solitus)** carries a high incidence of congenital heart disease (~90%), whereas **dextrocardia with situs inversus totalis** usually has a structurally normal heart.
- Right-sided leads are also how you diagnose **right ventricular infarction (V4R ST elevation)** in inferior MI — such patients are **preload-dependent: give fluids, avoid nitrates**.

**EXAM CONNECTION:** An ECG image with **upright aVR** is the fastest recognition point;
...
## 107. Adrenaline First in Cardiac Arrest

**PEARL:** **Adrenaline (epinephrine) 1 mg IV** is the **first drug** in the ALS algorithm
— given **immediately** in non-shockable rhythms (PEA/asystole), and **after the third
shock** in shockable rhythms (VF/pulseless VT), then every **3–5 minutes**. **Amiodarone
300 mg** also goes in after the **third shock** (a further 150 mg after the fifth).

**UNDERSTAND:** Adrenaline's benefit in arrest is **not** its inotropy but its **alpha-1
vasoconstriction**, which raises aortic diastolic pressure and therefore **coronary and
cerebral perfusion pressure during compressions**. In VF it is given later because
**defibrillation, not drugs, is the definitive treatment** — anything that delays a shock
costs survival. Hence the ordering rule: shock first when shockable, drug first when not.

**MUST KNOW:**
- **Shockable: VF, pulseless VT** → immediate defibrillation (**150–200 J biphasic**), CPR 2 min, rhythm check; **adrenaline + amiodarone after the 3rd shock**, adrenaline every 3–5 min thereafter.
- **Non-shockable: PEA, asystole** → **adrenaline as soon as IV/IO access is obtained**, then every 3–5 min. **Never shock asystole.**
- **Atropine has been removed** from the arrest algorithm (no benefit in asystole/PEA); it remains the drug for **symptomatic bradycardia**.
- **Sodium bicarbonate is NOT routine** — reserved for **hyperkalaemia, tricyclic overdose, and severe pre-existing acidosis**.
- High-quality CPR is the priority: **100–120/min, 5–6 cm depth, full recoil, minimal interruptions, ratio 30:2** until an advanced airway is in, then continuous compressions with 10 breaths/min.
...
**EXAM CONNECTION:** The question is the **order and timing** — "first drug" is adrenaline
regardless of rhythm; what changes is **when**.
...
## 108. Brown-Séquard Syndrome

**PEARL:** Cord hemisection gives **ipsilateral loss of motor power, fine touch, vibration
and proprioception BELOW the lesion; contralateral loss of pain and temperature 1–2
segments BELOW; and a band of COMPLETE ipsilateral LMN paralysis and total sensory loss
AT the level of the lesion.**

**UNDERSTAND:** The pattern is entirely explained by **where each tract crosses**. The
**dorsal columns and corticospinal tract cross high (medulla and pyramidal decussation)**,
so at cord level they are still ipsilateral → ipsilateral deficits. The **spinothalamic
tract crosses within 1–2 segments of entry**, so damage above the crossing affects fibres
that have already come from the **opposite** side → contralateral pain and temperature
loss. At the level of the lesion itself, the **anterior horn cells and the entering
dorsal roots** are destroyed, giving a segmental band of **LMN** signs and complete
sensory loss on that side.

**MUST KNOW:**
- Signs summary — **ipsilateral**: UMN weakness below (spastic, hyperreflexia, extensor plantar), loss of vibration/proprioception/fine touch below, **LMN weakness and total anaesthesia at the level**, and **loss of sweating/vasomotor tone** if sympathetics are involved. **Contralateral**: loss of pain and temperature starting 1–2 segments below.
...
**EXAM CONNECTION:** Options mix the sides and modalities. Check two things: **pain/
temperature must be CONTRALATERAL**, everything else **IPSILATERAL** — and the LMN band is
**at** the level, on the same side.
...
## 109. Glioblastoma: Pseudopalisading Necrosis

**PEARL:** Progressive headache with nausea and vomiting in an adult, with a ring-enhancing
tumour crossing the corpus callosum, is **glioblastoma** — histology shows
**pseudopalisading tumour cells around serpiginous necrosis** with **microvascular
(glomeruloid) proliferation** and marked pleomorphism.

**UNDERSTAND:** Glioblastoma grows faster than it can build a blood supply, so the tumour
core outstrips its perfusion and infarcts. Cells at the rim of the dying zone **migrate
away from the hypoxia and pile up in a picket-fence line** — that migration is the
"pseudopalisade". The same hypoxia drives **VEGF**, producing the glomeruloid vascular
tufts and the intense ring enhancement seen on MRI. One mechanism, three findings.

**MUST KNOW:**
- **Histology-to-tumour matching (the exact discrimination this stem tests):**
  - **Pseudopalisading necrosis + microvascular proliferation → GLIOBLASTOMA** (WHO grade 4).
  - **Antoni A (hypercellular, spindle, Verocay bodies) and Antoni B (hypocellular, loose) → SCHWANNOMA** (S100 positive; vestibular schwannoma → **NF2**).
  - **Whorls of spindle cells + psammoma bodies → MENINGIOMA** (EMA positive, dural tail, arises from arachnoid cap cells).
  - **"Fried-egg" cells with clear cytoplasm + "chicken-wire" capillaries → OLIGODENDROGLIOMA** (**1p/19q co-deletion**, calcification, seizures, best prognosis, chemosensitive to PCV).
...
**EXAM CONNECTION:** All four options are correct descriptions of *some* tumour — the
question is only whether you can attach each histology to its name.

**REMEMBER:** *Cells fleeing the necrosis line up like a picket fence — glioblastoma.*

`Q10882`

---

## 110. Heat Stroke: When to Stop Cooling

**PEARL:** In heat stroke, cool **rapidly and aggressively — but STOP at about
101.5–102 °F (38.6–39 °C)**, not at normal temperature.

**UNDERSTAND:** Cooling has thermal inertia: the core keeps falling for some time after
active measures stop, because the cooled peripheral blood is still returning centrally. If
you continue to 98.6 °F you **overshoot into iatrogenic hypothermia**, which worsens
coagulopathy and arrhythmia in a patient already at risk of DIC. Stopping ~2 °F early
lets the body coast to normal.

**MUST KNOW:**
- **Heat stroke = core temperature >40 °C (104 °F) PLUS altered mental status (CNS dysfunction)** — this is what separates it from heat exhaustion, where the CNS is intact and the temperature usually <40 °C.
- **Exertional** heat stroke (young soldiers, athletes, sweating often present) vs **classic/non-exertional** (elderly, heatwave, **anhidrosis**, dry hot skin).
- **Best cooling method: cold-water/ice-water IMMERSION** — the fastest, and the standard for exertional heat stroke. Alternatives: evaporative cooling (spray + fans), ice packs to groin/axilla/neck, cold IV fluids, gastric/bladder lavage.
- **Antipyretics (paracetamol, NSAIDs) DO NOT WORK and are harmful** — the hypothalamic set-point is normal; the problem is failed heat dissipation, not a raised set-point. They add hepatic and renal injury.
- **Do not use dantrolene** (that is for **malignant hyperthermia** and neuroleptic malignant syndrome); **shivering** during cooling can be suppressed with benzodiazepines.
- Complications: **rhabdomyolysis with myoglobinuric acute kidney injury, DIC, hepatic failure, ARDS, seizures** — check CK, coagulation, potassium and give aggressive fluids.

**EXAM CONNECTION:** The number is the question. **Stop at 101.5–102 °F**, and reject
antipyretics in any heat-stroke management stem.
...
**PEARL:** The congenital infection that mimics **erythroblastosis fetalis** — hydrops,
hepatosplenomegaly, jaundice, anaemia and thrombocytopenia with a **blueberry-muffin
rash** — is **cytomegalovirus**.

**UNDERSTAND:** CMV infects fetal haematopoietic tissue directly, causing marrow
suppression **and** driving **extramedullary haematopoiesis** in skin, liver and spleen —
the same final picture as the compensatory erythropoiesis of Rh haemolytic disease. The
difference is the mechanism: immune haemolysis in erythroblastosis, direct viral
cytopathic marrow injury in CMV. The **blueberry-muffin lesions are dermal
erythropoiesis**, not a viral exanthem.

**MUST KNOW:**
- **CMV is the commonest congenital infection worldwide and the leading infectious cause of SENSORINEURAL HEARING LOSS** in children — hearing loss may be **late-onset and progressive**, so screen serially even in asymptomatic infected babies.
...
**EXAM CONNECTION:** "Which infection mimics erythroblastosis fetalis?" → **CMV**. The
calcification pattern (periventricular vs diffuse) is the other recurring discriminator.
...
## 112. Small Cell Lung Cancer and SIADH

**PEARL:** A heavy smoker with a **central/hilar mass** and small blue cells on biopsy has
**small cell lung carcinoma** → expect **HYPONATRAEMIA from SIADH**. **Hypercalcaemia
belongs to SQUAMOUS** cell carcinoma (PTH-related peptide).

**UNDERSTAND:** Small cell carcinoma arises from **neuroendocrine (Kulchitsky) cells**, so
it is intrinsically capable of **synthesising and secreting peptide hormones** — ADH and
ACTH being the commonest. Squamous carcinoma has no such machinery; its hypercalcaemia
comes from **PTHrP**, a mimic of a paracrine growth factor, and from bone metastases. The
histological origin predicts the syndrome.

**MUST KNOW:**
- **SCLC paraneoplastic syndromes: SIADH (hyponatraemia), ectopic ACTH (Cushing — hypokalaemic alkalosis, hyperpigmentation, rapid onset without the classic body habitus), LAMBERT–EATON myasthenic syndrome (proximal weakness that IMPROVES with repeated use; anti-VGCC antibodies), paraneoplastic cerebellar degeneration and limbic encephalitis (anti-Hu).**
- **Squamous cell:** **hypercalcaemia (PTHrP)**, central cavitating lesion, keratin pearls and intercellular bridges. **Adenocarcinoma:** peripheral, commonest overall and in **non-smokers and women**, **hypertrophic pulmonary osteoarthropathy**, **EGFR/ALK/ROS1** targetable mutations. **Large cell:** peripheral, undifferentiated, gynaecomastia.
- **SCLC is staged as limited (confined to one hemithorax, treatable in a single radiation field) vs extensive** and is **NOT a surgical disease** — treat with **platinum–etoposide chemotherapy + radiotherapy** (+ immunotherapy in extensive stage); it is exquisitely chemosensitive but relapses early. Prophylactic cranial irradiation for responders.
- **SIADH:** hyponatraemia with **low serum osmolality, inappropriately concentrated urine (>100 mOsm/kg), urine sodium >40 mmol/L, euvolaemia, normal thyroid/adrenal function**. Treat with **fluid restriction**, salt/urea, **tolvaptan**; correct **no faster than 8–10 mmol/L per 24 h** to avoid **osmotic demyelination (central pontine myelinolysis)**.
- **Pancoast tumour** — apical, causes **Horner syndrome and lower brachial plexus (C8–T1) pain**; **SVC obstruction** is most often caused by SCLC.

**EXAM CONNECTION:** Match the histology to the electrolyte. **Small cell → low sodium;
squamous → high calcium.**
...
## 113. Takotsubo Cardiomyopathy

**PEARL:** **Chest pain, ST elevation and raised troponin with NORMAL coronary arteries on
angiography**, plus **apical ballooning with an akinetic apex and hypercontractile base**
on ventriculography = **Takotsubo (stress) cardiomyopathy**.

**UNDERSTAND:** The trigger — severe physical or emotional stress (here, major trauma) —
produces a **catecholamine surge**. The **apical myocardium has the highest density of
β-adrenoceptors**, so it is stunned preferentially, while the catecholamine-driven base
contracts even harder. The result is a ventricle shaped like the Japanese octopus pot
(*takotsubo*) — a **narrow neck and round base** — with **no coronary occlusion** because
the injury is neurohormonal, not thrombotic.

**MUST KNOW:**
- Typically **postmenopausal women**; triggers include bereavement, trauma, surgery, sepsis, phaeochromocytoma, subarachnoid haemorrhage, and acute neurological events.
- ECG mimics anterior STEMI (**ST elevation in V2–V4**), troponin is raised but **modestly, out of proportion to the huge wall-motion abnormality** — the key discordance.
- **Mayo criteria:** transient regional wall-motion abnormality **extending beyond a single coronary territory**, absence of obstructive coronary disease or plaque rupture, new ECG changes or troponin rise, absence of phaeochromocytoma/myocarditis.
- **Management is supportive** — beta-blockers and ACE inhibitors, diuretics for congestion, anticoagulation if there is an apical thrombus. **Avoid inotropes** (they are catecholamines — they worsen it); use an intra-aortic balloon pump/levosimendan for shock. **Recovery of ejection fraction in days to weeks is the rule.**
- Complications: acute heart failure, **LV outflow tract obstruction** (from the hypercontractile base — a reason to avoid inotropes and to give fluids), apical thrombus, arrhythmia, and rarely rupture.
...
**EXAM CONNECTION:** The stem always states **"CAG was normal"** after a STEMI picture in a
stressed patient. The answer describes the **apical ballooning**.
...
## 114. Hypertensive Emergency: Labetalol

**PEARL:** Hypertensive **emergency** (very high BP **with acute end-organ damage** —
here, retinal cotton-wool spots and neurological symptoms) is treated with **IV
LABETALOL** — reduce the mean arterial pressure by **no more than 20–25% in the first
hour**.

**UNDERSTAND:** Chronic hypertension shifts the **cerebral autoregulation curve to the
right**, so the brain is perfused normally at high pressures but becomes **ischaemic at
"normal" ones**. Dropping the pressure to 120/80 in an hour causes watershed infarction,
blindness and acute kidney injury. Labetalol is favoured because it is a **combined
alpha-1 and beta blocker given as titratable boluses or infusion**, lowering resistance
without reflex tachycardia and **without raising intracranial pressure** (unlike direct
vasodilators such as nitroprusside).

**MUST KNOW:**
- **Emergency (with end-organ damage — encephalopathy, papilloedema/grade III–IV retinopathy, ACS, pulmonary oedema, aortic dissection, eclampsia, AKI) → IV therapy in ICU.** **Urgency (no end-organ damage) → oral agents over 24–48 hours; NEVER sublingual nifedipine** (uncontrolled fall → stroke/MI).
- Drug by scenario: **aortic dissection → esmolol/labetalol FIRST (target SBP <120 and HR <60), then a vasodilator** — beta-blockade must precede vasodilatation to prevent reflex tachycardia and increased shear stress; **pulmonary oedema → nitroglycerin + furosemide**; **phaeochromocytoma or cocaine → PHENTOLAMINE (never a pure beta-blocker — unopposed alpha)**; **eclampsia → labetalol/hydralazine + magnesium**; **acute ischaemic stroke → treat only if >220/120, or >185/110 before thrombolysis**.
- **Clonidine** is not used acutely (rebound hypertension risk on withdrawal); **nitroprusside** risks **cyanide/thiocyanate toxicity** and raised ICP.
- **Malignant hypertension** = severe hypertension with **retinal haemorrhages, exudates or papilloedema**; the pathology is **fibrinoid necrosis** of arterioles and **onion-skin hyperplastic arteriolosclerosis**.
- Look for a secondary cause in any young or resistant hypertensive: renal artery stenosis, phaeochromocytoma, primary aldosteronism, coarctation, Cushing.

**EXAM CONNECTION:** Two decisions are being tested — **is there end-organ damage
(emergency vs urgency)** and **which agent fits the scenario**. Labetalol is the default
safe answer unless the stem names dissection, phaeochromocytoma or pulmonary oedema.

**REMEMBER:** *Lower it fast enough to save the organs, slowly enough not to starve the
brain — 25% in the first hour.*

`Q10918`

---

## 115. Drug-Resistant Tuberculosis: The Definitions

**PEARL:** **MDR-TB = resistance to isoniazid AND rifampicin. Pre-XDR = MDR/RR-TB PLUS
resistance to any FLUOROQUINOLONE. XDR = pre-XDR PLUS resistance to at least one Group A
drug (bedaquiline or linezolid).**

**UNDERSTAND:** The 2021 WHO redefinition is built around the **Group A drugs — levofloxacin/
moxifloxacin, bedaquiline and linezolid** — because these three now carry the regimen.
Losing the fluoroquinolone removes the backbone and defines pre-XDR; losing bedaquiline or
linezolid on top of that leaves almost nothing, which is what "extensively drug-resistant"
now means. The old definition (injectable + fluoroquinolone resistance) is obsolete because
injectables have been dropped from the regimens.

**MUST KNOW:**
- **RR-TB (rifampicin-resistant)** is treated as MDR — rifampicin resistance is the sentinel marker, detected by **CBNAAT/Xpert MTB/RIF or Truenat**, which gives a result in ~2 hours.
...
- India's programme is **NTEP (National TB Elimination Programme)**, target elimination by 2025; **Ni-kshay** portal for notification (**TB notification is mandatory**), **Ni-kshay Poshan Yojana** nutritional support, and universal **drug-susceptibility testing** for all diagnosed patients.

**EXAM CONNECTION:** The stem lists a resistance pattern. **INH + RIF + fluoroquinolone,
still sensitive to bedaquiline/linezolid = PRE-XDR.**
...
## 116. Enterococcal Endocarditis

**PEARL:** Enterococcal endocarditis is treated with **AMPICILLIN (or penicillin) PLUS a
synergistic aminoglycoside (gentamicin)**, or **ampicillin + ceftriaxone** — enterococci
are **intrinsically resistant to cephalosporins alone** and are only inhibited, not killed,
by a beta-lactam by itself.

**UNDERSTAND:** Two intrinsic resistances define enterococcal therapy. Enterococci have
**low-affinity penicillin-binding proteins (PBP5)**, so **all cephalosporins fail** —
which is why ceftriaxone monotherapy is wrong and why cephalosporin use selects for
enterococcal superinfection. And beta-lactams are only **bacteriostatic** against them, so
**endocarditis (which requires bactericidal therapy in an avascular vegetation) needs a
second agent**: the cell-wall-active drug lets the aminoglycoside enter the cell —
classical **synergy**.

**MUST KNOW:**
- **Ampicillin + ceftriaxone** is preferred when there is **high-level gentamicin resistance or renal impairment** — the two beta-lactams saturate different PBPs and act synergistically without nephrotoxicity.
- **Vancomycin** is for penicillin-allergic patients or ampicillin-resistant strains; **VRE (vanA/vanB)** → **linezolid or daptomycin**.
- Duration: **4–6 weeks**; native-valve enterococcal endocarditis relapses if treated shorter.
- *Enterococcus faecalis* endocarditis, especially in an elderly man, should trigger a **colonoscopy** — the association with **colonic neoplasia** (as with *Streptococcus gallolyticus*, formerly *S. bovis*).
- **Modified Duke criteria:** major — **typical organism in 2 separate blood cultures**, **echocardiographic evidence of endocardial involvement (vegetation, abscess, new dehiscence)**, new valvular regurgitation; minor — predisposition, fever >38 °C, vascular phenomena (**Janeway lesions**, emboli), immunological phenomena (**Osler nodes, Roth spots**, glomerulonephritis), atypical microbiology.
- **Take three sets of blood cultures from separate sites before antibiotics**; **TOE is more sensitive than TTE**. Surgery for heart failure, uncontrolled infection/abscess, or recurrent emboli with a large vegetation.

**EXAM CONNECTION:** The answer is the drug the organism is **not** intrinsically resistant
to. Cephalosporin options are always wrong for enterococcus.
...
## 117. Vein of Galen Malformation

**PEARL:** A neonate with **treatment-resistant high-output cardiac failure, a bulging
fontanelle, and a CRANIAL BRUIT**, with a midline vascular mass and hydrocephalus on
transfontanellar ultrasound = **vein of Galen malformation**.

**UNDERSTAND:** It is an arteriovenous shunt, so the physiology is **volume overload**: a
huge fraction of the cardiac output is short-circuited through the brain and back to the
heart without passing through a capillary bed. The result is **high-output failure that
does not respond to inotropes or diuretics** — you cannot treat a plumbing problem with
pump drugs. The dilated venous sac simultaneously compresses the **aqueduct**, producing
obstructive hydrocephalus and the bulging fontanelle.

**MUST KNOW:**
- Presentation by age: **neonate — high-output cardiac failure**; **infant — macrocephaly, hydrocephalus, seizures, developmental delay, prominent scalp veins**; **older child — headache, haemorrhage**.
- **Auscultate the head** — a **cranial bruit** in a neonate with unexplained heart failure is the diagnostic pearl. Imaging: **cranial Doppler ultrasound** first, then MRI/MRA and catheter angiography.
- **Treatment is ENDOVASCULAR EMBOLISATION**, staged; open surgery has a very high mortality. Ventriculoperitoneal shunting alone can worsen outcomes.
- Differential for a neonatal midline posterior fossa/midline mass: **arachnoid cyst** (CSF-density, no flow), **encephalocele** (skull defect), **medulloblastoma** (rare in neonates), **choroid plexus papilloma** (CSF overproduction hydrocephalus).
- Other causes of **high-output cardiac failure** worth listing: large AV fistula, severe anaemia, thyrotoxicosis, beriberi (thiamine deficiency), Paget disease, sepsis, and in newborns, **hepatic haemangioma** or a large placental chorioangioma.

**EXAM CONNECTION:** Two clues appear together and are unique — **cranial bruit** and
**refractory heart failure in a newborn**.
...
## 118. Spirometry: Obstructive vs Restrictive

**PEARL:** **FEV1/FVC <70% = OBSTRUCTIVE** (asthma, COPD, **bronchiectasis**). A
**preserved or raised FEV1/FVC with a reduced FVC and TLC = RESTRICTIVE** (pulmonary
fibrosis, sarcoidosis, neuromuscular and chest-wall disease).

**UNDERSTAND:** The ratio, not the absolute values, makes the diagnosis. In obstruction the
airway narrows, so **FEV1 falls far more than FVC** and the ratio drops. In restriction the
lung cannot expand, so **FEV1 and FVC fall together** — the ratio is preserved or even
**increased**, because a stiff lung has increased elastic recoil and empties briskly. The
flow–volume loop shows this visually: a **scooped, concave expiratory limb** in obstruction;
a **narrow, tall, "witch's hat" loop shifted right** in restriction.

**MUST KNOW:**
- **Obstructive:** ↓FEV1, ↓↓FEV1/FVC, **↑RV, ↑TLC, ↑FRC (air trapping and hyperinflation)**, ↓DLCO in emphysema but **normal or raised DLCO in asthma and chronic bronchitis**. **Reversibility (≥12% and ≥200 mL improvement after bronchodilator) = asthma**; irreversible = COPD.
- **Restrictive:** ↓FVC, ↓TLC, ↓RV, **normal or ↑FEV1/FVC**. **Intrinsic (parenchymal — fibrosis, sarcoid) → LOW DLCO**; **extrinsic (chest wall, obesity, neuromuscular) → NORMAL DLCO**. In neuromuscular disease, look for a **fall in vital capacity when supine (>20% suggests diaphragmatic weakness)** and **reduced maximal inspiratory/expiratory pressures**.
...
**EXAM CONNECTION:** Look for the ratio in the stem or the shape of the curve. If
**FEV1/FVC <70%**, only the obstructive option can be right — here, bronchiectasis among
...
**PEARL:** In **iron deficiency anaemia: serum iron LOW, TIBC/transferrin HIGH, ferritin
LOW, and TRANSFERRIN SATURATION LOW (<15%)**. In **anaemia of chronic disease: iron low,
TIBC LOW, ferritin HIGH or normal.** In **haemochromatosis/haemosiderosis: iron high,
ferritin high, SATURATION HIGH (>45–50%)**.

**UNDERSTAND:** Transferrin behaves as a **negative feedback carrier**: when iron stores are
empty the liver makes **more transferrin** to scavenge every available atom, so TIBC rises
while the iron available to fill it falls — hence a **doubly depressed saturation
(iron/TIBC)**. In inflammation, **hepcidin** rises, locking iron inside macrophages and
enterocytes: serum iron falls, but stores are full (**high ferritin**) and transferrin, being
a **negative acute-phase reactant**, falls too. One hormone and one acute-phase logic
explain both patterns.

**MUST KNOW:**

| | Serum iron | **TIBC/transferrin** | **Ferritin** | **Saturation** |
|---|---|---|---|---|
| **Iron deficiency** | ↓ | **↑** | **↓↓** | **↓ (<15%)** |
| **Anaemia of chronic disease** | ↓ | **↓** | **↑/normal** | Normal or ↓ |
| **Haemochromatosis / iron overload** | ↑ | ↓ | **↑↑** | **↑ (>45%)** |
...
**EXAM CONNECTION:** Pick the one condition where the **saturation falls** — only true iron
deficiency depresses iron **and** raises TIBC.

**REMEMBER:** *Empty stores raise the carrier and drop the saturation; inflammation drops
both.*
...
## 120. Hypercalcaemia: Inpatients vs Outpatients

**PEARL:** **MALIGNANCY is the commonest cause of hypercalcaemia in HOSPITALISED patients;
PRIMARY HYPERPARATHYROIDISM is the commonest cause in the outpatient/ambulatory setting**
(and overall).

**UNDERSTAND:** The two settings sample different populations. Primary
hyperparathyroidism is **mild, chronic and asymptomatic**, so it is discovered
incidentally on a routine biochemistry panel in a well outpatient. Malignant
hypercalcaemia is **rapid in onset and severe**, producing vomiting, dehydration and
confusion — so those patients arrive in hospital. **Severity, not prevalence, decides where
each is found.**
...
**EXAM CONNECTION:** Read the setting word — **"hospitalised" → malignancy**;
"incidentally detected"/"outpatient" → hyperparathyroidism.

**REMEMBER:** *Mild and chronic is found in clinic (parathyroid); severe and acute lands in
hospital (cancer).*
...
## 121. Non-Alcoholic Fatty Liver Disease

**PEARL:** Overweight patient, minimal alcohol, **mildly raised transaminases and hepatic
steatosis** = **NAFLD/MASLD**. Management is **lifestyle modification (weight loss) — and,
in biopsy-proven NASH, VITAMIN E** in non-diabetics. Ursodeoxycholic acid and pioglitazone
are not first-line/approved.

**UNDERSTAND:** NAFLD is the **hepatic manifestation of the metabolic syndrome**: insulin
resistance drives peripheral lipolysis and hepatic de novo lipogenesis, so fat accumulates;
oxidative stress and lipotoxicity then convert simple steatosis into **steatohepatitis
(NASH) with inflammation, ballooning and fibrosis**. Since the driver is systemic insulin
resistance, the only intervention that reverses the whole process is **weight loss** —
which is why it outranks every drug.
...
**EXAM CONNECTION:** The stem quantifies the alcohol intake to rule out alcoholic liver
disease. The management answer pairs **lifestyle change with vitamin E**.
...
## 122. Fostamatinib in Immune Thrombocytopenia

**PEARL:** **Fostamatinib is a SPLEEN TYROSINE KINASE (SYK) inhibitor** used in **chronic
immune thrombocytopenia**. It works by **blocking Fc-receptor signalling in macrophages**,
so antibody-coated platelets are no longer destroyed. **Eltrombopag and romiplostim are
thrombopoietin receptor agonists, not tyrosine kinase inhibitors.**

**UNDERSTAND:** ITP has two therapeutic angles: **stop the destruction** or **increase the
production**. Fostamatinib takes the first route — SYK is the intracellular signal
downstream of the **FcγR** on splenic macrophages, so inhibiting it is a **pharmacological
splenectomy**. TPO agonists take the second route, stimulating megakaryocytes to outpace
the destruction. Knowing which lever a drug pulls answers every ITP drug question.

**MUST KNOW:**
- **ITP** — autoantibodies (usually **anti-GPIIb/IIIa or GPIb/IX**) against platelets; **isolated thrombocytopenia with an otherwise normal blood count and film**; diagnosis of **exclusion**. Bone marrow shows **normal or increased megakaryocytes**.
- **Treatment ladder:** observe if platelets **>30 × 10⁹/L and no bleeding** → **corticosteroids (prednisolone or dexamethasone pulses)** first-line, **IVIG** when a rapid rise is needed (bleeding, surgery, pregnancy) → second-line **rituximab, TPO agonists (eltrombopag, romiplostim, avatrombopag), fostamatinib, splenectomy**.
- **Platelet transfusion is reserved for life-threatening bleeding** — transfused platelets are destroyed just as fast.
- **Secondary ITP:** **SLE, antiphospholipid syndrome, HIV, hepatitis C, H. pylori, CLL, drugs** — always test for **HIV and HCV**.
- **Childhood ITP** typically follows a viral illness, is acute and **self-limiting (>80% resolve spontaneously)**; adult ITP is usually chronic.
- **Do not confuse with TTP** — pentad of thrombocytopenia, **microangiopathic haemolytic anaemia (schistocytes)**, fever, renal impairment and neurological signs, from **ADAMTS13 deficiency** → treat with **plasma exchange**; **platelet transfusion is contraindicated**.

**EXAM CONNECTION:** The stem names the drug **class** (tyrosine kinase inhibitor). Only
fostamatinib qualifies; bosutinib is a BCR-ABL inhibitor for CML.
...
## 123. Pentagastrin Test and Achlorhydria

**PEARL:** **Pernicious anaemia (autoimmune atrophic gastritis) shows NO acid response to
pentagastrin** — the **parietal cells have been destroyed**, so no stimulus can produce
acid. Normal individuals, duodenal ulcer patients and gastrinoma patients all secrete acid.

**UNDERSTAND:** Pentagastrin is a synthetic gastrin analogue acting on the **CCK-B receptor
of the parietal cell** — it is a **maximal stimulus test of parietal cell MASS**. If the
cells are absent, the test measures nothing; this is the definition of **achlorhydria
refractory to stimulation**. The resulting absent acid removes the negative feedback on
antral G cells, producing the paradox of **very high gastrin with no acid** — the mirror
image of gastrinoma (high gastrin **with** high acid).

**MUST KNOW:**
- **Pernicious anaemia:** **anti-parietal cell antibodies (sensitive) and anti-INTRINSIC FACTOR antibodies (specific)**; **type A (autoimmune, body/fundus) atrophic gastritis**; achlorhydria, **hypergastrinaemia**, low pepsinogen I; **B12 deficiency with megaloblastic anaemia and subacute combined degeneration**.
- Complications: **enterochromaffin-like cell hyperplasia → type 1 GASTRIC CARCINOID**, and increased risk of **gastric adenocarcinoma**; also associated with other autoimmune disease (**Hashimoto thyroiditis, vitiligo, Addison disease, type 1 diabetes**).
- **Type B gastritis** is **H. pylori**-associated, antral, and is the one linked to **peptic ulcer, MALT lymphoma and distal gastric cancer**.
...
**EXAM CONNECTION:** "Which will NOT secrete acid after pentagastrin?" — the answer is the
condition with **no parietal cells**. Every other option has an intact or hyperactive
parietal cell mass.
...
## 124. Severe Dengue: The Organ-Involvement Criteria

**PEARL:** **AST or ALT ≥1000 IU/L** is a criterion for **SEVERE dengue** (severe organ
involvement). Persistent vomiting, hepatomegaly >2 cm and thrombocytopenia with rising
haematocrit are **WARNING SIGNS**, not severity criteria.

**UNDERSTAND:** The WHO 2009 scheme separates **"who might deteriorate" from "who already
has organ failure"**. Warning signs identify the patient entering the **critical
(plasma-leak) phase** as the fever settles — they mandate admission and fluid therapy.
Severe dengue means the damage has happened: **shock from leakage, severe bleeding, or
organ injury**. The transaminase cut-off is set at 1000 because that level represents
hepatocellular necrosis rather than the mild transaminitis almost every dengue patient has.
...
**EXAM CONNECTION:** The question hinges on distinguishing a warning sign from a severity
criterion. Only the **transaminase ≥1000** is in the severe category among the options.
...
## 125. CURB-65 and the Site of Care in Pneumonia

**PEARL:** **CURB-65: Confusion, Urea >7 mmol/L (≈20 mg/dL), Respiratory rate ≥30/min,
Blood pressure <90 systolic or ≤60 diastolic, age ≥65** — one point each. **0–1 →
outpatient; 2 → ward; ≥3 → ICU with IV antibiotics.**

**UNDERSTAND:** The score is not a diagnosis — it is a **triage tool** answering the single
most consequential question in pneumonia: **where should this patient be treated?**
Mortality tracks the score almost linearly (roughly 0.7% at 0, 9% at 2, 15–40% at ≥3),
which is why each additional point escalates the level of care rather than changing the
antibiotic class.
...
**EXAM CONNECTION:** Count the points in the stem. **Confusion + tachypnoea + age ≥65 = 3
→ ICU + IV therapy.** Oral options are wrong at that score.
...
**PEARL:** Inability to **name or recognise an object by sight**, while recognising it
immediately **by touch, sound or description**, is **ASSOCIATIVE VISUAL AGNOSIA** — a lesion
of the **occipito-temporal junction / INFERIOR TEMPORAL lobe** (the ventral "what" stream).

**UNDERSTAND:** Vision is processed along two anatomically separate streams. The **ventral
("what") stream** runs occipital → **inferior temporal** and handles **object identity**;
the **dorsal ("where/how") stream** runs occipital → **parietal** and handles spatial
location and visually guided action. Because the deficit here is of **identity with intact
perception and intact recognition by other senses**, the lesion must be in the ventral
stream *beyond* the primary visual cortex — which is why the patient sees the object
perfectly and can copy it, yet cannot say what it is.

**MUST KNOW:**
- **Apperceptive agnosia** — the percept itself is degraded; the patient **cannot copy or match** the object; lesion in the **parieto-occipital** region.
- **Associative agnosia** — perception intact (can copy the drawing accurately) but meaning is lost; **occipito-temporal**.
- **Prosopagnosia** — inability to recognise **faces**; bilateral (or right) **fusiform gyrus**.
- **Occipital (primary visual) lesions** → cortical blindness (**Anton syndrome** if the patient denies it), **achromatopsia**, homonymous hemianopia with macular sparing.
...
**EXAM CONNECTION:** The give-away is **"recognises the object by another modality"** —
which localises to the visual association cortex, not to the sensory pathway or the
language areas.
...
## 127. Oral Hairy Leukoplakia

**PEARL:** A **white, corrugated ("hairy") plaque on the LATERAL BORDER of the tongue that
CANNOT be scraped off**, in an immunocompromised patient, is **oral hairy leukoplakia —
caused by EPSTEIN–BARR VIRUS**. If it **wipes off**, it is **candidal thrush**.

**UNDERSTAND:** The scrape test separates the two because the pathology is different. Thrush
is a **superficial pseudomembrane of Candida hyphae and debris sitting on the mucosa** — it
lifts off, leaving an erythematous base. Hairy leukoplakia is **EBV replicating within the
epithelium itself**, producing **hyperparakeratosis and acanthosis with balloon cells** — the
white material *is* the epithelium, so it cannot be removed.
...
- **Oral lesions in HIV — a high-yield set:**
  - **Oral hairy leukoplakia → EBV** (lateral tongue, does not scrape off).
  - **Pseudomembranous candidiasis (thrush) → Candida albicans** (scrapes off; treat topical clotrimazole/nystatin, systemic fluconazole).
  - **Kaposi sarcoma → HHV-8** (purple palatal or gingival plaques/nodules; also seen on skin and viscera).
  - **Herpes labialis/gingivostomatitis → HSV-1** (painful vesicles and ulcers).
  - **Oral warts → HPV**; **aphthous ulcers**; **necrotising ulcerative periodontitis**.
...
**EXAM CONNECTION:** Two words settle it — **"lateral tongue"** and **"cannot be rubbed
...
## 128. Metabolic Syndrome (NCEP-ATP III)

**PEARL:** Metabolic syndrome needs **3 of 5: central obesity by WAIST circumference (not
hip, not BMI), triglycerides ≥150 mg/dL, HDL <40 (men)/<50 (women) mg/dL, BP ≥130/85, and
fasting glucose ≥100 mg/dL.** **LDL is not a criterion.**

**UNDERSTAND:** Every criterion is a downstream marker of **visceral adiposity and insulin
resistance** — which is precisely why **waist** circumference is measured and hip is not.
Visceral fat drains into the **portal vein**, delivering free fatty acids straight to the
liver: that drives hepatic **VLDL output (high triglycerides, low HDL)**, **gluconeogenesis
(high glucose)** and sympathetic/RAAS activation (**hypertension**). Gluteofemoral (hip) fat
is metabolically protective, so including it would blur the very signal being measured.

**MUST KNOW:**
- **Waist thresholds:** NCEP — **>102 cm men, >88 cm women**; **for South Asians (IDF) the cut-offs are much lower: >90 cm men, >80 cm women** — an important Indian-exam point.
- **Insulin resistance is the accepted unifying mechanism**; NAFLD is the hepatic manifestation, **PCOS the ovarian one**, and the syndrome predicts **type 2 diabetes and cardiovascular disease**.
- **BMI and LDL are NOT part of the definition** — LDL is treated on its own cardiovascular-risk footing.
...
**EXAM CONNECTION:** "Which is NOT measured?" — the answer is **hip circumference** (or BMI
or LDL). Remember the syndrome is about **where** the fat is, not how much a person weighs.
...
## 129. Sickle Cell Fragility

**PEARL:** Sickle cells have **HIGH mechanical fragility** (rigid, non-deformable → they
shear and haemolyse) but **LOW osmotic fragility** (they are relatively dehydrated with a
high surface-area-to-volume ratio, so they tolerate swelling in hypotonic solution).
Thalassaemic cells behave the same way.

**UNDERSTAND:** Osmotic fragility measures **how much water a cell can take up before it
bursts** — it is really a test of **surface area relative to volume**. A **spherocyte** has
lost membrane, is already spherical, and bursts at almost normal tonicity → **high osmotic
fragility**. A **target cell** (thalassaemia, sickle cell, iron deficiency, liver disease)
has **excess membrane for its haemoglobin content**, so it can swell a long way → **low
osmotic fragility**. Mechanical fragility is a different property — it measures
**deformability**, and sickled or rigid cells fail that test badly.

**MUST KNOW:**

| Condition | Mechanical fragility | Osmotic fragility |
|---|---|---|
| **Hereditary spherocytosis** | High | **HIGH (the classic)** |
| **Sickle cell disease** | **High** | **LOW** |
| **Thalassaemia** | High | **LOW** |
| **Iron deficiency (target cells)** | — | Low |
| **Malaria** | High | High |

- **Sickling** occurs when deoxygenated **HbS polymerises**; triggers are **hypoxia, dehydration, acidosis, infection, cold and stress**. Repeated sickling makes the membrane permanently rigid (irreversibly sickled cells) → **vaso-occlusion and extravascular haemolysis**.
- **Sickle cell trait is protective against falciparum malaria** — the reason for its geographic distribution (balanced polymorphism).
- Complications: **vaso-occlusive crisis, acute chest syndrome (the leading cause of death), splenic sequestration, autosplenectomy → encapsulated organism sepsis, aplastic crisis with PARVOVIRUS B19, avascular necrosis of the femoral head, priapism, stroke, renal papillary necrosis, leg ulcers, pigment gallstones**.
- Management: **hydroxyurea** (raises HbF), hydration, analgesia, oxygen, **penicillin prophylaxis and vaccination**, transfusion/exchange for stroke and acute chest syndrome, **transcranial Doppler screening in children**; **haematopoietic stem cell transplant and gene therapy** are curative options.
- **Osmotic fragility testing** is now largely replaced by **EMA binding and flow cytometry** for spherocytosis.

**EXAM CONNECTION:** Straight two-variable matching. Remember the rule: **cells with too
much membrane (targets) resist osmotic lysis; cells with too little (spherocytes) do not.**

**REMEMBER:** *Sickle cells break under force but survive water — mechanically fragile,
osmotically tough.*
...
## 130. Cardiac Amyloidosis and Restrictive Cardiomyopathy

**PEARL:** **Amyloid infiltration is the classic cause of RESTRICTIVE cardiomyopathy** —
biopsy shows **hyaline eosinophilic deposits**, **Congo red stain** giving **apple-green
birefringence under polarised light**. The signature clue is **thickened ventricular walls
on echo with LOW-VOLTAGE QRS on ECG**.

**UNDERSTAND:** The voltage–thickness mismatch is the whole diagnosis. In hypertrophic
cardiomyopathy, thick walls mean **more myocardium** → tall QRS complexes. In amyloidosis,
the wall is thick because it is **stuffed with electrically inert protein between the
myocytes** → the muscle mass is actually reduced, so the QRS voltage falls. The same
infiltrate makes the ventricle **stiff but not dilated**, giving **preserved systolic
function with severe diastolic failure** — the definition of restriction.
...
**EXAM CONNECTION:** The biopsy image or the phrase "hyaline eosinophilic deposits" fixes
amyloid; the physiology it produces is **restrictive**, not dilated or hypertrophic.
...
**PEARL:** **Progressive hyperpigmentation + a rapidly enlarging pituitary tumour causing
VISUAL FIELD LOSS, after BILATERAL ADRENALECTOMY for Cushing disease = NELSON SYNDROME.**

**UNDERSTAND:** Cushing disease is a **corticotroph adenoma partially restrained by cortisol
feedback**. Removing both adrenals cures the hypercortisolism but **abolishes that restraint
entirely** — CRH and the adenoma are now unopposed, so the tumour grows aggressively and
pours out **ACTH and its precursor POMC**. Since POMC is cleaved to **MSH**, the patient
becomes intensely pigmented; as the adenoma expands it compresses the **optic chiasm** →
bitemporal hemianopia.

**MUST KNOW:**
- **Triad: hyperpigmentation, very high ACTH, and an enlarging corticotroph adenoma.**
- **Prevention: prophylactic pituitary irradiation at the time of adrenalectomy**, and **lifelong surveillance with plasma ACTH and periodic pituitary MRI**.
- **Treatment: transsphenoidal surgery** ± radiotherapy; these patients also need **lifelong glucocorticoid AND mineralocorticoid replacement** (they are surgically Addisonian).
- **Cushing syndrome work-up:** confirm hypercortisolism (**24-h urinary free cortisol, late-night salivary cortisol, low-dose dexamethasone suppression**) → then **measure ACTH**: **suppressed = adrenal (adenoma, carcinoma); raised = ACTH-dependent** → **high-dose dexamethasone suppression and CRH test** to separate **pituitary (suppresses, responds)** from **ectopic ACTH (does not)**. **Inferior petrosal sinus sampling** is the gold standard.
- **Ectopic ACTH** (small cell lung cancer, carcinoid) → **rapid onset, profound hypokalaemic alkalosis and pigmentation, without the classic body habitus**.
- Distinguish from **Addison disease** (primary adrenal failure — pigmentation but **no pituitary mass**, and hypotension) and **Sheehan syndrome** (postpartum pituitary infarction — **failure of lactation and amenorrhoea, PALLOR not pigmentation**).

**EXAM CONNECTION:** The stem always gives the **prior adrenalectomy**. Pigmentation plus a
visual deficit after that operation is Nelson syndrome by definition.
...
## 132. Cryptococcal Meningitis in HIV

**PEARL:** In cryptococcal meningitis, **DELAY antiretroviral therapy by 2–5 weeks** after
starting antifungal treatment (to avoid **IRIS**), treat with **amphotericin B PLUS
flucytosine — never amphotericin monotherapy** — and expect a **HIGH opening pressure**
requiring **serial therapeutic lumbar punctures**.

**UNDERSTAND:** Starting ART immediately restores CD4 function while the fungal antigen load
is still enormous; the recovering immune system then mounts a violent inflammatory response
inside a closed skull — **immune reconstitution inflammatory syndrome**, which can be
fatal in cryptococcal meningitis. Delaying ART lets the antifungals reduce the organism
burden first. Likewise, the raised pressure is **not from hydrocephalus but from
polysaccharide capsular material blocking the arachnoid granulations**, so it is relieved
mechanically by drainage, not by steroids or acetazolamide.

**MUST KNOW:**
- **Regimen: INDUCTION — liposomal amphotericin B + flucytosine for 2 weeks; CONSOLIDATION — fluconazole 800 mg daily for 8 weeks; MAINTENANCE — fluconazole 200 mg until CD4 >100 for ≥3 months on ART.** A **single high-dose liposomal amphotericin + flucytosine + fluconazole** regimen is now WHO-preferred in resource-limited settings.
- **Manage the raised ICP aggressively: repeat LPs to keep the opening pressure <20 cm H₂O** — this, not the antifungal, determines early survival. **Steroids are HARMFUL** in cryptococcal meningitis (they worsen outcomes).
- **Diagnosis: INDIA INK shows the encapsulated yeast (60–80% sensitive); CSF and serum CRYPTOCOCCAL ANTIGEN (CrAg, lateral flow assay) is >95% sensitive and is the test of choice**; culture on Sabouraud; **mucicarmine and Fontana–Masson** stain the capsule and melanin.
- CSF often shows a **paucity of inflammatory cells** in advanced AIDS; **imaging is normal in most (~50–80%)** — a normal CT does not exclude the diagnosis.
...
**EXAM CONNECTION:** Four statements, one true. The ART timing (**2–5 weeks, not
immediately**) is the examinable fact; monotherapy and "normal opening pressure" are both
wrong.
...
## 133. Brugada Syndrome

**PEARL:** **Coved ("shark-fin") ST elevation ≥2 mm with a descending ST segment and an
inverted T wave in V1–V3 (type 1 pattern)**, plus **syncope or nocturnal agonal
respiration/sudden death in a young man**, is **BRUGADA SYNDROME** — a **SCN5A sodium
channelopathy**. Treatment is an **implantable cardioverter-defibrillator**; no drug
prevents sudden death.

**UNDERSTAND:** The loss-of-function sodium channel reduces the inward current in **phase 0
and 1**, most severely in the **right ventricular OUTFLOW TRACT epicardium**, where the
transient outward K⁺ current (Ito) is strongest. That creates a **voltage gradient between
epicardium and endocardium** — seen on the ECG as ST elevation in the right precordial leads
— and a substrate for **phase 2 re-entry → polymorphic VT/VF**. Because vagal tone and
slow heart rates accentuate this gradient, events occur characteristically **at night, at
rest, or with fever**.
...
**EXAM CONNECTION:** Young patient + syncope at night + **coved ST elevation in V1–V3** with
...
**PEARL:** **Symmetrical polyarthritis of the small joints (MCP, PIP, wrists, MTPs) with
morning stiffness lasting MORE THAN ONE HOUR that improves with activity, and NO erythema**
= **rheumatoid arthritis**. **DIP joints are spared** — their involvement points to
osteoarthritis or psoriatic arthritis.

**UNDERSTAND:** RA is a **synovial** disease: the inflamed, proliferating synovium (pannus)
erodes cartilage and bone from the joint margins. Because the synovium is what is
inflamed, **rest allows inflammatory exudate and gel to accumulate → prolonged morning
stiffness**, and movement disperses it → improvement with use. **Osteoarthritis is a
cartilage-wear disease**, so its logic is exactly inverted: **stiffness lasts <30 minutes
and pain WORSENS with activity**.

**MUST KNOW:**
- **Deformities: ulnar deviation at the MCPs, SWAN-NECK (PIP hyperextension, DIP flexion), BOUTONNIÈRE (PIP flexion, DIP hyperextension), Z-thumb, "piano-key" ulnar head.** Also **atlantoaxial subluxation — always assess the cervical spine before intubation.**
- **Serology: rheumatoid factor (sensitive, not specific) and ANTI-CCP (highly SPECIFIC, predicts erosive disease)**; raised ESR/CRP; **X-ray — periarticular osteopenia, soft-tissue swelling, joint space narrowing and MARGINAL EROSIONS**; ultrasound/MRI detect synovitis earlier.
- **Extra-articular: rheumatoid nodules (on extensor surfaces, associated with RF positivity), interstitial lung disease and pleural effusion (with LOW GLUCOSE), pericarditis, scleritis and episcleritis, KERATOCONJUNCTIVITIS SICCA, vasculitis, anaemia of chronic disease, amyloidosis, FELTY syndrome (RA + splenomegaly + neutropenia), and CAPLAN syndrome (RA + pneumoconiosis).** **Cardiovascular disease is the leading cause of death.**
- **Treatment: start a DMARD EARLY — METHOTREXATE is first-line** (weekly, with **folic acid**; monitor LFTs and FBC; contraindicated in pregnancy); add hydroxychloroquine, sulfasalazine, leflunomide; **biologics (anti-TNF, tocilizumab, rituximab, abatacept) and JAK inhibitors** for inadequate response. **Steroids only as a bridge.** **Screen for latent TB and hepatitis B before anti-TNF therapy.**
- Contrast **osteoarthritis: DIP (Heberden) and PIP (Bouchard) nodes, asymmetrical, weight-bearing joints, X-ray shows joint space narrowing, subchondral SCLEROSIS and CYSTS, and OSTEOPHYTES.**

**EXAM CONNECTION:** The duration of morning stiffness and the **joint distribution** are the
discriminators. **>1 hour and symmetrical small joints = RA.**

**REMEMBER:** *An hour of stiffness that eases with use, sparing the DIPs.*

`Q11037`

---

## 135. Which Heart Failure Drugs Change Mortality

**PEARL:** **ACE inhibitors (captopril, enalapril, ramipril) DELAY PROGRESSION and IMPROVE
SURVIVAL** in heart failure with reduced ejection fraction. **Furosemide relieves symptoms
but has NO mortality benefit; DIGOXIN reduces hospitalisation but not mortality.**

**UNDERSTAND:** The distinction is between **treating congestion and treating the disease**.
Heart failure progresses because of **chronic neurohormonal activation — RAAS and
sympathetic overdrive — which drives adverse ventricular REMODELLING (hypertrophy,
fibrosis, dilatation)**. Diuretics remove fluid but do nothing to that process (and by
activating renin, may even worsen it). ACE inhibitors, beta-blockers, MRAs and ARNIs block
the remodelling pathway itself, which is why only they prolong life.
...
**EXAM CONNECTION:** The stem asks what **"delays progression"** or **"improves survival"** —
not what relieves breathlessness. Diuretic options are always the trap.
...
## 136. Unstable Tachyarrhythmia: Cardiovert Immediately

**PEARL:** A tachyarrhythmia with **ADVERSE FEATURES — hypotension/shock, altered
consciousness, ischaemic chest pain or heart failure — requires IMMEDIATE SYNCHRONISED DC
CARDIOVERSION**, not adenosine, verapamil or vagal manoeuvres.

**UNDERSTAND:** Drugs take minutes to work and **all antiarrhythmics are also negatively
inotropic and pro-arrhythmic** — exactly the wrong thing in a patient whose cardiac output
has already failed. Electricity resets the entire myocardium in one beat. The
**synchronisation** matters: the shock is timed to the R wave so it cannot land on the
**T wave (the vulnerable period)** and precipitate ventricular fibrillation — which is why
you synchronise for any organised rhythm with a pulse and **defibrillate (unsynchronised)
only for VF and pulseless VT**.

**MUST KNOW:**
- **Sedate the conscious patient first** if time allows. Energies (biphasic): **narrow-complex regular (SVT/atrial flutter) 70–120 J; atrial fibrillation 120–150 J; VT with a pulse 120–150 J**, escalating.
- If three shocks fail: **amiodarone 300 mg IV over 10–20 minutes, then repeat the shock**, and correct **the 4 Hs and 4 Ts** (especially potassium and magnesium).
- **STABLE narrow-complex regular tachycardia → vagal manoeuvres (Valsalva, ideally the MODIFIED Valsalva) → ADENOSINE 6 mg rapid IV push, then 12 mg** (warn about the transient sense of doom; **contraindicated in asthma; potentiated by dipyridamole; blocked by theophylline/caffeine**). **Stable broad-complex → treat as VT with amiodarone.**
- **Adenosine also unmasks the underlying rhythm** in atrial flutter by transiently blocking the AV node.
- **NEVER give AV nodal blockers (adenosine, verapamil, diltiazem, digoxin, beta-blockers) in pre-excited AF (WPW with an irregular broad-complex tachycardia)** — blocking the node drives conduction down the accessory pathway and can precipitate VF. Use **procainamide or electricity**.
- **Torsades de pointes: IV MAGNESIUM SULPHATE**, correct potassium, stop the offending QT-prolonging drug, overdrive pacing.

**EXAM CONNECTION:** Scan the stem for **haemodynamic instability** before looking at the
...
## 137. Digoxin Toxicity

**PEARL:** All of these are true: the **earliest manifestations are GASTROINTESTINAL
(anorexia, nausea, vomiting)**; **visual changes (blurring, non-specific disturbance, and
classically XANTHOPSIA — yellow-green haloes) occur**; **early toxicity may NOT correlate
with serum levels**; **neurological symptoms can occur without cardiac changes**; and
**HYPOKALAEMIA increases toxicity**.

**UNDERSTAND:** Digoxin competes with **potassium for the same binding site on the
Na⁺/K⁺-ATPase**. When potassium falls, less of it occupies that site, so **more digoxin
binds and toxicity occurs at "therapeutic" serum levels** — which is exactly why the serum
concentration correlates so poorly with clinical toxicity, and why **any drug that lowers
potassium (loop and thiazide diuretics) is the commonest precipitant**. Conversely,
hyperkalaemia in acute overdose signals massive pump inhibition and predicts death.
...
**EXAM CONNECTION:** "All are true" is often the right answer here — the statements are all
classical teaching. The one to remember is **poor level–toxicity correlation** and the
**hypokalaemia link**.

**REMEMBER:** *Low potassium lets digoxin in — the level in the blood tells you less than
the patient does.*
...
## 138. Differentiation Syndrome in Acute Promyelocytic Leukaemia

**PEARL:** **Fever, dyspnoea, pulmonary infiltrates, weight gain, oedema, hypotension and
renal impairment days after starting ATRA (all-trans retinoic acid) or arsenic trioxide for
APML = DIFFERENTIATION SYNDROME** — treat immediately with **high-dose DEXAMETHASONE (10 mg
twice daily)**.

**UNDERSTAND:** ATRA does not kill the leukaemic promyelocytes; it **forces them to
differentiate into mature granulocytes**. Those newly mature cells **express adhesion
molecules and release inflammatory cytokines**, so they marginate in the pulmonary
microvasculature and cause a **capillary leak syndrome**. The syndrome is thus a consequence
of the treatment working — which is why the answer is a **corticosteroid to suppress the
cytokine response**, not more chemotherapy, and why treatment must not be delayed for
confirmation.
...
- **Arsenic trioxide also causes QT PROLONGATION** — monitor the ECG and electrolytes.
- Distinguish from **tumour lysis syndrome** (hyperkalaemia, hyperphosphataemia, hyperuricaemia, hypocalcaemia and AKI — prevent with hydration and **rasburicase/allopurinol**) and from **neutropenic sepsis or pneumonia**, which the syndrome closely mimics — in practice antibiotics are given alongside the steroid.

**EXAM CONNECTION:** The stem gives a **recently treated APML patient with new respiratory
symptoms and infiltrates**. The answer is the steroid, not another cytotoxic drug.
...
## 139. Cortical vs Subcortical Dementia

**PEARL:** **PICK's DISEASE (frontotemporal dementia) is a CORTICAL dementia.** The
**SUBCORTICAL** dementias are those of **PARKINSON's disease, HUNTINGTON's disease, WILSON's
disease, progressive supranuclear palsy, HIV, normal pressure hydrocephalus and vascular
(lacunar) dementia**.

**UNDERSTAND:** The distinction predicts the **clinical signature**. **Cortical dementias
destroy the "instrument" of cognition** — so they produce **aphasia, apraxia, agnosia and
amnesia** with normal movement early on. **Subcortical dementias damage the basal ganglia,
thalamus and white matter — the "wiring"** — so cognition is **slowed rather than lost**
(bradyphrenia), and the striking features are **movement disorder, apathy, depression and
impaired executive function with relatively preserved language and recognition**.
...
| Speed | Normal early | **Markedly SLOWED (bradyphrenia)** |
...
**EXAM CONNECTION:** "Which is NOT subcortical?" — the answer is the **frontal/cortical
degeneration (Pick's)**. Any dementia listed with a **movement disorder** is subcortical.
...
## 140. Rabies: Diagnosis and the Corneal Impression

**PEARL:** Rabies is diagnosed **ante-mortem** by **direct fluorescent antibody testing or
RT-PCR on saliva, nuchal skin biopsy and CSF, with corneal impression smears** — but the
**definitive, confirmatory standard remains VIRAL ISOLATION (culture)**. **Negri bodies
(Seller stain) are a POST-MORTEM brain finding and are only ~70% sensitive.**

**UNDERSTAND:** No single ante-mortem test is sensitive enough, because the virus is
**sequestered in neural tissue and shed intermittently** — which is why multiple specimens
are tested in parallel and repeated. Culture and PCR detect the **virus itself**, whereas
serology is useless in an unvaccinated patient who dies before mounting antibody. And no
test should ever delay **post-exposure prophylaxis**, because once symptoms appear rabies is
**essentially 100% fatal**.

**MUST KNOW:**
- **Clinical: incubation weeks to months (shorter with bites of the face/hands, which are richly innervated); prodrome with pain or paraesthesia AT THE BITE SITE (highly suggestive); then FURIOUS rabies (hydrophobia, aerophobia, agitation, autonomic storm) or PARALYTIC/dumb rabies (ascending flaccid paralysis, mimicking Guillain–Barré).**
...
**EXAM CONNECTION:** Note whether the question asks for the **conclusive/definitive** test
(culture or PCR) or the **rapid ante-mortem** one (dFA on corneal impression/nuchal skin).
...
## 141. Crohn Disease vs Ulcerative Colitis

**PEARL:** **PERIANAL FISTULAE, skip lesions, transmural inflammation, granulomas and
strictures = CROHN DISEASE.** **Continuous lesions from the rectum proximally,
mucosal-only inflammation, HAEMATOCHEZIA and the higher risk of COLORECTAL MALIGNANCY =
ULCERATIVE COLITIS.**

**UNDERSTAND:** Nearly every difference follows from **depth versus extent**. Crohn is
**transmural**, so inflammation burrows through the full wall → **fistulae, abscesses,
sinus tracts and fibrotic strictures**, and it can occur **anywhere from mouth to anus in
discontinuous segments**. Ulcerative colitis is confined to **mucosa and submucosa**, so it
cannot fistulate, but the whole mucosal surface bleeds → **bloody diarrhoea with mucus**;
and because it is a **continuous field of chronically inflamed colonic mucosa**, its cancer
risk is higher.
...
|---|---|---|
| Site | **Mouth to anus; TERMINAL ILEUM commonest; RECTAL SPARING** | **Rectum ALWAYS, extending proximally, continuous** |
| Depth | **Transmural** | **Mucosa/submucosa only** |
| Endoscopy | **Skip lesions, COBBLESTONING, aphthous and deep linear ulcers** | Continuous granular friable mucosa, **PSEUDOPOLYPS** |
| Histology | **NON-CASEATING GRANULOMAS**, lymphoid aggregates | **CRYPT ABSCESSES**, goblet cell depletion |
| Complications | **Fistulae, perianal disease, abscess, STRICTURES, obstruction, gallstones and OXALATE renal stones (ileal disease), B12 deficiency** | **TOXIC MEGACOLON, massive haemorrhage, higher CANCER risk** |
| Smoking | **WORSENS** it | **PROTECTIVE** (paradoxical) |
...
**EXAM CONNECTION:** Only one feature in the option list belongs uniquely to Crohn:
**perianal fistulae**. Continuous lesions, haematochezia and higher malignancy risk all
belong to UC.

**REMEMBER:** *Crohn goes through the wall (fistulae); colitis stays on the surface
(bleeding).*
...
## 142. Infectious Oesophagitis at Endoscopy

**PEARL:** **Large, LINEAR or SERPIGINOUS (snake-like) shallow ulcers in the DISTAL
oesophagus = CMV OESOPHAGITIS.** **Small, punched-out "VOLCANO-like" ulcers = HSV.**
**White adherent plaques that scrape off = CANDIDA.**

**UNDERSTAND:** The morphology reflects **which cell each pathogen infects**. **CMV infects
ENDOTHELIAL and mesenchymal cells deep in the ulcer base**, so it destroys tissue from below
and produces **broad, deep, coalescing ulcers** — and biopsies must be taken **from the
ULCER BASE**. **HSV infects SQUAMOUS EPITHELIAL cells at the margin**, so it makes **shallow,
small, discrete ulcers with heaped-up edges**, and biopsies are taken **from the EDGE**.
**Candida grows ON the mucosal surface**, forming a removable pseudomembrane.
...
- Non-infectious causes of odynophagia/dysphagia to know: **pill oesophagitis (doxycycline, NSAIDs, bisphosphonates, potassium — taken with too little water or lying down), reflux oesophagitis, EOSINOPHILIC oesophagitis (young atopic men, food impaction, RINGS/trachealisation and linear furrows, >15 eosinophils/hpf — treat with topical steroids and dietary elimination), and radiation oesophagitis.**

**EXAM CONNECTION:** Endoscopic morphology → organism. **Serpiginous/linear = CMV;
punched-out = HSV; plaques = Candida.**

**REMEMBER:** *CMV digs long snaking ulcers from below; herpes punches small craters from
the surface.*
...
## 143. Polycythaemia: Check the Erythropoietin First

**PEARL:** In any patient with a raised haemoglobin, the discriminating test is the **SERUM
ERYTHROPOIETIN level**: **LOW EPO = POLYCYTHAEMIA VERA (primary, autonomous — then test
JAK2)**; **HIGH or normal EPO = SECONDARY polycythaemia (hypoxia — high altitude, COPD,
sleep apnoea, cyanotic heart disease, smoking — or an EPO-secreting tumour)**.

**UNDERSTAND:** Erythropoietin is the **physiological feedback signal**. In a **primary**
marrow disorder the red cell mass rises independently of any stimulus, so the normal
feedback **suppresses EPO to a low level**. In a **secondary** polycythaemia the raised red
cell mass is the *consequence* of a high EPO drive, so EPO is high or inappropriately
normal. One test therefore splits the differential in two before any expensive genetic
testing.

**MUST KNOW:**
- **Polycythaemia vera: JAK2 V617F mutation in >95%**; features include **ERYTHROMELALGIA, aquagenic PRURITUS (itching after a hot bath), splenomegaly, thrombosis (the leading cause of death, arterial and venous — including BUDD–CHIARI syndrome), gout, and a low EPO.** **Treatment: VENESECTION to a haematocrit <45%, LOW-DOSE ASPIRIN, and HYDROXYUREA (or ruxolitinib) for high-risk patients.**
- **Secondary polycythaemia at high altitude** is a physiological adaptation — the patient in the stem also has a **leucoerythroblastic-looking picture with myelocytes**, so **CML must be excluded** (BCR-ABL/Philadelphia chromosome) if the EPO and clinical setting do not fit.
- **RELATIVE (spurious/Gaisböck) polycythaemia:** raised haematocrit from **plasma volume contraction** — dehydration, diuretics, burns; the **red cell mass is normal**.
...
**EXAM CONNECTION:** "Next step" in a polycythaemic patient with an environmental clue
(**high altitude**) → **erythropoietin level** before JAK2 or a marrow biopsy.
...
## 144. Sodium Deficit and the Glasgow Coma Scale

**PEARL:** **Sodium deficit (mEq) = Total body water × (target Na⁺ − actual Na⁺)**, where
**TBW = 0.6 × weight in men (0.5 in women, 0.45 in elderly women)**. For a 70 kg man with
Na⁺ 120: **0.6 × 70 × (140 − 120) = 840 mEq.** **GCS = Eye (4) + Verbal (5) + Motor (6),
minimum 3, maximum 15.**

**UNDERSTAND:** The formula works because sodium distributes through **total body water**,
not just plasma — so replacing a deficit means raising the concentration across the entire
water compartment, which is why body weight enters the calculation. The critical clinical
corollary is that the calculated deficit tells you **how much**, never **how fast**:
**correct by no more than 8–10 mEq/L in 24 hours** (4–6 mEq/L in high-risk patients), or the
brain's osmotic adaptation is outstripped and **osmotic demyelination (central pontine
myelinolysis)** follows.
...
**EXAM CONNECTION:** Both are pure calculations. Watch the GCS trap: **localising with ONE
arm scores 5**, even if the other arm only flexes — always the best response.

**REMEMBER:** *0.6 × weight × the gap for sodium; best eye, best voice, best movement for
GCS.*
...
## 145. Hashimoto Thyroiditis

**PEARL:** Hypothyroid symptoms + a **diffusely enlarged, firm, NON-TENDER goitre** +
**raised TSH with low free T4** + histology showing **lymphocytic infiltration with GERMINAL
CENTRES and HÜRTHLE (Askanazy) cell change** = **HASHIMOTO (chronic lymphocytic)
THYROIDITIS** — the commonest cause of hypothyroidism in iodine-sufficient areas.

**UNDERSTAND:** Hashimoto is a **cell-mediated autoimmune destruction** of thyroid follicles,
so the gland is **infiltrated rather than inflamed by infection** — which is why it is
**firm and painless**, in contrast to the exquisitely **tender** gland of subacute (de
Quervain) thyroiditis. The **Hürthle cells** are follicular cells that have accumulated
mitochondria in response to chronic injury — a marker of the long-standing metabolic stress
on the surviving follicles.
...
- **Distinguish the thyroiditides:**
  - **SUBACUTE (de Quervain) — PAINFUL tender gland after a viral illness, raised ESR, LOW radioiodine uptake, transient thyrotoxicosis then hypothyroidism then recovery; treat with NSAIDs/steroids and beta-blockers, NOT antithyroid drugs.**
  - **RIEDEL — a "woody hard", fixed, infiltrative gland that mimics carcinoma and compresses adjacent structures; an IgG4-related fibrosing disorder; treat with steroids/tamoxifen.**
  - **Silent/postpartum thyroiditis — painless, antibody-positive, self-limiting.**
  - **Toxic multinodular goitre — HYPERthyroid with a nodular gland and patchy ("hot and cold") uptake.**
...
**EXAM CONNECTION:** Painless firm goitre with hypothyroidism → Hashimoto; a **painful**
gland points to de Quervain; a **rock-hard fixed** gland to Riedel or carcinoma.
...
## 146. Neurofibromatosis Type 1

**PEARL:** A slowly enlarging, **rubbery, painless neck mass** in a child with **CAFÉ-AU-LAIT
MACULES and axillary/inguinal FRECKLING** is a **NEURAL TUMOUR (plexiform neurofibroma) in
NEUROFIBROMATOSIS TYPE 1** — not a cystic hygroma, lipoma or chemodectoma.

**UNDERSTAND:** NF1 is a **RAS-pathway disorder**: the *NF1* gene encodes **neurofibromin, a
GTPase-activating protein that switches RAS off**. Losing it leaves RAS constitutively
active, so **any cell of neural crest origin proliferates** — Schwann cells (neurofibromas),
melanocytes (café-au-lait macules and freckling), iris melanocytes (Lisch nodules) and
adrenal chromaffin cells (phaeochromocytoma). One failed brake explains the whole syndrome.
...
**EXAM CONNECTION:** The skin findings in the stem are the diagnosis — the mass is
incidental to recognising the **syndrome**.
...
## 147. Rapidly Progressive Glomerulonephritis: Reading the Immunofluorescence

**PEARL:** Crescentic glomerulonephritis with **p-ANCA (anti-MPO) positivity** shows
**PAUCI-IMMUNE immunofluorescence — minimal or absent immunoglobulin and complement
deposition.** **Linear IgG = anti-GBM (Goodpasture); granular mesangial IgA = IgA
nephropathy/HSP; granular subepithelial C3 = post-streptococcal GN.**

**UNDERSTAND:** "Pauci-immune" seems paradoxical in an antibody-associated disease, but ANCA
does not deposit in the glomerulus — it **activates circulating NEUTROPHILS**, which then
degranulate against the endothelium. The damage is therefore **cell-mediated at the vessel
wall**, leaving nothing to stain. Immunofluorescence is thus a direct readout of
**mechanism**: linear = antibody against a fixed structural antigen; granular = immune
complexes trapped; absent = leucocyte-mediated.
...
**EXAM CONNECTION:** The serology in the stem dictates the IF answer. **ANCA → pauci-immune**;
...
## 148. Type 4 Respiratory Failure

**PEARL:** **TYPE 4 respiratory failure = respiratory muscle hypoperfusion in SHOCK.** The
patient is intubated **not for hypoxaemia or hypercapnia** — the PaO₂ may be normal — but to
**unload the respiratory muscles and redistribute cardiac output to vital organs**.

**UNDERSTAND:** Breathing normally consumes about **5% of cardiac output**; in shock with
severe metabolic acidosis, the compensatory hyperventilation can drive that to **30–40%**.
In a patient whose total oxygen delivery is already inadequate, the diaphragm becomes a
**competing organ**, stealing perfusion from the brain, gut and kidneys — and eventually
fatiguing and failing itself. Mechanical ventilation abolishes that work, which is why
intubation can improve haemodynamics even when the gas exchange is fine.

**MUST KNOW — the four types:**
- **TYPE 1 (hypoxaemic): PaO₂ <60 mmHg with a normal or low PaCO₂** — a V/Q mismatch or shunt: **pneumonia, pulmonary oedema, ARDS, pulmonary embolism, atelectasis**.
- **TYPE 2 (hypercapnic/ventilatory): PaCO₂ >50 mmHg** — a pump failure: **COPD, severe asthma, opioid overdose, neuromuscular disease (Guillain–Barré, myasthenia), chest wall disease, obesity hypoventilation**.
- **TYPE 3 (perioperative/atelectatic):** reduced functional residual capacity with atelectasis after anaesthesia or abdominal surgery — prevented by **incentive spirometry, analgesia and early mobilisation**.
- **TYPE 4 (shock-related):** as above; intubation reduces the metabolic cost of breathing.
- **In type 2 failure from COPD, NON-INVASIVE VENTILATION (BiPAP) is first-line**; in type 1 from pulmonary oedema, **CPAP**. Watch for the danger of **uncontrolled oxygen in chronic CO₂ retainers** (target SpO₂ 88–92%).
- **A rising PaCO₂ in acute severe asthma is an ominous sign** — it means the patient is tiring and needs intensive care, not more nebulisers.

**EXAM CONNECTION:** The stem gives **shock + tachypnoea + NORMAL PaO₂**. That combination
exists only in type 4.

**REMEMBER:** *In shock, the diaphragm steals the blood the organs need — ventilate to buy
it back.*
...
## 149. Hepatitis D and Bulevirtide

**PEARL:** **BULEVIRTIDE is the first-in-class ENTRY INHIBITOR for chronic hepatitis D** — it
binds the **sodium taurocholate cotransporting polypeptide (NTCP)** receptor on hepatocytes,
the shared entry portal for **both HBV and HDV**.

**UNDERSTAND:** HDV is a **defective (satellite) RNA virus** that cannot make its own
envelope — it borrows **HBsAg**, so it can only infect someone who already has HBV, and it
enters cells by the same HBsAg–NTCP interaction. Blocking NTCP therefore stops HDV spreading
to uninfected hepatocytes without needing to target the virus's own replication machinery.
That dependence also explains the central prevention fact: **the hepatitis B vaccine
prevents hepatitis D**.

**MUST KNOW:**
- **CO-INFECTION (HBV and HDV acquired together): usually a severe acute hepatitis but with a LOW risk of chronicity (<5%). SUPERINFECTION (HDV acquired by a chronic HBV carrier): high risk of chronic HDV, fulminant hepatitis, and ACCELERATED progression to cirrhosis and HCC.** Chronic hepatitis D is the **most severe form of chronic viral hepatitis**.
- **Diagnosis: anti-HDV IgM/IgG and HDV RNA in a patient who is HBsAg-positive.** Test every HBsAg-positive patient at least once.
- **Historic treatment was PEGYLATED INTERFERON ALFA** (poorly effective, ~25% sustained response); nucleoside analogues (tenofovir, entecavir) suppress HBV but **do NOT treat HDV**.
- Related new agents in the options worth recognising: **bepirovirsen** — an antisense oligonucleotide against HBV; **fostemsavir** — an HIV **attachment (gp120)** inhibitor; **pegylated interferon lambda** — an investigational HDV therapy.
- **Hepatitis serology reminders: HBsAg = current infection; anti-HBc IgM = window period; HBeAg = high infectivity; anti-HBs alone = vaccination.**

**EXAM CONNECTION:** New-drug recall. **Entry inhibitor + hepatitis D = bulevirtide**; the
distractors are drugs for HBV and HIV.
...
**PEARL:** A young woman with **relapsing–remitting neurological deficits separated in TIME
and SPACE** (optic neuritis, sensory symptoms) and MRI showing **DAWSON'S FINGERS —
periventricular ovoid lesions oriented PERPENDICULAR to the ventricles on FLAIR** — has
**MULTIPLE SCLEROSIS**; the CSF shows **OLIGOCLONAL IgG BANDS not present in serum**.

**UNDERSTAND:** Dawson's fingers are perpendicular because demyelination begins as
**perivenular inflammation around the deep medullary veins**, which radiate outward from the
ventricles — so the plaques trace the venous anatomy. Oligoclonal bands mean
**INTRATHECAL immunoglobulin synthesis by a restricted number of plasma cell clones behind
the blood–brain barrier**; they must be **absent from a paired serum sample**, otherwise
they merely reflect systemic disease.

**MUST KNOW:**
- **McDonald criteria: dissemination in SPACE (lesions in ≥2 of 4 typical CNS sites — periventricular, juxtacortical, infratentorial, spinal cord) and in TIME (a new lesion on follow-up, or simultaneous enhancing and non-enhancing lesions); CSF-specific oligoclonal bands can substitute for dissemination in time.**
- **Classic features: OPTIC NEURITIS (painful monocular loss with a relative afferent pupillary defect), INTERNUCLEAR OPHTHALMOPLEGIA (medial longitudinal fasciculus — bilateral INO in a young person is MS until proven otherwise), LHERMITTE's sign (electric shock down the spine on neck flexion), UHTHOFF's phenomenon (symptoms worsen with heat), trigeminal neuralgia in a young patient, cerebellar signs, spasticity with CLONUS and extensor plantars, and bladder dysfunction.**
- **Treatment: acute relapse — high-dose IV methylprednisolone (shortens the relapse, does not change long-term outcome); disease-modifying therapy — interferon-β, glatiramer, teriflunomide, dimethyl fumarate, fingolimod, natalizumab (beware PML — check JC virus serology), ocrelizumab, alemtuzumab.** Plus symptomatic care: baclofen for spasticity, anticholinergics for bladder, amantadine/modafinil for fatigue.
- **Differentials the question tests: NEUROMYELITIS OPTICA (aquaporin-4 antibody — LONGITUDINALLY EXTENSIVE transverse myelitis over ≥3 vertebral segments with severe optic neuritis, usually oligoclonal-band NEGATIVE); NEUROSARCOIDOSIS (raised serum ACE, hilar lymphadenopathy); SLE and antiphospholipid syndrome; B12 deficiency; ADEM (monophasic, post-infectious, in children).**
- **CLONUS (≥3 sustained beats) is an upper motor neuron sign** — it occurs because the loss of descending inhibition leaves the stretch reflex unopposed, so each contraction re-stretches the spindle in a self-perpetuating loop. Also seen in stroke, cord compression, cerebral palsy and hepatic encephalopathy.

**EXAM CONNECTION:** MRI showing perpendicular periventricular lesions plus relapsing
symptoms → the answer is **CSF oligoclonal bands**. Aquaporin-4 is the trap for NMO.
...
## 151. SAAG and the Ascitic Protein

**PEARL:** **SAAG (serum albumin − ascitic albumin) ≥1.1 g/dL = PORTAL HYPERTENSION.** Then
use the **ASCITIC PROTEIN to find the level of the block: ≥2.5 g/dL = POST-hepatic/cardiac
(CONGESTIVE HEART FAILURE, constrictive pericarditis, Budd–Chiari); <2.5 g/dL = SINUSOIDAL
(CIRRHOSIS).** **SAAG <1.1 = NOT portal hypertension** (peritoneal TB, malignancy,
pancreatitis, nephrotic syndrome).

**UNDERSTAND:** The two numbers answer two different anatomical questions. **SAAG measures
the oncotic–hydrostatic balance across the sinusoid**, so it detects portal hypertension. The
**protein content then tells you whether the sinusoidal wall is intact**: in cardiac ascites
the sinusoids are **normal and permeable**, so protein-rich fluid is squeezed out by high
venous pressure; in cirrhosis the sinusoids are **capillarised and scarred**, so they retain
protein and the ascites is protein-poor. One test localises the lesion **above or within**
the liver.

**MUST KNOW:**
- **SAAG ≥1.1: cirrhosis, alcoholic hepatitis, cardiac failure, constrictive pericarditis, Budd–Chiari, massive liver metastases, portal vein thrombosis, myxoedema.** **SAAG <1.1: peritoneal carcinomatosis, TUBERCULOUS peritonitis, PANCREATIC ascites (very high amylase), nephrotic syndrome, serositis, bowel obstruction/infarction.**
- **SPONTANEOUS BACTERIAL PERITONITIS: ascitic neutrophil count ≥250/mm³** (regardless of culture) — treat with **cefotaxime plus IV ALBUMIN** (albumin reduces hepatorenal syndrome and mortality); then **norfloxacin/ciprofloxacin prophylaxis**. Usually **monomicrobial (E. coli)** — polymicrobial growth or a total protein >1 g/dL with multiple organisms suggests **secondary** peritonitis needing imaging and surgery.
- **Tuberculous peritonitis: high ascitic ADENOSINE DEAMINASE (ADA), lymphocyte predominance, low SAAG; diagnosis often by laparoscopic biopsy.**
- **Management of cirrhotic ascites: salt restriction (<2 g/day), SPIRONOLACTONE with furosemide in a 100:40 ratio, large-volume paracentesis with albumin cover (8 g per litre removed beyond 5 L), TIPS for refractory ascites, and transplantation.**
- Always perform a **diagnostic paracentesis on every new ascites** — cell count, albumin, protein, culture in blood-culture bottles at the bedside, and cytology/ADA as indicated.

**EXAM CONNECTION:** Two numbers are given, so both must be used. **High SAAG + high protein
= cardiac; high SAAG + low protein = cirrhosis.**
...
## 152. Giant Cell Arteritis and Polymyalgia Rheumatica

**PEARL:** New headache, **scalp tenderness and a tender, thickened, pulseless temporal
artery** with visual disturbance in someone **over 50** = **GIANT CELL (temporal)
ARTERITIS**, which coexists with **POLYMYALGIA RHEUMATICA — pain and morning stiffness of
the SHOULDER and HIP girdles — in up to 40–50%.**

**UNDERSTAND:** GCA is a **granulomatous vasculitis of large and medium arteries with an
INTERNAL ELASTIC LAMINA** — which is precisely why it targets the **branches of the external
and internal carotid (temporal, ophthalmic, posterior ciliary) and the aorta**, and why it
**spares intracranial vessels** (they lack that lamina). Blindness occurs through occlusion
of the **posterior ciliary arteries → anterior ischaemic optic neuropathy**, is **sudden,
painless and usually IRREVERSIBLE**, and the second eye follows within days if untreated —
hence the absolute rule below.
...
- Give **bone protection (calcium, vitamin D, bisphosphonate) and gastric protection** with long-term steroids, and taper over 1–2 years.

**EXAM CONNECTION:** The stem gives the temporal artery findings; the associated symptom
asked for is the **girdle pain of PMR**. Claudication of the *limbs* is the distractor.

**REMEMBER:** *Over 50 with a tender temporal artery — steroids first, biopsy after, and ask
about the shoulders.*
...
## 153. Cardiac Tamponade vs Constrictive Pericarditis

**PEARL:** Both share **diastolic equalisation of intracardiac pressures, exaggerated
respiratory variation in ventricular inflow, raised JVP and a small stroke volume.** The
**SMALL, COMPRESSED RIGHT VENTRICULAR CAVITY with diastolic RV collapse and a "swinging
heart" belongs to TAMPONADE only.** The **JVP waveform separates them: tamponade — prominent
X descent with an ABSENT Y descent; constriction — prominent X AND Y (the "M/W" pattern,
Friedreich's sign).**

**UNDERSTAND:** The y descent reflects **early diastolic ventricular filling**. In
**tamponade** the pericardial fluid presses on the heart **throughout diastole**, so filling
is impeded from the very start — the **y descent disappears**. In **constriction** the rigid
pericardium is a fixed shell: **early filling is rapid and unimpeded** (a steep y descent and
a **pericardial KNOCK**) and then stops abruptly when the ventricle hits the shell — the
**"dip and plateau"/square-root sign**. One is continuous compression, the other is a sudden
stop.
...
| Treatment | **Urgent PERICARDIOCENTESIS** | **PERICARDIECTOMY** |

- **In India, TUBERCULOSIS is the leading cause of constrictive pericarditis**; a calcified pericardium on lateral chest X-ray is a classic finding.
- **In tamponade, AVOID vasodilators and diuretics** — the patient is preload-dependent; give fluids and drain the pericardium.
...
**EXAM CONNECTION:** "Not common to both" — the answer is the **echocardiographic chamber
finding**, since the haemodynamic features overlap almost completely.
...
## 154. Dementia with Lewy Bodies

**PEARL:** Progressive dementia with **(1) recurrent, well-formed VISUAL HALLUCINATIONS,
(2) FLUCTUATING cognition, (3) spontaneous PARKINSONISM and (4) REM SLEEP BEHAVIOUR
DISORDER** = **DEMENTIA WITH LEWY BODIES**. The **"1-YEAR RULE": dementia beginning before,
or within 12 months of, the parkinsonism = DLB; parkinsonism for more than a year before
dementia = PARKINSON DISEASE DEMENTIA.**

**UNDERSTAND:** DLB is a **synucleinopathy** in which α-synuclein Lewy bodies accumulate in
the **CORTEX** as well as the substantia nigra — so cortical and nigral features appear
together, unlike Parkinson disease where nigral loss long precedes cortical spread. The
**severe cholinergic deficit (worse than in Alzheimer disease)** explains the hallucinations
and fluctuating attention, and is the reason **cholinesterase inhibitors (rivastigmine,
donepezil) work particularly well**.

**MUST KNOW:**
- **NEUROLEPTIC SENSITIVITY is the critical safety point — antipsychotics (especially typicals like haloperidol) can cause severe, sometimes fatal, worsening of rigidity and consciousness.** If a drug is unavoidable, use **low-dose quetiapine or clozapine**; never a typical antipsychotic.
- **Supportive features: repeated falls, syncope, autonomic dysfunction, depression, systematised delusions, and reduced dopamine transporter uptake on DaTscan** (a useful confirmatory investigation).
- **REM sleep behaviour disorder — acting out dreams — may precede the dementia by YEARS** and is one of the strongest prodromal markers of a synucleinopathy.
- **Contrast the dementias: ALZHEIMER — insidious anterograde memory loss first, no early hallucinations or parkinsonism (amyloid plaques and neurofibrillary tau tangles); VASCULAR — STEPWISE decline with focal signs and vascular risk factors; FRONTOTEMPORAL (Pick) — early personality and behavioural change with preserved memory; CREUTZFELDT–JAKOB — rapid decline with MYOCLONUS and periodic sharp waves on EEG.**
- Parkinsonism in DLB is often **symmetrical, with less tremor and a poorer levodopa response**; levodopa may worsen hallucinations.

**EXAM CONNECTION:** Visual hallucinations **early** in a dementia is the single most
discriminating feature; add fluctuation and parkinsonism and the diagnosis is fixed.
...
## 155. Withdrawing Antiepileptic Drugs
...
**MUST KNOW:**
- If seizure-free for only 6 months, the answer is **continue for another 18 months** to complete two years — the arithmetic is the exam point.
- Highest relapse risk: **abnormal EEG at withdrawal, symptomatic (structural) epilepsy, juvenile myoclonic epilepsy (usually lifelong therapy), multiple seizure types, need for more than one drug, long time to initial control**.
- **Juvenile myoclonic epilepsy** is the classic exception — relapse rate after withdrawal is very high, so treatment is generally lifelong.
- Withdraw **one drug at a time** if on polytherapy; counsel about **driving regulations**.
- **Phenytoin causes folate deficiency and megaloblastic anaemia** (and gum hypertrophy, hirsutism, cerebellar atrophy, osteomalacia via vitamin D metabolism, and is teratogenic) — **supplement folic acid**.
...
## 156. Loud S1 vs Soft S1

**PEARL:** **Rheumatic (pliable) mitral stenosis gives a LOUD S1; calcified mitral stenosis gives a SOFT S1.**

**UNDERSTAND:** Loudness depends on **how far and how fast the valve travels to close**. In early, pliable MS the raised left atrial pressure holds the leaflets **wide open** right up to the moment of ventricular systole, so they slam shut across a large excursion — loud. Once the leaflets **calcify and become immobile**, they barely move, and a valve that does not travel cannot make a loud sound.

**MUST KNOW:**
- **Loud S1:** early/pliable mitral stenosis, **short PR interval**, tachycardia, high-output states (fever, anaemia, thyrotoxicosis), **left atrial myxoma**.
- **Soft S1:** **calcified MS**, mitral regurgitation, **aortic regurgitation** (premature mitral closure from the regurgitant jet), **long PR interval (first-degree block)**, LV dysfunction, beta-blockers, and anything that muffles transmission (**obesity, COPD, pericardial effusion**).
- **Variable S1:** **complete heart block** (varying PR), **atrial fibrillation**.
- Other mitral stenosis signs to keep aligned: **opening snap** — a **shorter A2–OS interval means MORE severe** stenosis; the **length of the mid-diastolic murmur** tracks severity; loud P2 and a parasternal heave signal pulmonary hypertension.
- **Loud S1 + opening snap = pliable valve → suitable for balloon valvotomy**. Their disappearance signals calcification.

**EXAM CONNECTION:** All four options can involve the mitral valve. The discriminator is **pliable vs calcified**, and aortic regurgitation is included because it makes S1 *soft*, not loud.
...
## 157. Drugs That Precipitate Gout — CANT LEAP

**PEARL:** **HIGH-dose salicylates (> 3 g/day) are URICOSURIC** and do not precipitate gout; **LOW-dose aspirin (< 2–3 g/day) causes urate retention** and does.

**UNDERSTAND:** Salicylate has **biphasic** renal effects because it interferes with both urate **secretion** and **reabsorption** in the proximal tubule. At low doses the **secretory** block dominates (urate is retained); at high doses the **reabsorptive** block dominates (urate is excreted). Same drug, opposite effects, dose-dependent.

**MUST KNOW:**
- **CANT LEAP:** **C**yclosporine (and cytotoxics), **A**spirin (low dose), **N**icotinic acid, **T**hiazides, **L**asix (loop diuretics), **E**thambutol, **A**lcohol, **P**yrazinamide.
- Even at high doses, **aspirin is not used to treat acute gout** — NSAIDs, colchicine or steroids are.
- Acute attack: **NSAIDs, colchicine, or intra-articular/oral steroids**. **Do not start or stop urate-lowering therapy during an acute attack** (though established therapy is continued).
...
## 158. Osmolality Patterns — DI vs SIADH vs Psychogenic Polydipsia
...
**MUST KNOW:**
- Normal: **plasma 285–295 mOsm/kg**, **urine 500–850 mOsm/kg**.
- **Psychogenic polydipsia:** **hyponatraemia**, low plasma osm, **low urine osm**, urine output can be 6–10 L/day. Often psychiatric, or follows head injury.
- **DI:** **hypernatraemia**, high plasma osm, low urine osm. **Water deprivation test then desmopressin**: urine concentrates with DDAVP in **central** DI, not in **nephrogenic**.
- **SIADH:** euvolaemic hyponatraemia, **urine Na > 40 mEq/L**, low uric acid, normal thyroid/adrenal function. **Cerebral salt wasting** looks identical but the patient is **hypovolaemic** — that volume status is the only reliable discriminator, and it dictates opposite treatment (**fluid restriction in SIADH, salt and fluid replacement in CSW**).
- Hyponatraemia is the **commonest electrolyte abnormality** and a common cause of **seizures**; correct **no faster than 8–10 mEq/L in 24 h** to avoid **osmotic demyelination**.

**EXAM CONNECTION:** The numbers *are* the question. Compare each against the normal range before looking at the options — the pattern uniquely identifies the diagnosis.
...
## 159. Central vs Peripheral Cyanosis

**PEARL:** Cyanosis appears when **reduced haemoglobin exceeds 5 g/dL** in capillary blood — an **absolute**, not fractional, threshold. **Hypothermia causes PERIPHERAL, not central, cyanosis.**

**UNDERSTAND:** Because the threshold is absolute, **anaemia masks cyanosis** (a patient with Hb 6 g/dL can be profoundly hypoxic and never look blue) while **polycythaemia exaggerates it**. Central cyanosis means the **arterial blood leaving the heart is already desaturated or abnormal**; peripheral cyanosis means normally saturated blood has been **over-extracted during sluggish capillary transit**.
...
## 160. Thyroid Storm with Atrial Fibrillation in an Asthmatic

**PEARL:** In a **thyroid storm patient with asthma and AF**, use **diltiazem** — not propranolol or esmolol — for rate control.

**UNDERSTAND:** Beta-blockade is the standard first move in thyroid storm because it blunts the catecholamine-driven arrhythmia and (with propranolol) inhibits **T4→T3 conversion**. But in **bronchospastic disease**, non-selective β-blockade risks fatal bronchoconstriction. A **non-dihydropyridine calcium channel blocker** achieves the same AV-nodal rate control by a different route.
...
## 161. Aortic Dissection — Investigation by Stability

**PEARL:** **Unequal pulses + chest pain + non-specific ECG = aortic dissection.** In the **haemodynamically UNSTABLE** patient the investigation of choice is **transoesophageal echocardiography (TOE)**; in the stable patient it is **CT angiography**.

**UNDERSTAND:** The choice is dictated by **whether the patient can leave the resuscitation area**. TOE is done at the bedside in minutes with the patient monitored; CT requires transport to a scanner, which an unstable dissection patient may not survive. Diagnostic accuracy is comparable — availability and safety decide.

**MUST KNOW:**
- Features favouring dissection over MI: **pulse deficit / blood pressure difference between arms (> 20 mmHg)**, **tearing pain radiating to the back**, **aortic regurgitation murmur**, **widened mediastinum**, focal neurological deficit — with **non-specific ECG changes**.
- **Stanford A** (ascending aorta involved) → **surgical emergency**; **Stanford B** (descending only) → **medical management**, with intervention for complications. DeBakey I/II/III maps onto this.
- Medical therapy: **rapid reduction of blood pressure and dP/dt with IV beta-blocker FIRST (esmolol/labetalol)**, then a vasodilator (**nitroprusside**). Never give the vasodilator first — reflex tachycardia increases shear stress.
- Risks: **hypertension** (commonest), **Marfan and Ehlers–Danlos**, bicuspid aortic valve, coarctation, pregnancy, cocaine, trauma.
...
## 162. Hyperkalaemia — ECG Sequence
...
**MUST KNOW:**
- Progression: **5.5–6.5 → tall peaked T waves**; **6.5–8.0 → flattened/absent P waves, prolonged PR, QRS widening**; **> 8.0 → sine wave, VF, asystole**.
- Emergency treatment, in order: **IV calcium gluconate** (stabilises the myocardium — does not lower K⁺) → **insulin with dextrose**, **salbutamol nebs**, **sodium bicarbonate if acidotic** (shift K⁺ into cells) → **potassium binders (patiromer, sodium zirconium), loop diuretics, dialysis** (remove K⁺).
- Drug causes: **ACE inhibitors/ARBs, spironolactone/eplerenone, amiloride, NSAIDs, heparin, trimethoprim, digoxin toxicity, succinylcholine, beta-blockers**.
- **Pseudohyperkalaemia** — haemolysed sample, prolonged tourniquet, marked thrombocytosis or leucocytosis — always repeat before treating an asymptomatic result.
- **Spironolactone** also causes **gynaecomastia** (eplerenone does not) — and it **reduces mortality in HFrEF**, which is why it is not simply avoided.
...
## 163. CSF Patterns in Meningitis

**PEARL:** **Lymphocytes + NORMAL glucose = VIRAL.** **Lymphocytes + LOW glucose = tuberculous or fungal.** **Neutrophils + LOW glucose = bacterial.**

**UNDERSTAND:** Glucose is the discriminator because it is **consumed by organisms and by activated leucocytes**. Viruses are **intracellular and do not metabolise CSF glucose**, so the level stays normal — while bacteria, mycobacteria and fungi actively consume it. The cell type tells you the immune response; the glucose tells you whether something is *eating*.

**MUST KNOW:**
- **Normal CSF:** glucose 40–85 mg/dL (**≈ 2/3 of blood glucose** — always check a paired blood sugar), protein 15–45 mg/dL, **< 5 cells/µL**, opening pressure 10–20 cmH₂O.
- **Bacterial:** glucose < 40, protein > 100, **> 1000 cells with ≥ 80% PMNs**.
- **Viral (Coxsackie, echovirus, mumps, HSV-2):** **normal glucose**, protein 20–80, 25–500 lymphocytes.
- **Tuberculous:** glucose < 50 in 75%, protein 100–200 (may be very high), 25–100 lymphocytes, **cobweb coagulum**, high adenosine deaminase.
- **Cryptococcal (HIV, CD4 < 100):** lymphocytic, low glucose, **very high opening pressure** (yeast obstruct CSF outflow — repeated therapeutic LPs are part of treatment); **India ink** and **cryptococcal antigen (CrAg)**. Treat with **amphotericin B + flucytosine** induction, then fluconazole.
- **Very early bacterial meningitis** can be lymphocytic, and **partially treated** bacterial meningitis mimics viral — the two classical exceptions.

**EXAM CONNECTION:** Two stems from the same table. Read glucose first, cell type second — that order resolves every variant.
...
## 164. Prerenal vs Intrinsic AKI

**PEARL:** **NSAIDs, ACE inhibitors and (partly) rhabdomyolysis cause PRERENAL AKI. Myeloma light chains, snake venom and myoglobin cause INTRINSIC (ATN) injury.**

**UNDERSTAND:** Prerenal injury is a **haemodynamic** problem — the glomerulus is starved of perfusion pressure but the tubules are structurally intact, so it is **fully reversible with volume**. NSAIDs remove the **afferent vasodilator (prostaglandin)**; ACE inhibitors remove the **efferent vasoconstrictor (angiotensin II)** — opposite ends of the glomerulus, same net fall in filtration pressure. Nephrotoxins, by contrast, **kill tubular cells**.
...
- Rhabdomyolysis management: **aggressive IV fluids**, monitor **CK, potassium, calcium, phosphate**; **hyperkalaemia is the immediate killer**.
...
## 165. Berry Aneurysm — Commonest vs Most Likely to Rupture

**PEARL:** The **commonest site** of a berry aneurysm is the **anterior communicating artery**, but the **highest rupture risk** is at the **posterior communicating artery origin** and the **basilar tip**.

**UNDERSTAND:** Frequency and rupture risk are **different questions**. Frequency reflects **haemodynamic stress at branch points** of the anterior circulation; rupture risk reflects **wall geometry and location** — posterior circulation aneurysms have thinner walls and less surrounding support, so they rupture at smaller sizes.
...
**PEARL:** Respiratory distress after transfusion with **HYPERTENSION and raised BNP = TACO** (circulatory overload) — investigate with **chest X-ray and BNP**. **TRALI causes HYPOtension** and is associated with **anti-leucocyte antibodies**.

**UNDERSTAND:** The two produce identical pictures on the chest film — bilateral infiltrates and hypoxia — because both fill the alveoli. The difference is **hydrostatic vs permeability** oedema. TACO is a **volume problem**: filling pressures rise, so blood pressure and BNP rise. TRALI is **capillary leak** triggered by donor antibodies against recipient neutrophils, so the patient is **vasodilated, hypotensive and often febrile**, with normal filling pressures.

**MUST KNOW:**
- **TACO:** within **6 hours**, hypertension, raised **BNP**, raised JVP, responds to **stopping the transfusion, oxygen and diuretics**. Risk factors: **elderly, cardiac or renal dysfunction, pre-existing fluid overload, large volumes, rapid transfusion**.
- **TRALI:** within **6 hours**, **hypotension, fever, hypoxia**, normal BNP and JVP; **diuretics do NOT help** — treat with **supportive ventilation**. Implicated: **plasma-rich components** from **multiparous female donors** (HLA/HNA antibodies) — hence male-predominant plasma donation policies.
- **TRALI is a leading cause of transfusion-related death**; **TACO is the commonest serious transfusion reaction**.
- Other reactions to keep separate: **acute haemolytic (ABO incompatibility — fever, loin pain, haemoglobinuria)**, **febrile non-haemolytic (leucocyte cytokines — commonest overall)**, **anaphylaxis in IgA deficiency**, **bacterial contamination (platelets, highest risk)**.
...
## 167. Rheumatoid Arthritis vs SLE on X-ray
...
## 168. Choosing an Antidiabetic — and Stopping Metformin

**PEARL:** **Stop metformin when eGFR < 30 mL/min** (or creatinine markedly raised) — risk of **lactic acidosis**. In an obese patient with **prior pancreatitis and a family history of bladder cancer** who refuses injections, the answer is an **SGLT-2 inhibitor**.

**UNDERSTAND:** Each option is eliminated by a specific clause in the stem. **DPP-4 inhibitors and GLP-1 agonists carry a pancreatitis association**; **pioglitazone carries a bladder cancer signal** (plus weight gain and heart failure); **GLP-1 agonists are injectable**. **SGLT-2 inhibitors** clear every hurdle and additionally cause **weight loss** — desirable in obesity.

**MUST KNOW:**
- **Metformin** — first line; adverse effects **lactic acidosis** and **B12 deficiency**; hold before **iodinated contrast**, in **hypoxia, sepsis, decompensated heart failure and liver disease**. **Weight-neutral or weight-reducing, no hypoglycaemia.**
- **SGLT-2 inhibitors** — weight loss, blood pressure reduction, and proven **cardiovascular and renal benefit (and in heart failure regardless of diabetes)**; adverse effects **genital mycotic and urinary infections, euglycaemic DKA, volume depletion, Fournier's gangrene**.
- **GLP-1 agonists** — greatest weight loss, cardiovascular benefit, but injectable (except oral semaglutide), with pancreatitis and **medullary thyroid carcinoma/MEN2** contraindications.
- **Sulfonylureas** — hypoglycaemia and weight gain. **Pioglitazone** — fluid retention, heart failure, fractures, bladder cancer.
- Comorbidity-driven choice: **ASCVD or heart failure or CKD → SGLT-2i (± GLP-1 RA)**; **obesity → GLP-1 RA or SGLT-2i**; **cost the priority → sulfonylurea**.
- **Diabetic nephropathy:** strict glycaemic control **does** prevent and can reverse microalbuminuria; **microalbuminuria IS a marker of cardiovascular risk**; **protein restriction is not recommended** by the ADA; **successful pancreas transplantation can reverse early lesions**.

**EXAM CONNECTION:** These are elimination questions — every distractor is disqualified by one phrase in the history. Map each clause to a drug before choosing.

**REMEMBER:** *Read the comorbidities as a list of exclusions.*

`MB9265, MB9260, MB7128`

---

## 169. Restless Legs Syndrome

**PEARL:** RLS symptoms **begin or worsen with REST**, are **relieved by movement**, are **worse in the evening/night**, and involve an **urge to move with unpleasant sensations**. Symptoms worsening with *activity* argues against it.

**UNDERSTAND:** The disorder reflects **dopaminergic hypofunction** and **brain iron deficiency** (iron is the cofactor for tyrosine hydroxylase). The circadian worsening tracks the **evening trough in dopamine**; movement transiently relieves symptoms by generating competing sensory input.

**MUST KNOW:**
- Four essential (URGE) criteria: **U**rge to move the legs; **R**est brings it on; **G**ets better with movement; **E**vening/night worsening.
- **Always check ferritin — target > 75 µg/L** and replace iron first. Other secondary causes: **pregnancy, chronic kidney disease/dialysis, peripheral neuropathy, and drugs (antidepressants, antipsychotics, antihistamines)**.
- Modern first-line drug therapy is a **gabapentinoid (gabapentin enacarbil, pregabalin)** in preference to dopamine agonists, because agonists cause **augmentation** — symptoms starting earlier in the day and spreading to the arms, the major long-term complication.
- Dopamine agonists (**pramipexole, ropinirole, rotigotine**) remain options; **levodopa causes the most augmentation**.
...
## 170. ARDS — Haemodynamic Definition

**PEARL:** In ARDS the oedema is **non-cardiogenic**: **PCWP ≤ 18 mmHg with a NORMAL left atrial pressure**, alongside **reduced compliance, raised pulmonary artery pressure and severe hypoxaemia**.

**UNDERSTAND:** ARDS is **increased permeability** oedema — protein-rich fluid crosses a damaged alveolar–capillary membrane at normal filling pressures. That is precisely why the left atrial pressure must be **normal**: a raised one would mean hydrostatic (cardiogenic) oedema instead. The pulmonary *artery* pressure rises separately, from **hypoxic vasoconstriction and microthrombi**.

**MUST KNOW:**
- **Berlin definition:** onset within **1 week** of a known insult; **bilateral opacities** not fully explained by effusion/collapse/nodules; respiratory failure **not fully explained by cardiac failure or fluid overload**; and **PaO₂/FiO₂ with PEEP ≥ 5** — **mild 200–300, moderate 100–200, severe ≤ 100**.
...
## 171. Hairy Cell Leukaemia

**PEARL:** **Cladribine (2-CDA) is the drug of choice for hairy cell leukaemia** — a purine analogue with **curative** activity.

**UNDERSTAND:** Hairy cells lack **adenosine deaminase** activity relative to their purine load, so a **deoxyadenosine analogue accumulates** and triggers apoptosis even in these slowly dividing cells. That biochemical vulnerability is why a single short course produces durable complete remissions in a chronic leukaemia.

**MUST KNOW:**
- Presentation: **middle-aged man, massive splenomegaly WITHOUT lymphadenopathy, pancytopenia and monocytopenia**, recurrent infection (**atypical mycobacteria**).
...
## 172. Reading BT, PT and aPTT Together

**PEARL:** **Prolonged BT + normal PT + prolonged aPTT = von Willebrand disease.** **Normal BT + normal PT + prolonged aPTT = haemophilia A or B.**

**UNDERSTAND:** vWF has **two jobs**: it mediates **platelet adhesion to subendothelial collagen** (so its loss prolongs the bleeding time) and it **carries and protects factor VIII** in plasma (so its loss also drops factor VIII and prolongs the aPTT). Haemophilia touches only the second function — platelets work normally, so the bleeding time is normal.

**MUST KNOW:**
- **vWD is the commonest inherited bleeding disorder** and is usually **autosomal dominant** — hence affected fathers **and** sisters in the family history, unlike X-linked haemophilia. Presents with **mucocutaneous bleeding: epistaxis, menorrhagia, bleeding after dental extraction**.
- vWF is stored in endothelial **Weibel–Palade bodies** and platelet **α-granules**.
- Treatment: **desmopressin (DDAVP)** for type 1 (releases stored vWF), **vWF-containing factor VIII concentrate** for type 3 and type 2B; **tranexamic acid** as an adjunct.
- **Haemophilia** presents with **deep bleeding — haemarthrosis and muscle haematoma** in a **male child**, after trivial trauma. **A = factor VIII, B = factor IX (Christmas disease)**; both X-linked recessive.
- Test logic: **PT = extrinsic (factor VII)**; **aPTT = intrinsic (VIII, IX, XI, XII)**; **both prolonged = common pathway (II, V, X, fibrinogen)** or liver disease/DIC/warfarin overdose.
- **Mixing study**: corrects → factor deficiency; fails to correct → **inhibitor (e.g. lupus anticoagulant, acquired factor VIII inhibitor)**.

**EXAM CONNECTION:** Always identify the **bleeding pattern first** (mucosal vs deep), then match the three-test grid. Menorrhagia with a dominant family history is vWD; haemarthrosis in a boy is haemophilia.
...
**PEARL:** **Fever + neutrophilia + painful, oedematous red plaques on face, neck and upper limbs = Sweet syndrome** (acute febrile neutrophilic dermatosis). It is **NOT associated with aplastic anaemia** — the point being that it needs neutrophils to exist.

**UNDERSTAND:** The lesion *is* a **dense dermal infiltrate of neutrophils**. A marrow that cannot produce neutrophils therefore cannot produce the disease — which is exactly why **AML (and G-CSF therapy) are the classic associations** and aplastic anaemia is the odd one out.
...
## 174. Kartagener Syndrome

**PEARL:** The **triad is situs inversus + chronic sinusitis + bronchiectasis**. **Male infertility is typical but NOT part of the triad.**

**UNDERSTAND:** The defect is in the **dynein arms of cilia**. Ciliary beating does three things: it **clears airway mucus** (loss → sinusitis and bronchiectasis), it **determines left–right asymmetry at the embryonic node** (loss → situs randomised, so **50% get situs inversus**), and it **powers the sperm flagellum** (loss → immotile sperm). Infertility is real but affects only males, so it cannot be part of a universal triad.

**MUST KNOW:**
- **Primary ciliary dyskinesia (immotile cilia syndrome)** is **autosomal recessive**; Kartagener is the subset **with situs inversus** — hence only about **half** of PCD patients have Kartagener syndrome.
- **Female subfertility** also occurs (fallopian tube cilia) with increased **ectopic pregnancy** risk.
- Also causes **neonatal respiratory distress at term, chronic otitis media with hearing loss, nasal polyps**.
- Screening: **low nasal nitric oxide**; confirm with **electron microscopy of ciliary ultrastructure** and genetic testing.
...
## 175. Beck's Triad and the Trauma Chest

**PEARL:** **Hypotension + distended neck veins + muffled heart sounds = Beck's triad = cardiac tamponade** → **urgent pericardiocentesis**. **Breath sounds are the discriminator** from tension pneumothorax.

**UNDERSTAND:** Both conditions cause **obstructive shock with raised JVP**, so the classic triad cannot separate them. The difference is **where the pressure is**: tension pneumothorax has **air in the pleural space** — so **breath sounds are ABSENT with hyperresonance and tracheal deviation**. Tamponade has fluid in the pericardium and **normal, equal breath sounds**.

**MUST KNOW:**
- Fluid transfusion alone will not work in tamponade — the heart cannot fill until the pericardium is decompressed. **Decompress first, then resuscitate.**
- **Pericardiocentesis approaches:** subxiphoid (commonest, under echo guidance), parasternal, apical.
- Tension pneumothorax is a **clinical diagnosis** → immediate **needle decompression** then intercostal drain; do **not** wait for an X-ray.
- Other tamponade signs: **pulsus paradoxus > 10 mmHg**, raised JVP with **absent y descent**, electrical alternans and low voltage on ECG, Kussmaul's sign (more typical of constriction).
- **JVP y descent:** **ABSENT/attenuated in tamponade** (filling is impeded throughout diastole) but **DEEP in constrictive pericarditis, restrictive cardiomyopathy and tricuspid regurgitation** (early diastolic filling is rapid, then abruptly halted — the "dip and plateau"/square-root sign).

**EXAM CONNECTION:** Two related stems — the management question (**pericardial tap**) and the discrimination question (**breath sounds**). The y-descent question is the physiology version of the same topic.
...
## 176. Second-Degree AV Block — Mobitz I vs II

**PEARL:** **Mobitz I (Wenckebach): progressively lengthening PR, then a dropped beat.** **Mobitz II: CONSTANT PR interval with a sudden dropped beat.** Mobitz II needs a **pacemaker**.

**UNDERSTAND:** The site of block determines both the pattern and the danger. **Mobitz I is in the AV node**, where tissue shows **decremental conduction** — each impulse fatigues the node a little more until one fails, then it recovers. **Mobitz II is infranodal (His–Purkinje)**, where conduction is **all-or-none**: the fascicle either conducts or it does not, so the PR never varies and failure is **unpredictable**, with a real risk of sudden progression to complete heart block and asystole.

**MUST KNOW:**
- **Mobitz I** is often benign — young athletes, high vagal tone, inferior MI, drugs (**beta-blockers, digoxin, calcium channel blockers**). **Improves with atropine and exercise.**
- **Mobitz II** is always pathological — anterior MI, fibrosis of the conduction system (Lev/Lenègre disease). **Worsens with exercise and does not respond to atropine.** → **permanent pacemaker**.
- **First-degree block:** PR **> 200 ms**, every beat conducted.
- **Third-degree (complete) block:** **AV dissociation** — P waves and QRS marching independently, with a slower escape rhythm; **cannon a waves** in the JVP and a **variable-intensity S1**.
- **2:1 block** is unclassifiable as I or II from the surface ECG alone — a narrow QRS suggests nodal, a wide QRS infranodal.
...
## 177. Lupus Nephritis — Class IV and the Wire Loop

**PEARL:** **Wire-loop capillary thickening = subendothelial immune deposits = Class IV (diffuse) lupus nephritis** — the **most severe** class, involving **≥ 50% of glomeruli**.

**UNDERSTAND:** Where the immune complexes deposit determines both the histology and the clinical picture. **Subendothelial** deposits sit on the **blood side** of the basement membrane, in direct contact with the circulation — so they trigger **complement activation and an inflammatory, proliferative, nephritic** response. **Subepithelial** deposits (Class V) are shielded from circulating leucocytes, so they cause **proteinuria without inflammation** — a nephrotic picture.

**MUST KNOW:**
- **Class I** minimal mesangial (deposits only, normal light microscopy). **Class II** mesangial proliferative. **Class III** focal (< 50% glomeruli). **Class IV** diffuse (≥ 50%) — **commonest and most severe**, wire loops. **Class V** membranous — **nephrotic syndrome**. **Class VI** advanced sclerosing (≥ 90% sclerosed).
...
**PEARL:** **Chronic smoker + raised JVP + pitting oedema + tender hepatomegaly + ascites = right ventricular failure from COPD = cor pulmonale.**

**UNDERSTAND:** Chronic hypoxia causes **pulmonary vasoconstriction and vascular remodelling** → pulmonary hypertension → the thin-walled **right ventricle faces a pressure load it was never designed for**, dilates, and fails backwards into the systemic veins. Every sign in the stem is **venous congestion**, which is why the liver, legs and neck veins are involved and the lungs are not.
...
## 180. Localising Myocardial Infarction on ECG
...
**MUST KNOW:**
- **Anteroseptal — V1–V4** (LAD). **Anterolateral — I, aVL, V3–V6**. **Extensive anterior — I, aVL, V1–V6** (proximal LAD). **Lateral — I, aVL, V5, V6** (circumflex). **Inferior — II, III, aVF** (right coronary in ~80%). **Posterior — ST DEPRESSION with tall R in V1–V4** (reciprocal; confirm with V7–V9). **Right ventricular — V4R** (always check in inferior MI: avoid nitrates, give fluids).
- **New LBBB with ischaemic chest pain is an MI equivalent** — treat as STEMI. Recognise it: **QRS ≥ 120 ms, QS in V1, broad notched R in V6**. **WiLLiaM** (W in V1, M in V6 → LBBB); **MaRRoW** (M in V1, W in V6 → RBBB).
- Evolution: **hyperacute T waves → ST elevation (injury) → T inversion → pathological Q waves (late)**.
- **Ventricular tachycardia**: rate > 100 with **wide QRS ≥ 120 ms**, no visible P waves, AV dissociation, fusion and capture beats. Distinguish from **VF** (no discrete QRS complexes).
- **Ventricular bigeminy** = one PVC after every normal beat; **trigeminy** = PVC after two normals; **quadrigeminy** = after three. **Unifocal** PVCs share one morphology; **multifocal** have several.
...
## 181. Antibodies in Systemic Sclerosis and Overlap Syndromes
...
## 183. Screening for Diabetic Retinopathy

**PEARL:** **Type 2 diabetes — refer to ophthalmology AT DIAGNOSIS.** **Type 1 diabetes — screen 3–5 years after diagnosis.** Then **annually** in both.

**UNDERSTAND:** The difference reflects **when the disease actually started**. Type 2 diabetes is present **years before** it is diagnosed — retinopathy may already exist on day one. Type 1 has an **abrupt, symptomatic onset**, so the clock starts accurately at diagnosis and retinopathy takes years to develop.

**MUST KNOW:**
- Screening in **pregnancy**: each trimester (retinopathy can progress rapidly); pre-existing diabetes only — **not** gestational.
- **Rapid tightening of glycaemic control can transiently worsen retinopathy** — a recognised paradox.
- Modifiable risk factors: **glycaemic control, blood pressure, lipids, duration of disease, anaemia, pregnancy, nephropathy**.
- Grading: **non-proliferative (microaneurysms, dot-blot haemorrhages, hard exudates, cotton-wool spots, venous beading, IRMA)** → **proliferative (neovascularisation)** → complications (vitreous haemorrhage, tractional detachment, rubeosis).
- Treatment: **pan-retinal photocoagulation** for proliferative disease; **intravitreal anti-VEGF** for **diabetic macular oedema** (the commonest cause of visual loss in diabetes).
- Related nephropathy screening: **annual urine albumin:creatinine ratio** from diagnosis in type 2, and from 5 years in type 1 — the same logic.
...
## 184. Hypertension in a Diabetic Patient
...
**MUST KNOW:**
- Preferred agents in diabetes: **ACE inhibitor or ARB** (renoprotective, first choice **especially with albuminuria**), then **calcium channel blocker** or **thiazide-like diuretic**.
- **Do not combine ACE inhibitor with ARB** — increased hyperkalaemia and renal injury without benefit.
- Beta-blockers can **mask hypoglycaemic warning symptoms** and worsen glycaemic control — not first line in uncomplicated diabetic hypertension.
- Lifestyle targets: **BMI < 25**, **DASH-type diet**, **salt < 5–6 g/day**, alcohol limits, **30 min brisk aerobic activity most days**.
- **α-blockers (prazosin)** cause **first-dose orthostatic hypotension** — reserved for resistant hypertension or coexisting BPH, given at night.

**EXAM CONNECTION:** The word **dizziness** in the stem exists solely to eliminate prazosin. Read incidental symptoms as drug-exclusion clues.
...
## 185. Investigating Graves' Disease
...
## 186. Syndrome Z and the Metabolic Syndrome Criteria

**PEARL:** **Syndrome Z = obstructive sleep apnoea + metabolic syndrome.** **LDL is NOT one of the metabolic syndrome criteria** — the lipid criteria are **high triglycerides and low HDL**.

**UNDERSTAND:** Metabolic syndrome is defined around **insulin resistance and visceral adiposity**, whose lipid signature is the **atherogenic dyslipidaemia triad — high TG, low HDL, small dense LDL particles**. Total LDL *concentration* is often normal; it is the particle quality that changes. That is why LDL was never a criterion.

**MUST KNOW:**
- **NCEP ATP III — any 3 of 5:** **waist circumference > 102 cm (M) / > 88 cm (F)**; **triglycerides ≥ 150 mg/dL**; **HDL < 40 (M) / < 50 (F)**; **BP ≥ 130/85**; **fasting glucose ≥ 100 mg/dL** (or specific treatment for any of these).
- Asian cut-offs for waist circumference are lower (**≥ 90 cm men, ≥ 80 cm women** in IDF criteria), and IDF makes **central obesity mandatory**.
- **Syndrome X** = the metabolic syndrome itself (also, confusingly, "cardiac syndrome X" = angina with normal coronaries). **Syndrome Z** adds **OSA**.
- OSA and metabolic syndrome are bidirectional — **intermittent hypoxia worsens insulin resistance**, and obesity worsens apnoea. **CPAP** improves blood pressure.
- Screening for OSA: **STOP-BANG** questionnaire; diagnosis by **polysomnography (AHI ≥ 5 with symptoms)**.

**EXAM CONNECTION:** "Not a component" → **LDL > 100**. Every real criterion has a specific number; learn the five.

**REMEMBER:** *Metabolic syndrome counts triglycerides and HDL, never LDL.*

`MC5298`

---

## 187. Upper vs Lower Motor Neuron Signs

**PEARL:** **UMN lesion → exaggerated (brisk) reflexes**, spasticity, hypertonia, **extensor plantar (Babinski)**, no wasting. **LMN lesion → depressed or absent reflexes**, flaccidity, **wasting and fasciculations**.

**UNDERSTAND:** The corticospinal tract is normally **inhibitory to the spinal reflex arc**. Remove that descending inhibition and the arc runs unrestrained — hence brisk reflexes and clonus. An LMN lesion **breaks the arc itself**, so no reflex is possible, and the muscle loses its trophic innervation and wastes.

**MUST KNOW:**
- **Spinal shock:** immediately after an acute UMN (cord) lesion, reflexes are **absent** for days to weeks before hyperreflexia appears — a classic exam trap.
- **UMN weakness pattern:** pyramidal distribution — **upper limb extensors and lower limb flexors are weaker**; **clasp-knife** spasticity.
- **LMN:** includes anterior horn cell, root (**radiculopathy**), plexus, and peripheral nerve (**polyneuropathy**) — all give **depressed** reflexes.
...
**PEARL:**
- **LOUD S1:** early rheumatic **mitral stenosis** (pliable valve), **short PR interval**, hyperdynamic circulation (fever, anaemia, thyrotoxicosis, exercise).
- **SOFT S1:** **calcified/rigid leaflets**, **long PR interval**, associated **aortic regurgitation**, and **LV dysfunction**.

**UNDERSTAND:** S1 loudness = how **wide open** the mitral leaflets are when the ventricle contracts. A short PR gives the leaflets no time to drift shut → they slam from wide open → loud. A long PR lets them float almost closed → quiet. A calcified valve simply cannot move.

**MUST KNOW:** **Variable S1** = atrial fibrillation and complete heart block ("cannon" variability).

**REMEMBER:** *Mobile valve + short PR = loud S1; stiff valve + long PR = soft S1.*

---

## 189. Tricuspid Stenosis — ECG
...
## 190. P2Y12 Blocker After Stenting

**PEARL:** A **P2Y12 receptor blocker must be added (to aspirin) to prevent stent thrombosis** — dual antiplatelet therapy.

**MUST KNOW:** Options: **clopidogrel, prasugrel, ticagrelor**. Typical duration: **12 months after ACS**, ~6 months after elective PCI with a drug-eluting stent (shorter if bleeding risk is high). Premature discontinuation is the strongest predictor of **acute stent thrombosis**, which presents as sudden STEMI with high mortality.

**REMEMBER:** *Stent = two antiplatelets; stopping early = thrombosis.*

---

## 191. Malignant MCA Infarct — Decompressive Surgery

**PEARL:** Decompressive hemicraniectomy is recommended for patients **under 60 years** who **deteriorate within ≤48 hours** despite medical therapy.

**MUST KNOW:** Predictors of severe cerebral oedema: **hypodensity involving ≥1/3 of the MCA territory on CT within 6 hours**, **early midline shift**, and **DWI lesion volume ≥80 mL within 6 hours** on MRI. Surgery reduces mortality substantially (though survivors carry disability); benefit falls off above age 60 and after 48 hours.

**REMEMBER:** *Under 60, within 48 hours, ≥1/3 MCA territory → decompress.*

---

## 192. Substantia Nigra

**PEARL:** On a transverse section of the brainstem at the level of the **superior colliculus**, the **substantia nigra** is seen; its degeneration causes **Parkinson's disease (paralysis agitans)**.

**MUST KNOW:** Loss of **dopaminergic neurons in the pars compacta** (depigmentation), with **Lewy bodies (α-synuclein)**. Symptoms appear only after **~60–80% of neurons are lost**. Cardinal features: bradykinesia + rest tremor (4–6 Hz, pill-rolling) + rigidity (cogwheel) + postural instability.

**REMEMBER:** *Superior colliculus level = midbrain = substantia nigra = Parkinson's.*

---

## 193. Central Cord Syndrome
...
**REMEMBER:** *Weak hands, spastic legs = central cord.*

---

## 194. Myasthenia Gravis — Reflexes

**PEARL:** **Absent deep tendon reflexes are NOT a feature of myasthenia gravis** — reflexes are **preserved** (they may even be brisk relative to the weakness).

**EXAM CONNECTION:** **Lambert–Eaton myasthenic syndrome** *does* have **absent/reduced reflexes** that **improve after exercise (post-tetanic potentiation)**, along with autonomic features and small-cell lung cancer. MG worsens with activity; LEMS improves.

**REMEMBER:** *MG: reflexes present, weakness with use. LEMS: reflexes absent, strength with use.*
...
**REMEMBER:** *Dancing eyes in a child → hunt for neuroblastoma.*

---

## 196. Whipple Disease — CNS

**PEARL:** The commonest CNS manifestation of Whipple disease is **progressive dementia**.

**MUST KNOW:** *Tropheryma whipplei*; classic triad of **arthralgia, diarrhoea/malabsorption and weight loss**. Pathognomonic CNS sign: **oculomasticatory myorhythmia**. Diagnosis: duodenal biopsy with **PAS-positive, diastase-resistant foamy macrophages**. Treatment: **ceftriaxone induction then long-term cotrimoxazole (≥1 year)** — cotrimoxazole is chosen because it **crosses the blood–brain barrier**.

**REMEMBER:** *Whipple = PAS-positive macrophages; the brain gets dementia, and needs a year of cotrimoxazole.*

---

## 197. Bedside Cognitive Testing

**PEARL:**
- **Digit span** is the best test for **immediate memory** — **forward up to 7 digits, with 2 skips allowed**.
- **Serial 100−7 subtraction** is the best test for **concentration/attention**.

**MUST KNOW:** Normal digit span = **7 ± 2 forward**, 5 backward. Immediate memory ≈ working memory (seconds); recent memory = events of the day (**3-object recall at 5 minutes**); remote memory = past life events.

**REMEMBER:** *Digits = memory; serial sevens = concentration.*

---

## 198. Akinetic Mutism

**PEARL:** **Akinetic mutism is a feature of stupor** — the patient is awake with eyes open but does not move or speak.

**MUST KNOW:** Caused by bilateral **frontal (anterior cingulate)** lesions or damage to the ascending reticular activating system/thalamus. Contrast **locked-in syndrome** (ventral pons — quadriplegia and anarthria but **fully conscious, communicating by vertical eye movements**) and **persistent vegetative state** (sleep-wake cycles, no awareness).

**REMEMBER:** *Awake but silent and still = akinetic mutism; check the frontal lobes.*

---

## 199. Acromegaly — Best Screening Test

**PEARL:** **Serum IGF-1** is the best screening test.

**UNDERSTAND:** GH is secreted in **pulses** and has a short half-life, so a random GH level is meaningless; **IGF-1 has a long half-life and reflects integrated 24-hour GH secretion**.

**MUST KNOW:** **Confirmatory test = failure of GH suppression below 1 ng/mL after a 75 g oral glucose load**. Then localize with **MRI pituitary**. Treatment: **transsphenoidal surgery** first-line; then octreotide/lanreotide, pegvisomant, cabergoline.

**REMEMBER:** *Screen with IGF-1, confirm with the glucose suppression test.*

---

## 200. Conn Syndrome and the Aldosterone Escape

**PEARL:** In primary hyperaldosteronism (aldosterone-producing tumour of the **zona glomerulosa**), **true hypernatraemia is uncommon** — because of the **escape phenomenon**.

**UNDERSTAND:** Aldosterone retains sodium with equivalent water, so osmolality stays normal. As the extracellular volume expands, **pressure natriuresis and natriuretic peptides (ANP/BNP)** kick in, dumping sodium in the urine. A new steady state is reached and further retention stops — hence **hypertension without oedema**, and near-normal sodium.

**MUST KNOW:** The classic picture is **hypertension + hypokalaemia + metabolic alkalosis** with a **high aldosterone : low renin ratio**. **Potassium escape does NOT occur** — that is why hypokalaemia persists while hypernatraemia does not.

**REMEMBER:** *Sodium escapes, potassium doesn't — hypertension without oedema, with low K⁺.*

---

## 201. Metformin

**PEARL:** Metformin **reduces hepatic glucose production** and improves peripheral glucose utilization. Its specific adverse effects are **lactic acidosis** and **vitamin B12 deficiency**.

**MUST KNOW:**
- **Contraindicated/stop when: eGFR <30 mL/min, acidosis, unstable CCF, liver disease, severe hypoxaemia, and before contrast studies.**
- Older creatinine-based cut-offs: stop if **creatinine >1.5 mg/dL in men, >1.4 mg/dL in women** (current guidance uses eGFR).
- It does **not** cause hypoglycaemia alone and is **weight-neutral/weight-reducing** — hence first-line in type 2 diabetes.
- Check **B12 annually** with long-term use (can present as neuropathy mistaken for diabetic neuropathy).

**REMEMBER:** *Metformin: no sugar crash, but watch lactate, B12, kidneys and contrast dye.*

---

## 202. Calcium and Magnesium Disorders

**PEARL:**
- **Hypocalcaemia:** paraesthesia, **tetany**, seizures, **prolonged QT**.
- **Hypercalcaemia:** fatigue, depression, confusion, anorexia, nausea/vomiting, **constipation**, polyuria (reversible renal tubular defect), **short QT**, arrhythmias.
- **Hypomagnesaemia also causes tetany.**
...
## 203. Potassium Disorders — Weakness

**PEARL:** **Both hypokalaemia and hyperkalaemia present with ASCENDING muscle weakness.**

**MUST KNOW:**
- Hypokalaemia: weakness, cramps, ileus, **U waves, ST depression, flat T**, arrhythmia; potentiates digoxin toxicity.
- Hyperkalaemia: **tall peaked T waves → wide QRS → sine wave → arrest**; treat with **calcium gluconate (membrane stabilization) first**, then insulin-dextrose/salbutamol (shift), then resins/dialysis (removal).
- Ascending weakness with sensory sparing also raises **Guillain–Barré** — check potassium before assuming it.

**REMEMBER:** *Too much or too little potassium — the legs go first.*

---

## 204. Anion Gap in Acidosis

**PEARL:**
- **HIGH anion gap:** **salicylate poisoning**, methanol, ethylene glycol, lactic acidosis, ketoacidosis, uraemia (**MUDPILES**).
- **NORMAL anion gap (hyperchloraemic):** **proximal (type 2) RTA**, **pancreatitis (pancreatic fistula)**, **severe diarrhoea**, acetazolamide, ureteric diversion.

**UNDERSTAND:** Normal gap = you **lost bicarbonate** (gut or kidney) and chloride replaced it. High gap = you **added an acid** whose anion is unmeasured.

**MUST KNOW:** **Salicylate poisoning classically gives a mixed picture — respiratory alkalosis (direct respiratory centre stimulation) PLUS high-anion-gap metabolic acidosis.** Anion gap = Na⁺ − (Cl⁻ + HCO3⁻), normal 8–12.

**REMEMBER:** *Lost bicarbonate = normal gap; gained acid = high gap.*

---

## 205. Corrected Reticulocyte Count

**PEARL:**
**CRC = Reticulocyte % × (Actual Hb / Normal Hb)** — or equivalently **× (Actual PCV / Normal PCV)**.

**MUST KNOW:**
- Normal Hb taken as 15 g/dL; normal PCV as 45%.
- **CRC >2–3% = adequate marrow response** → haemolysis or blood loss. **CRC <2% = hypoproliferative** marrow → deficiency, aplasia, marrow infiltration, CKD.
- A further correction (**reticulocyte production index**, dividing by the maturation factor 1–2.5) is used in severe anaemia because reticulocytes are released early and live longer in circulation.

**REMEMBER:** *Correct the retic for the anaemia; >2% means the marrow is working.*

---

## 206. Desmopressin in Haemophilia A

**PEARL:** **Intranasal (or IV) desmopressin raises factor VIII levels** — effective for **mild to moderate** bleeding in haemophilia A.

**UNDERSTAND:** DDAVP releases stored **factor VIII and vWF from Weibel–Palade bodies of endothelium** — it works only if there is a store to release, so it is **useless in severe haemophilia A** (and in **haemophilia B**, where factor IX is not stored this way).

**MUST KNOW:** Also used in **von Willebrand disease type 1** (contraindicated in **type 2B** — causes thrombocytopenia). Side effects: **hyponatraemia/water retention** and tachyphylaxis with repeated doses. Severe haemophilia A needs **factor VIII concentrate** (or emicizumab).

**REMEMBER:** *DDAVP empties the endothelial store — no store, no effect.*

---

## 207. Hydroxyurea in Beta-Thalassaemia Intermedia

**PEARL:** Hydroxyurea **induces HbF synthesis**, reducing anaemia, decreasing ineffective erythropoiesis, and improving oxygen-carrying capacity in **β-thalassaemia intermedia**.

**UNDERSTAND:** HbF (α2γ2) needs no β chains — so raising it directly compensates for absent β-globin and **rebalances the α:β chain ratio**, which is what actually kills erythroid precursors.

**MUST KNOW:** Same drug, same mechanism, is the mainstay in **sickle cell disease**. It reduces (but does not abolish) transfusion requirements; monitor for **myelosuppression**.

**REMEMBER:** *Hydroxyurea turns the gamma gene back on — helps both thalassaemia intermedia and sickle cell.*

---

## 208. Platelet Storage

**PEARL:** Platelets are stored **in plasma for 5 days at ~22°C (20–24°C) with continuous gentle agitation**.

**UNDERSTAND:** They are kept at room temperature because refrigeration causes clustering and rapid clearance — which is exactly why the shelf life is short: **room temperature allows bacterial growth**, and **bacterial contamination is the leading infectious risk of platelet transfusion**.

**MUST KNOW:** Contrast: **packed red cells — 1–6°C for 35–42 days**; **fresh frozen plasma — −30°C for 1 year**; **cryoprecipitate — −30°C for 1 year**.

**REMEMBER:** *Platelets: warm, shaken, and only 5 days.*

---

## 209. APL Differentiation Syndrome

**PEARL:** An adverse effect of **all-trans retinoic acid (ATRA)** in acute promyelocytic leukaemia — managed with **glucocorticoids (dexamethasone)**.

**MUST KNOW:**
- Features: **fever, dyspnoea, hypoxia, pulmonary infiltrates, weight gain, pleural/pericardial effusion, hypotension, AKI** — usually in the first 1–3 weeks; caused by a cytokine storm from differentiating promyelocytes.
- Treatment: **dexamethasone 10 mg BD immediately** (do not wait for confirmation); withhold ATRA only in severe cases.
- APL = **t(15;17), PML-RARA**; the other emergency is **DIC**, which is the main cause of early death. ATRA is started on clinical suspicion, before cytogenetics return.

**REMEMBER:** *ATRA + fever + hypoxia = differentiation syndrome → dexamethasone now.*

---

## 210. Peutz–Jeghers Syndrome

**PEARL:** **Pancreatic cancer has the highest association** with Peutz–Jeghers syndrome.

**MUST KNOW:** Autosomal dominant, **STK11/LKB1** mutation; **mucocutaneous melanotic pigmentation (perioral, buccal, digits) + hamartomatous GI polyps**, which cause **intussusception and bleeding** rather than malignant transformation themselves. Massive overall cancer risk (~90% lifetime): pancreas, breast, colon, stomach, and the distinctive **SCTAT of the ovary** and **Sertoli cell tumour of the testis**.

**REMEMBER:** *Perioral freckles + hamartomas = PJS; the pancreas carries the highest risk.*

---

## 211. Meningococcal Meningitis — Empirical Treatment

**PEARL:** **Ceftriaxone or cefotaxime** is the drug of choice for empirical treatment.

**MUST KNOW:** Add **vancomycin** where penicillin-resistant pneumococcus is prevalent, and **ampicillin** for *Listeria* in neonates, >50 years, or the immunocompromised. **Dexamethasone before/with the first antibiotic dose** improves outcome in pneumococcal meningitis. Chemoprophylaxis of contacts: **rifampicin/ciprofloxacin/ceftriaxone**.

**REMEMBER:** *Third-generation cephalosporin empirically; add vancomycin and steroid for pneumococcus.*

---

## 212. Pneumatoceles in an Infant

**PEARL:** Pneumonia with **pneumatoceles** (thin-walled air-filled cysts) in an infant → ***Staphylococcus aureus***.

**MUST KNOW:** Staph pneumonia in children is rapidly progressive with **empyema, pyopneumothorax and abscesses**; pneumatoceles usually **resolve spontaneously** over weeks-months and need no drainage. Consider **MRSA cover (vancomycin/clindamycin)**. Other causes of cavitating/cystic lung lesions: Klebsiella, TB, *Pneumocystis*.

**REMEMBER:** *Infant + pneumatocele = Staph aureus; leave the cysts alone.*

---

## 213. Burkholderia cepacia Complex
...
## 214. Epigenetic Modification in Autoimmunity

**PEARL:** Epigenetic modifications can **suppress autoreactive T-cell proliferation, reduce B-cell activation and autoantibody production, and downregulate pro-inflammatory cytokines (IL-6, TNF-α)** — thereby limiting inflammation and tissue damage.

**MUST KNOW:** Mechanisms: **DNA methylation, histone acetylation/deacetylation (HDAC), and microRNAs** — heritable changes in expression **without altering the DNA sequence**. **DNA hypomethylation of T cells is implicated in SLE**, and drugs such as **hydralazine and procainamide are DNA methylation inhibitors** — the link to **drug-induced lupus**.

**REMEMBER:** *Epigenetics = expression changed, sequence unchanged; hypomethylated T cells drive lupus.*

---

## 215. Spiral (Helical) CT

**PEARL:** Spiral CT is made possible by **slip-ring technology**.

**UNDERSTAND:** The slip ring transfers power and data through sliding electrical contacts instead of cables, so the gantry can **rotate continuously** while the table moves — producing a continuous volumetric spiral of data rather than one slice at a time.

**MUST KNOW:** Advantages: fast (single breath-hold), volumetric data allowing **multiplanar and 3D reconstruction**, no interslice gaps, optimal contrast timing (**CT angiography, pulmonary embolism protocols**).

**REMEMBER:** *No slip ring, no spiral.*

---

*Cross-references: digit span and serial sevens also appear as mental status examination items in the Psychiatry file; myasthenia drug list, metformin details and hydroxyurea also appear in the Pharmacology file; hydroxyurea and platelet storage overlap with Haematology.*

## 216. MDS with Isolated del(5q)

**PEARL:** A distinct subtype of myelodysplastic syndrome with a **relatively good prognosis**; **treatment of choice = LENALIDOMIDE**.
...
**REMEMBER:** *5q− = old lady, big red cells, high platelets, funny megakaryocytes → lenalidomide.*

---

## 217. Targeted Agents — Quick Grid

**PEARL:**
- **Vemurafenib** → **BRAF inhibitor** (BRAF V600E **melanoma**)
- **Alemtuzumab** → **anti-CD52** monoclonal antibody
- **Lonafarnib** → **farnesyl transferase inhibitor**
- **Vorinostat** → **HDAC (histone deacetylase) inhibitor** — used in **cutaneous T-cell lymphoma**, including **mycosis fungoides and Sézary syndrome**

**MUST KNOW:**
- **Alemtuzumab** (CD52 is on both B and T lymphocytes) is used in **CLL and in multiple sclerosis**, causing profound, prolonged lymphopenia → **opportunistic infection (CMV reactivation)**.
- **Lonafarnib** is approved for **Hutchinson–Gilford progeria** — it blocks farnesylation of the abnormal **progerin**.
- **Vemurafenib must not be used in BRAF wild-type tumours** (paradoxical MAPK activation); it causes **cutaneous squamous cell carcinomas/keratoacanthomas** — reduced by adding a MEK inhibitor.
- Other CTCL options: bexarotene, romidepsin (also an HDAC inhibitor), PUVA, mogamulizumab.

**REMEMBER:** *Vemurafenib–BRAF, Alemtuzumab–CD52, Lonafarnib–farnesyl (progeria), Vorinostat–HDAC (CTCL).*

---

## 218. Emperipolesis

**PEARL:** **Intact lymphocytes or other cells lying WITHIN the cytoplasm of a histiocyte/plasma cell — without being destroyed.** Classically seen in:
- **Rosai–Dorfman disease** — the **hallmark** feature
- **Autoimmune hepatitis** — plasma cells with emperipolesis
- **Hodgkin lymphoma** — occasionally in Reed–Sternberg cells

**UNDERSTAND:** The distinction from **phagocytosis** is the whole point — in emperipolesis the engulfed cell is **alive, intact and can walk out again**; in phagocytosis it is digested.

**MUST KNOW:** Rosai–Dorfman disease = **sinus histiocytosis with massive lymphadenopathy** — painless bilateral cervical nodes in a young person, with **S100-positive, CD1a-NEGATIVE** histiocytes (contrast **Langerhans cell histiocytosis: S100+ AND CD1a+/langerin+**).

**REMEMBER:** *Cells alive inside a histiocyte = emperipolesis = Rosai–Dorfman (S100+, CD1a−).*

---

## 219. Langerhans Cell Histiocytosis — Classic Presentation

**PEARL:** **Seborrhoeic dermatitis-like rash + "punched-out" lytic skull lesions** are the classic LCH combination.

**MUST KNOW:**
- The rash is **scaly, crusted, petechial, in the scalp and flexures** — often mistaken for cradle cap or nappy rash that "won't clear".
- Skeletal signs: punched-out skull lesions **without a sclerotic rim**, **beveled edge / hole-within-a-hole**, **button sequestrum**, **floating teeth** (loss of lamina dura), and **vertebra plana** (LCH is the commonest cause in children).
- **Diabetes insipidus** from pituitary stalk involvement is the commonest endocrine complication.
- Diagnosis: **Birbeck granules** on EM; **CD1a, S100 and langerin (CD207) positive**. Many carry the **BRAF V600E** mutation — hence a role for vemurafenib.

**REMEMBER:** *Refractory "cradle cap" + punched-out skull holes = LCH; CD1a positive.*

---

## 220. B-cell Acute Lymphoblastic Leukaemia — Markers

**PEARL:** Blasts expressing **CD19, CD20 and CD79a** are of **B-cell lineage** → **B-ALL**.

**MUST KNOW:**
- **B-lineage:** CD19, CD20, CD79a, CD22, **CD10 (CALLA — good prognosis)**, TdT.
- **T-lineage:** CD3 (most specific), CD2, CD5, CD7 — T-ALL classically presents in an **adolescent boy with a mediastinal mass**.
- **TdT is positive in both** and marks a **lymphoblast** (absent in AML); **myeloperoxidase and Auer rods** indicate AML.
- Prognosis in B-ALL: **hyperdiploidy and t(12;21) are good; t(9;22) (Philadelphia) and hypodiploidy are poor**.

**REMEMBER:** *CD19/20/79a = B; CD3 = T; TdT = lymphoblast either way.*

---

## 221. Childhood ALL — Presentation

**PEARL:** Presents with **bone pain, pallor, fever, bruising and hepatosplenomegaly** from marrow infiltration by lymphoblasts.

**MUST KNOW:**
- **Bone pain and a limp/refusal to walk** are highly characteristic in children — ALL is a classic mimic of **juvenile idiopathic arthritis** and "growing pains". The clue is **cytopenias with bone pain**.
- Peak age **2–5 years**; the commonest childhood malignancy, with **>85% cure**.
- **CNS and testes are sanctuary sites** → intrathecal prophylaxis is mandatory.
- Watch for **tumour lysis syndrome** on starting therapy (hydration ± rasburicase).

**REMEMBER:** *Child + bone pain + pancytopenia = ALL until the marrow says otherwise.*

---

## 222. Pseudo-Gaucher Cells

**PEARL:** A **large macrophage engorged with ingested haematopoietic cells** (a **pseudo-Gaucher / sea-blue histiocyte-like** appearance) is classically associated with **high cell turnover states — especially chronic myeloid leukaemia**.
...
*Cross-references: related haematology pearls sit in other files — **corrected reticulocyte count, platelet storage, desmopressin in haemophilia A, hydroxyurea in thalassaemia intermedia, and APL differentiation syndrome** sit elsewhere in this file; **G6PD and pyruvate kinase deficiency** in **Biochemistry.md**; **sickle cell drugs, filgrastim and checkpoint inhibitors** in **Pharmacology.md**.*

---

## 223. Alzheimer Disease — Genes and Chromosomes

**PEARL:**
- **APP (amyloid precursor protein) → chromosome 21** — explains **early Alzheimer disease in Down syndrome (trisomy 21)**
- **Presenilin-1 (PSEN1) → chromosome 14** — the **commonest cause of early-onset familial** Alzheimer's
- **Presenilin-2 (PSEN2) → chromosome 1**
- **APOE ε4 → chromosome 19** — the **strongest genetic risk factor for LATE-onset** disease
- **LRP1 → chromosome 12**

**UNDERSTAND:** Down syndrome patients carry **three copies of APP** — a lifelong 50% excess of amyloid precursor — so essentially all develop Alzheimer pathology by their 40s.

**MUST KNOW:**
- **APP, PSEN1 and PSEN2 are autosomal dominant and CAUSAL** (early onset, <65 years, rare); **APOE ε4 is only a RISK allele** — inheriting it does not guarantee disease, and **APOE ε2 is protective**.
- Pathology: **extracellular amyloid (Aβ42) plaques** + **intracellular neurofibrillary tangles of hyperphosphorylated tau**; earliest atrophy in the **hippocampus/entorhinal cortex**.
- CSF: **low Aβ42, high tau/phospho-tau**.

**REMEMBER:** *21-14-1-19: APP, PSEN1, PSEN2, APOE. Causal genes are early; APOE is late and only a risk.*

---

## 224. Pick Bodies

**PEARL:** **Pick bodies = intracellular inclusions of hyperphosphorylated TAU protein.**

**MUST KNOW:** They are **round, silver-staining (argyrophilic), tau-positive** cytoplasmic inclusions of **Pick disease**, a **frontotemporal dementia** — which presents with **personality change, disinhibition, apathy and language decline BEFORE memory loss** (the reverse of Alzheimer's), with **frontal and temporal "knife-edge" atrophy**. Other tauopathies: **progressive supranuclear palsy, corticobasal degeneration**, and the tangles of Alzheimer's. Contrast **Lewy bodies = α-synuclein**.

**REMEMBER:** *Pick body = tau; Lewy body = α-synuclein. FTD changes the personality first.*

---

## 225. Binswanger Disease vs Normal Pressure Hydrocephalus

**PEARL:**
- **Binswanger disease:** **hypertension + UMN signs + extensive white-matter lesions** (subcortical arteriosclerotic encephalopathy).
- **Normal pressure hydrocephalus:** the **classic triad is present, but with ventriculomegaly WITHOUT marked white-matter ischaemia**.

**MUST KNOW:**
- NPH triad — **"wet, wobbly, wacky"**: **gait apraxia (magnetic gait, earliest and most responsive), urinary incontinence, and dementia**.
- NPH: **normal CSF opening pressure**, ventricles enlarged **out of proportion to sulcal atrophy**; a positive **large-volume tap test** predicts benefit from a **ventriculoperitoneal shunt** — one of the few *reversible* dementias.
- Binswanger's is a **vascular dementia** — treat the vascular risk factors; the deficits are stepwise and not shunt-responsive.

**REMEMBER:** *Same triad — white matter disease = Binswanger (treat the BP); big ventricles alone = NPH (shunt it).*

---

## 226. Amitriptyline in Tension-Type Headache

**PEARL:** Amitriptyline is used **only for CHRONIC tension-type headache** — defined as **≥15 days per month for >3 months** — **not** for infrequent or episodic attacks.

**UNDERSTAND:** It is a **preventive (prophylactic)** drug requiring weeks to work, so it is justified only when headaches are frequent enough for prevention to be worthwhile. Episodic attacks are treated **acutely** with simple analgesics (paracetamol/NSAIDs).

**MUST KNOW:**
- Amitriptyline works here **independently of its antidepressant effect** (low dose, at night); it also helps neuropathic pain and migraine prophylaxis.
- ADRs limiting use: **anticholinergic effects (dry mouth, urinary retention, constipation), sedation, weight gain, postural hypotension**, and **cardiotoxicity in overdose**.
- Beware **medication-overuse headache** from frequent analgesic use — a common reason chronic headache fails to improve.

**REMEMBER:** *15 days a month for 3 months = chronic = prophylaxis (amitriptyline). Otherwise just treat the attack.*

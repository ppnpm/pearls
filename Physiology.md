# Physiology — Bookmark-Derived Pearls
...
**PEARL:** ***THE ABSOLUTE REFRACTORY PERIOD IS CAUSED BY CLOSURE OF THE INACTIVATION GATE OF THE VOLTAGE-GATED SODIUM CHANNEL*** — during it, **NO stimulus of any strength can excite the membrane.** The channel has ***THREE states, not two: CLOSED (resting — activation gate shut, inactivation gate open), OPEN (both open, for ~0.5–1 ms), and INACTIVATED (activation gate open but INACTIVATION GATE SHUT).*** Recovery requires **REPOLARISATION to reset the inactivation gate.**

**UNDERSTAND:** Two gates with different speeds explain the whole action potential. **Depolarisation opens the ACTIVATION gate FAST and closes the INACTIVATION gate SLOWLY** — so sodium enters for only a moment. This single design feature produces: **the self-terminating spike, the ABSOLUTE refractory period (all channels inactivated), the RELATIVE refractory period (some channels recovered, but potassium efflux is still hyperpolarising, so a STRONGER stimulus is needed), the ALL-OR-NONE law, and ONE-WAY PROPAGATION** — the wave cannot travel backwards into tissue that is still refractory.

**MUST KNOW:**
- **The refractory period sets the ***MAXIMUM FIRING FREQUENCY*** of a neurone and prevents tetanic contraction of cardiac muscle — ***CARDIAC MUSCLE HAS A LONG ABSOLUTE REFRACTORY PERIOD (from the plateau/calcium phase) THAT LASTS ALMOST THE WHOLE CONTRACTION, WHICH IS WHY THE HEART CANNOT BE TETANISED.***
...
## 2. The Sarcomere and the Cross-Bridge Cycle
...
**MUST KNOW:**
- **Clearance concepts: ***INULIN clearance = GFR (freely filtered, neither secreted nor reabsorbed); PAH clearance = EFFECTIVE RENAL PLASMA FLOW (completely cleared in one pass at low concentrations); CREATININE slightly OVERESTIMATES GFR (small tubular secretion).*** ***FILTRATION FRACTION = GFR/RPF ≈ 20%.***
- ***EFFERENT ARTERIOLAR CONSTRICTION (e.g. angiotensin II): MODERATE constriction RAISES glomerular hydrostatic pressure and GFR; SEVERE constriction REDUCES RENAL BLOOD FLOW so much that plasma protein concentration and therefore GLOMERULAR ONCOTIC PRESSURE RISE — and GFR then FALLS.*** In every case, efferent constriction ***INCREASES (never decreases) oncotic pressure*** and **raises filtration fraction** — the physiology behind the fall in GFR when ACE inhibitors are given in bilateral renal artery stenosis.
- **AFFERENT constriction reduces both renal blood flow and GFR; afferent DILATATION (prostaglandins) increases both — which is why NSAIDs are dangerous in prerenal states.**
- **Autoregulation of GFR (between mean pressures of ~80 and 180 mmHg) is by the ***MYOGENIC RESPONSE and TUBULOGLOMERULAR FEEDBACK*** — the macula densa sensing distal NaCl and adjusting afferent tone via adenosine.
- ***GLYCOSURIA WITH A NORMAL BLOOD GLUCOSE means a LOW renal threshold — pregnancy, SGLT2 inhibitors or FANCONI SYNDROME (generalised proximal tubular failure: glycosuria, aminoaciduria, phosphaturia, bicarbonaturia and uricosuria).***
...
**EXAM CONNECTION:** Baroreceptor afferents go to → **nucleus tractus solitarius**; Bezold–Jarisch reflex → **serotonin**; immediate event on standing → **decreased baroreceptor firing**; raised pulse pressure in the elderly → **decreased arterial compliance**.
...
**EXAM CONNECTION:** Feed-forward control is used in → **temperature regulation**; infinite feedback gain → **blood volume control by the kidney**.
...
## 6. Cardiac Muscle Junctions and Ultrastructure

**PEARL:** **The INTERCALATED DISC contains ***FASCIA ADHERENS (the analogue of zonula adherens — anchoring actin between cells), MACULA ADHERENS/DESMOSOMES (anchoring intermediate filaments) and GAP JUNCTIONS (electrical coupling) — but NO ZONULA OCCLUDENS (tight junction).***

**UNDERSTAND:** The absence of tight junctions and the presence of gap junctions is what makes the heart a ***FUNCTIONAL SYNCYTIUM***: connexon channels give a low-resistance path so that an action potential spreads from cell to cell without a synapse, and the atria and ventricles each contract as a unit. Tight junctions, by contrast, exist to create **impermeable barriers between compartments** (blood–brain barrier, gut epithelium, blood–testis barrier) — a function the myocardium has no use for.

**MUST KNOW:**
- **Junction types and their molecules: ***TIGHT (zonula occludens — claudins and occludins); ADHERENS (cadherins, linked to actin); DESMOSOME (macula adherens — desmoglein/desmocollin, linked to keratin); HEMIDESMOSOME (INTEGRINS attaching cells to BASEMENT MEMBRANE); GAP JUNCTION (connexins forming connexons).***
...
## 7. Leptin and the Onset of Puberty
...
## 8. Erythropoiesis and Haemoglobin Switching
...
- **The ***HALDANE EFFECT*** is the converse: deoxygenated haemoglobin carries more CO2, which is how CO2 is loaded in the tissues and released in the lungs.

**EXAM CONNECTION:** Earliest stage at which erythropoietin acts → **burst-forming units**; gamma-chain curve → **falls to undetectable by ~6 months**.
...
**PEARL:** ***INTEGRINS BIND FIBRONECTIN*** (and laminin and collagen) — they are transmembrane heterodimers that link the ***EXTRACELLULAR MATRIX to the INTRACELLULAR CYTOSKELETON***, transmitting both mechanical force and signals in both directions. ***THE TRANSMEMBRANE SEGMENT OF ANY MEMBRANE PROTEIN IS MADE OF HYDROPHOBIC AMINO ACIDS (usually an alpha-helix)***, matching the lipid core, with hydrophilic residues at both surfaces.

**UNDERSTAND:** The four adhesion families divide by job: ***INTEGRINS — cell-to-MATRIX (and leucocyte firm adhesion); CADHERINS — CALCIUM-dependent cell-to-cell (E-cadherin in epithelium); SELECTINS — CARBOHYDRATE-binding, mediating the transient ROLLING of leucocytes; and the IMMUNOGLOBULIN SUPERFAMILY (ICAM, VCAM, PECAM) — the endothelial partners for integrins.*** The leucocyte adhesion cascade uses them in sequence: ***MARGINATION → ROLLING (selectins) → FIRM ADHESION (integrins–ICAM) → TRANSMIGRATION (PECAM-1) → CHEMOTAXIS.***

**MUST KNOW:**
- **Diseases that prove the mechanism: ***LEUCOCYTE ADHESION DEFICIENCY TYPE 1 (CD18/beta-2 integrin defect — DELAYED UMBILICAL CORD SEPARATION, recurrent infections WITHOUT PUS, marked neutrophilia); TYPE 2 (selectin-ligand/sialyl-Lewis X defect); GLANZMANN THROMBASTHENIA (GpIIb/IIIa is an INTEGRIN); and LOSS OF E-CADHERIN in DIFFUSE GASTRIC and LOBULAR BREAST CARCINOMA — the molecular basis of invasion and metastasis.***
- **Therapeutic exploitation: ***ABCIXIMAB/eptifibatide (GpIIb/IIIa integrin), NATALIZUMAB (alpha-4 integrin, in multiple sclerosis and Crohn — risk of PML), VEDOLIZUMAB (gut-selective alpha-4-beta-7) and CRIZANLIZUMAB (P-selectin, in sickle cell disease).***
- **Membrane structure essentials: the ***FLUID MOSAIC MODEL***; cholesterol buffers fluidity; ***INTEGRAL proteins span the bilayer (removable only with detergent) while PERIPHERAL proteins attach to the surface***; glycoproteins and glycolipids face outward, forming the glycocalyx and carrying blood group antigens; and ***PHOSPHATIDYLSERINE IS NORMALLY CONFINED TO THE INNER LEAFLET — ITS APPEARANCE ON THE OUTER SURFACE FLAGS APOPTOSIS (and is the annexin V assay) AND ACTIVATES COAGULATION ON PLATELETS.***
- **Basement membrane components: ***TYPE IV COLLAGEN (the antigen in GOODPASTURE disease), LAMININ, entactin/nidogen and heparan sulphate proteoglycan (whose loss causes the protein leak of minimal change disease).***
- ***DESMOSOMES AND HEMIDESMOSOMES USE INTERMEDIATE FILAMENTS; ADHERENS JUNCTIONS AND FOCAL ADHESIONS USE ACTIN*** — a distinction worth carrying with the junction list.
...
## 10. Cerebellar Circuitry and the Optic Pathway
...
- **The ***TRIPLE RESPONSE to intradermal histamine — RED REACTION (capillary dilatation, independent of nerves), FLARE (arteriolar dilatation by the AXON REFLEX, SUBSTANCE P mediated) and WHEAL (increased permeability)*** — is the bedside test of the axon reflex. ***THE FLARE IS ABSENT WHEN THE POSTGANGLIONIC SENSORY FIBRE HAS DEGENERATED (e.g. a brachial plexus TRUNK injury or local anaesthesia), giving a wheal WITHOUT a flare — while the response is PRESERVED after sympathectomy or a PREGANGLIONIC lesion.***

**EXAM CONNECTION:** Mechanism of TENS → **gate control theory**; structure whose stimulation relieves pain (brainstem section) → **periaqueductal grey**; histamine test after a brachial plexus trunk injury → **wheal without flare**.
...
## 12. Respiratory Control and Lung Volumes
...
**MUST KNOW:**
- **Volumes and capacities: ***TIDAL VOLUME 500 mL; anatomical DEAD SPACE ~150 mL (≈2.2 mL/kg); RESIDUAL VOLUME, FRC and TOTAL LUNG CAPACITY CANNOT be measured by spirometry — they need HELIUM DILUTION, NITROGEN WASHOUT or BODY PLETHYSMOGRAPHY (the only method that also measures trapped gas).*** ***FRC IS THE VOLUME AT WHICH THE INWARD ELASTIC RECOIL OF THE LUNG EXACTLY BALANCES THE OUTWARD RECOIL OF THE CHEST WALL.*** If the lung is removed from the chest and allowed to recoil freely, it collapses to its ***MINIMAL VOLUME (less than residual volume)***, because the opposing chest-wall force is gone.
...
## 13. Vasopressin Escape and Water Handling
...
**MUST KNOW:**
- **ADH acts on ***V2 receptors (collecting duct — Gs/cAMP → AQUAPORIN-2 insertion, water reabsorption and urea recycling) and V1 receptors (vascular smooth muscle — vasoconstriction, and V1b on the pituitary releasing ACTH).*** Its stimuli, in order of potency: ***PLASMA OSMOLALITY (osmoreceptors, sensitive to a 1% change) then VOLUME/PRESSURE (baroreceptors, needing a ~10% fall but then overriding osmolality entirely — the reason a hypovolaemic patient will retain water even at the cost of hyponatraemia).*** Also nausea, pain, stress, nicotine and drugs.
- ***INSENSIBLE WATER LOSS IS ABOUT 500–700 mL/DAY (skin evaporation and respiration) AND MUST BE INCLUDED IN FLUID BALANCE CALCULATIONS*** — it rises with fever (about 10% per °C), tachypnoea and burns.
- **Daily water balance: intake ~2500 mL (drink 1500, food 700, metabolic water 300); output ~2500 (urine 1500, insensible 600–700, sweat 100, faeces 200).**
- **The counter-current mechanism: ***the LOOP OF HENLE multiplies (thick ascending limb NKCC2 pumping salt without water), the VASA RECTA exchanges passively (preserving the gradient), and UREA RECYCLING contributes about half of the medullary hypertonicity — which is why a MALNOURISHED patient (low urea) cannot concentrate urine maximally.***
- ***DIABETES INSIPIDUS (central vs nephrogenic) is separated by the DESMOPRESSIN TEST after water deprivation: urine osmolality RISES in CENTRAL DI and does NOT in NEPHROGENIC DI*** (lithium, hypercalcaemia, hypokalaemia, chronic renal disease).
...
## 14. Gastrointestinal Slow Waves

**PEARL:** ***THE BASAL ELECTRICAL RHYTHM (SLOW WAVES) IS GENERATED BY THE INTERSTITIAL CELLS OF CAJAL.*** Frequencies: ***STOMACH 3–4/min (the LOWEST), DUODENUM 12/min (the HIGHEST), distal ileum 8/min, caecum 2/min, sigmoid colon 6/min.*** ***SLOW WAVES DO NOT THEMSELVES CAUSE CONTRACTION*** — contraction occurs only when ***SPIKE POTENTIALS are superimposed on the depolarising phase.***

**UNDERSTAND:** The slow wave is a **permissive timing signal, not a command**: it sets the **MAXIMUM POSSIBLE FREQUENCY of contraction in each segment**, while whether a contraction actually happens depends on neural and hormonal modulation (***ACETYLCHOLINE increases spike potentials and tension; ADRENALINE decreases them***). The descending frequency gradient from duodenum to ileum is what gives the small intestine its net ***ABORAL (downward) propulsion*** — each segment paces slightly slower than the one above.
...
**PEARL:** ***A PARATHYROID ADENOMA CAUSES PRIMARY, NOT SECONDARY, HYPERPARATHYROIDISM.*** Secondary hyperparathyroidism is an **APPROPRIATE compensatory rise in PTH in response to HYPOCALCAEMIA**: ***CHRONIC KIDNEY DISEASE (failure of 1-alpha-hydroxylation and phosphate retention), VITAMIN D DEFICIENCY, malabsorption — and LITHIUM, which raises the calcium set-point for PTH suppression and causes parathyroid hyperplasia.***

**UNDERSTAND:** The distinction is entirely about ***WHERE THE FAULT LIES AND WHAT THE CALCIUM IS DOING.*** ***PRIMARY — an autonomous gland: HIGH PTH with HIGH calcium and LOW phosphate. SECONDARY — a normal gland responding correctly: HIGH PTH with LOW or NORMAL calcium and (in CKD) HIGH phosphate. TERTIARY — prolonged secondary stimulation makes the hyperplastic gland autonomous: HIGH PTH with HIGH calcium***, classically after long-standing renal failure or transplantation.
...
## 16. Cellular Organelles and Secretory Polarity
...
## 17. The Cardiac Cycle in Sequence
...
## 18. Body Fluid Compartments and Indicator Dilution

**PEARL:** ***VOLUME = (AMOUNT INJECTED − AMOUNT EXCRETED) ÷ PLASMA CONCENTRATION.*** For 10 g of mannitol with 10% excreted and a plasma level of 65 mg/100 mL (0.65 g/L): **(10 − 1) ÷ 0.65 ≈ 14 L = the EXTRACELLULAR FLUID VOLUME**, because ***MANNITOL CANNOT CROSS CELL MEMBRANES AND SO DISTRIBUTES ONLY IN THE ECF.***

**UNDERSTAND:** The whole method rests on choosing an indicator that **distributes in exactly the compartment you want to measure and nowhere else.** That single principle generates the entire table: ***TOTAL BODY WATER — deuterium/tritiated water, antipyrine (they cross every membrane); ECF — INULIN, MANNITOL, sucrose, thiosulphate, radioactive sodium/chloride/bromide; PLASMA — EVANS BLUE and RADIOIODINATED ALBUMIN (they stay inside vessels); RED CELL MASS — Cr-51-labelled red cells.*** ***INTERSTITIAL FLUID AND INTRACELLULAR FLUID CANNOT BE MEASURED DIRECTLY — they are obtained by SUBTRACTION*** (interstitial = ECF − plasma; ICF = total body water − ECF).

**MUST KNOW:**
- **The 60-40-20 rule: ***TOTAL BODY WATER ≈ 60% of body weight (LOWER in women and the obese — fat is water-poor — and HIGHER in neonates at ~75%); ICF 40%; ECF 20% (INTERSTITIAL 15%, PLASMA 5%).*** Blood volume ≈ 8% of body weight (~5 L).
- ***THE MAJOR ECF CATION IS SODIUM AND THE MAJOR ICF CATION IS POTASSIUM***; ICF anions are largely PROTEIN and ORGANIC PHOSPHATE. ***OSMOLALITY IS IDENTICAL ACROSS THE CELL MEMBRANE (~285–295 mOsm/kg) because water moves freely — it is COMPOSITION, not concentration, that differs.***
- ***THE GIBBS–DONNAN EFFECT IS PRODUCED BY NON-DIFFUSIBLE INTRACELLULAR (and plasma) PROTEIN ANIONS***: because they cannot cross, the diffusible ions redistribute predictably, so the protein-containing compartment ends up with more total osmotically active particles — which is why plasma has a slightly higher small-ion content than interstitial fluid and why ***PLASMA PROTEINS CONTRIBUTE MORE OSMOTIC PULL THAN THEIR CONCENTRATION ALONE WOULD PREDICT.***
- **STARLING FORCES: filtration = ***(Pc − Pi) − σ(πc − πi).*** Typical values: capillary hydrostatic ~32 mmHg (arteriolar) falling to ~15 (venular); ***PLASMA ONCOTIC ~25 mmHg; INTERSTITIAL ONCOTIC ~12 mmHg (small, but not zero — highest in liver and kidney); interstitial hydrostatic slightly negative.*** ***OEDEMA ARISES FROM RAISED CAPILLARY PRESSURE (heart failure, venous obstruction), LOW PLASMA ONCOTIC PRESSURE (nephrotic syndrome, liver disease, malnutrition), INCREASED PERMEABILITY (inflammation, burns) OR LYMPHATIC OBSTRUCTION (filariasis, post-surgical).***
- **Fluid shifts to reason through: ***ISOTONIC SALINE expands ECF only; DEXTROSE 5% (effectively free water) expands BOTH compartments; HYPERTONIC SALINE expands ECF while SHRINKING cells; pure water loss (diabetes insipidus) shrinks both.***
...
## 19. Excitation–Contraction Coupling in Skeletal versus Cardiac Muscle
...
**EXAM CONNECTION:** True about skeletal muscle contraction → **calcium is released from the sarcoplasmic reticulum**; DHPR–RyR coupling in skeletal muscle → **mechanical**.
...
## 20. The Countercurrent Mechanism and the Juxtaglomerular Apparatus
...
**MUST KNOW:**
- ***UREA RECYCLING contributes about half of the medullary hypertonicity (ADH increases UT-A1 urea permeability in the inner medullary collecting duct)*** — hence protein-malnourished patients concentrate urine poorly.
- **The JGA has three cell types: ***MACULA DENSA (chemoreceptor, distal tubule), JUXTAGLOMERULAR/GRANULAR CELLS (modified afferent arteriolar smooth muscle — BARORECEPTORS that SECRETE RENIN) and LACIS/extraglomerular mesangial cells.***
- ***RENIN RELEASE IS STIMULATED BY: a FALL in afferent arteriolar pressure, a FALL in macula densa NaCl delivery, and SYMPATHETIC BETA-1 STIMULATION*** — which is why beta-blockers lower renin, and why renin is high in renovascular disease and low in primary hyperaldosteronism.
- **The RAAS cascade: ***renin → angiotensin I → (ACE, in lung) → ANGIOTENSIN II → efferent arteriolar constriction (preserving GFR), proximal sodium reabsorption, aldosterone release, ADH and thirst, and sympathetic facilitation.*** ***ALDOSTERONE ACTS ON MINERALOCORTICOID RECEPTORS PRESENT IN THE KIDNEY (principal cells), COLON, SALIVARY AND SWEAT GLANDS AND THE HIPPOCAMPUS — BUT NOT THE LIVER.***
- **Aldosterone selectivity is enforced by ***11-BETA-HYDROXYSTEROID DEHYDROGENASE TYPE 2***, which inactivates cortisol in mineralocorticoid target tissues; its inhibition (LIQUORICE) or mutation (apparent mineralocorticoid excess) lets cortisol act as a mineralocorticoid — hypertension with hypokalaemia and LOW renin AND LOW aldosterone.

**EXAM CONNECTION:** NOT a component of the countercurrent mechanism → **sodium outflow in the thin ascending limb**; function of tubuloglomerular feedback → **regulate GFR**; the sodium-sensing chemoreceptor of the JGA → **macula densa**; mineralocorticoid receptors absent in → **liver**.
...
## 21. Decerebrate and Decorticate Rigidity

**PEARL:** ***DECEREBRATE RIGIDITY (a MID-COLLICULAR transection, between the superior and inferior colliculi) = EXTENSION OF ALL FOUR LIMBS — extensor hypertonus, NOT rigidity of every muscle in the body.*** ***DECORTICATE RIGIDITY (a lesion ABOVE the red nucleus) = FLEXION OF THE UPPER LIMBS WITH EXTENSION OF THE LOWER LIMBS.*** Decerebrate rigidity is a ***GAMMA rigidity*** — the excitatory reticulospinal drive activates gamma motor neurones, which excite alpha motor neurones indirectly through Ia spindle afferents — and ***decerebration produces NO phenomenon akin to spinal shock.***

**UNDERSTAND:** The two postures are explained by ***WHICH DESCENDING TRACT SURVIVES.*** Above the red nucleus, the ***RUBROSPINAL tract is intact and facilitates UPPER LIMB FLEXORS → decorticate posture.*** Cut below the red nucleus and that flexor drive is lost, leaving only the ***unopposed EXCITATORY (pontine) RETICULOSPINAL and VESTIBULOSPINAL tracts, which drive antigravity EXTENSORS everywhere → decerebrate posture.*** ***A DEEPER LESION THEREFORE PRODUCES A WORSE POSTURE — the progression from decorticate to decerebrate posturing signals rostrocaudal deterioration and is an ominous clinical sign.***

**MUST KNOW:**
- **On the Glasgow Coma Scale, ***motor response: 6 obeys, 5 localises, 4 withdraws, 3 ABNORMAL FLEXION (decorticate), 2 EXTENSION (decerebrate), 1 none.***
- ***CUTTING THE DORSAL ROOTS ABOLISHES DECEREBRATE (GAMMA) RIGIDITY BUT NOT ALPHA RIGIDITY*** (which follows an anterior cerebellar lobe lesion) — the classic experimental distinction.
- **Related upper motor neurone physiology: ***SPINAL SHOCK (areflexia and flaccidity for days to weeks after cord transection, followed by hyperreflexia, spasticity and the BABINSKI sign); the CLASP-KNIFE phenomenon (velocity-dependent spasticity, mediated by Golgi tendon organ Ib afferents) versus LEAD-PIPE/COGWHEEL rigidity of extrapyramidal disease.***
- **The muscle spindle (in parallel with the muscle, sensing LENGTH and rate of change; Ia and II afferents; ***GAMMA motor neurones keep it sensitive during contraction — ALPHA-GAMMA COACTIVATION***) versus the ***GOLGI TENDON ORGAN (in SERIES, sensing TENSION, Ib afferents, INHIBITORY — the autogenic inhibition/inverse stretch reflex).***
- **Sensory pathway anatomy to pair with it: ***the DORSAL COLUMN–MEDIAL LEMNISCUS system carries FINE TOUCH, VIBRATION AND PROPRIOCEPTION, ascends IPSILATERALLY in the cord, decussates in the MEDULLA (internal arcuate fibres) and ascends as the MEDIAL LEMNISCUS to the contralateral VPL thalamus — SO A PONTINE MEDIAL LEMNISCUS LESION CAUSES CONTRALATERAL loss of tactile sensation and proprioception***, whereas the spinothalamic tract (pain and temperature) has already crossed in the cord.
...
## 22. Peripheral Chemoreceptors and Oxygen Sensing

**PEARL:** ***THE OXYGEN SENSOR OF THE CAROTID AND AORTIC BODIES IS AN OXYGEN-SENSITIVE POTASSIUM CHANNEL ON THE TYPE I (GLOMUS) CELL.*** Hypoxia ***REDUCES K+ conductance → less K+ efflux → DEPOLARISATION → voltage-gated CALCIUM entry → transmitter release → firing in the afferent (glossopharyngeal/vagal) nerve.***

**UNDERSTAND:** Note the direction of the logic — ***THE CHANNEL IS INHIBITED BY THE ABSENCE of oxygen, so the cell is depolarised by a FALL in an inhibitory conductance*** rather than by opening an excitatory one. The carotid body also has the ***HIGHEST BLOOD FLOW PER GRAM OF ANY TISSUE IN THE BODY (about 2000 mL/100 g/min)***, which means its venous PO2 is nearly arterial — so it senses ***ARTERIAL PO2, NOT OXYGEN CONTENT.*** That single fact explains why ***PERIPHERAL CHEMORECEPTORS ARE NOT STIMULATED IN ANAEMIA OR CARBON MONOXIDE POISONING*** (content is low but PaO2 is normal) — and why those patients do not hyperventilate.
...
## 23. Blood Storage and Anticoagulant Solutions

**PEARL:** ***ACD (acid citrate dextrose) — 21 DAYS. CPD (citrate phosphate dextrose, adding phosphate) — 21 DAYS (phosphate improves 2,3-BPG but does NOT extend shelf life). CPDA-1 (adding ADENINE) — 35 DAYS (adenine supports ATP regeneration). ADDITIVE SOLUTIONS (SAGM — saline, adenine, glucose, mannitol) — 42 DAYS.*** ***CITRATE IS THE ANTICOAGULANT — it CHELATES CALCIUM.***

**UNDERSTAND:** Each additive fixes a different failure of the stored red cell. **Citrate stops clotting; DEXTROSE fuels glycolysis; PHOSPHATE maintains 2,3-BPG (and therefore oxygen unloading); ADENINE supplies substrate for ATP synthesis (and ATP maintains membrane shape and the sodium pump); MANNITOL stabilises the membrane against haemolysis.** The limiting factor for shelf life is ***ATP DEPLETION AND HAEMOLYSIS, NOT OXYGEN AFFINITY*** — which is why adenine, not phosphate, buys extra days.
...
- ***IRRADIATED components for the severely immunocompromised; CMV-negative or leucodepleted for neonates and transplant recipients.***
...
## 24. Thermoregulation and Temperature Measurement

**PEARL:** ***WHEN THE ENVIRONMENTAL TEMPERATURE EXCEEDS BODY TEMPERATURE (e.g. 47 °C), THE ONLY MECHANISM OF HEAT LOSS IS EVAPORATION — SWEATING.*** Radiation, conduction and convection all reverse and become routes of heat GAIN. On measurement: ***RECTAL TEMPERATURE (core) > ORAL (≈0.5 °C lower) > TYMPANIC (≈0.8 °C lower than rectal) > AXILLARY (the LOWEST) — so rectal is HIGHER, never lower, than axillary.***

**UNDERSTAND:** Heat exchange by radiation, conduction and convection depends on a ***GRADIENT from body to environment***; once the environment is hotter, the gradient reverses. **Evaporation is the only mechanism that is independent of the gradient — it depends on the LATENT HEAT of vaporisation (0.58 kcal/mL of sweat)**, which is why it is the sole defence in a hot environment. ***AND IT FAILS IN HIGH HUMIDITY***, because sweat that drips rather than evaporates removes no heat — the physiological reason humid heat waves kill.
...
**PEARL:** ***E_ion = (61 ÷ z) × log₁₀ ([outside] ÷ [inside]) mV at 37 °C.*** For chloride with 100 mmol/L outside and 10 inside (z = −1): ***E_Cl = −61 × log(10) = −61 ≈ −60 mV.*** Typical equilibrium potentials: ***K+ ≈ −90 mV, Na+ ≈ +60 mV, Ca2+ ≈ +125 mV, Cl− ≈ −70 mV***, against a resting membrane potential of about −70 to −90 mV.

**UNDERSTAND:** The equilibrium potential is **the voltage at which the electrical force exactly opposes the concentration force for that ion — the potential the membrane would reach if it were permeable ONLY to that ion.** Hence the resting potential (dominated by potassium permeability) sits close to E_K, and the peak of the action potential (dominated by sodium permeability) approaches E_Na. ***THE GOLDMAN–HODGKIN–KATZ EQUATION IS SIMPLY THE WEIGHTED AVERAGE OF THESE POTENTIALS BY PERMEABILITY*** — so the membrane potential always moves toward the equilibrium potential of whichever channel is open.
...
## 26. Chemical Synapses, Junctions and Exosomes

**PEARL:** ***CHEMICAL SYNAPSES CONDUCT UNIDIRECTIONALLY***, have a cleft of **20–30 nm**, a synaptic delay of ~0.5 ms, transmitter vesicles and NO gap junctions. ***ELECTRICAL SYNAPSES have a gap of only 2–4 nm bridged by CONNEXONS, conduct BIDIRECTIONALLY with essentially no delay, and allow free ion movement.*** Junction molecules by tissue: ***OCCLUDIN — tight junctions (kidney tubule, epithelial barriers); CONNEXONS — heart; DESMOCOLLIN/desmoglein — skin (desmosomes); E-CADHERIN — epithelium generally.*** ***EXOSOMES — small membrane vesicles released from multivesicular bodies — are a major mediator of INTERCELLULAR COMMUNICATION in eukaryotes.***

**UNDERSTAND:** ***UNIDIRECTIONALITY IS THE PRICE AND THE POINT OF A CHEMICAL SYNAPSE***: transmitter is stored on only one side and receptors sit on the other, so the signal can only run one way — but that constraint is what permits **integration, amplification, INHIBITION and PLASTICITY**, none of which an electrical synapse can do. Speed is traded for computational power; the nervous system keeps electrical synapses only where **synchrony** matters (cardiac muscle, some brainstem escape reflexes, retina).

**MUST KNOW:**
- **Synaptic transmission steps: ***action potential → voltage-gated CALCIUM entry at the terminal → SNARE-mediated vesicle fusion → transmitter release → receptor binding → EPSP or IPSP → SUMMATION (temporal and spatial) at the AXON HILLOCK, which has the lowest threshold and is where the action potential is generated.***
- ***EXOSOMES carry proteins, mRNA and microRNA between cells and are being exploited as LIQUID-BIOPSY biomarkers and as drug-delivery vehicles***; they also transfer oncogenic signals and prion-like proteins. Distinguish them from ***MICROVESICLES (budding directly from the plasma membrane) and APOPTOTIC BODIES.***
- **Other forms of intercellular signalling: ***ENDOCRINE (bloodborne), PARACRINE (neighbouring cells), AUTOCRINE (the cell itself), JUXTACRINE (direct contact), NEUROCRINE and via GAP JUNCTIONS.***
- **Second messengers to keep together: ***cAMP (Gs/Gi — protein kinase A), IP3/DAG and CALCIUM (Gq — protein kinase C), cGMP (nitric oxide and ANP — protein kinase G), and the JAK–STAT and receptor tyrosine kinase pathways for cytokines and growth factors.***
- **Neurotransmitter removal is as important as release: ***ENZYMATIC degradation (acetylcholinesterase), REUPTAKE (monoamines — the target of SSRIs, cocaine and amphetamines) and diffusion; glutamate is cleared by ASTROCYTES, and its accumulation causes EXCITOTOXICITY.***
...
## 27. Memory and Dietary Fibre

**PEARL:** ***SHORT-TERM MEMORY LASTS SECONDS TO MINUTES (of the usual options, 30–300 seconds)*** and depends on continued rehearsal; encoding of short-term EXPLICIT memory involves the ***HIPPOCAMPUS***, while ***LONG-TERM memories are stored in the NEOCORTEX.*** Among dietary fibres — cellulose, hemicellulose, LIGNIN, PECTIN, INULIN and GUMS — ***GUMS CONTRIBUTE THE LEAST TO A NORMAL DIET.***

**UNDERSTAND:** The clinical value of the memory classification is that ***DIFFERENT ANATOMICAL SYSTEMS FAIL SEPARATELY.*** ***EXPLICIT (declarative) memory — facts and events — requires the HIPPOCAMPUS AND MEDIAL TEMPORAL LOBE, so bilateral damage (the patient H.M., herpes encephalitis, or thiamine deficiency in KORSAKOFF syndrome) produces a dense ANTEROGRADE amnesia with PRESERVED skills and preserved old memories.*** ***IMPLICIT (procedural) memory — skills, habits, conditioning — depends on the BASAL GANGLIA, CEREBELLUM AND AMYGDALA and survives hippocampal damage.*** That dissociation is the single most examined fact in memory physiology.
...
**MUST KNOW:**
- **The three stimulants of the parietal cell converge on the pump: ***ACETYLCHOLINE (M3, vagal — Gq/calcium), GASTRIN (CCK-B — Gq/calcium, and largely INDIRECT via histamine release from ECL cells) and HISTAMINE (H2 — Gs/cAMP, the FINAL COMMON AMPLIFIER — which is why H2 blockers work at all three).*** ***SOMATOSTATIN from D cells inhibits all of them, and is itself inhibited by vagal stimulation.***
- **Phases of secretion: ***CEPHALIC (~30%, vagal — sight, smell, thought of food), GASTRIC (~60%, distension and peptides — gastrin-mediated), INTESTINAL (~10%).*** ***ANTRAL ACIDIFICATION BELOW pH 3 SWITCHES OFF GASTRIN — the negative feedback lost in Zollinger–Ellison syndrome.***
- ***THE ENTEROGASTRIC INHIBITORY REFLEX IS TRIGGERED BY: DUODENAL DISTENSION, MUCOSAL IRRITATION, ACID (not alkali) IN THE DUODENUM, NON-ISOTONIC (hyper- OR hypotonic) CHYME, AND BREAKDOWN PRODUCTS OF PROTEIN AND FAT*** — all slowing gastric emptying so the duodenum is not overwhelmed. (Fat is the most potent inhibitor, acting through CCK and GIP.)
- **Mucosal defence: mucus–bicarbonate layer, prostaglandins (PGE2 — removed by NSAIDs), tight junctions, rapid restitution and mucosal blood flow.**
- **Intrinsic factor is also a parietal cell product — ***THE ONLY INDISPENSABLE GASTRIC SECRETION***, which is why atrophic gastritis and gastrectomy cause B12 deficiency.

**EXAM CONNECTION:** H+ is secreted in exchange for → **K+**; true about gastrin → **it is a polypeptide**; primary site of gastrin production → **antrum**; enterogastric reflex is triggered by all EXCEPT → **alkaline contents**.
...
## 29. Gastrointestinal Hormone Families
...
## 31. Absorption in the Small Intestine and Colon
...
**MUST KNOW:**
- **Site-specific absorption to memorise: ***IRON — DUODENUM (and upper jejunum); FOLATE — duodenum and jejunum; VITAMIN B12 — TERMINAL ILEUM (with intrinsic factor); BILE SALTS — terminal ileum; CALCIUM — duodenum (vitamin D dependent); most vitamins — upper small intestine; MAXIMUM WATER ABSORPTION — JEJUNUM; SCFAs and residual water and electrolytes — COLON.***
- **Daily fluid handling: ***~2 L ingested + ~7 L of secretions (saliva 1.5, gastric 2, bile 0.5, pancreatic 1.5, intestinal 1.5) = ~9 L presented; ~8–8.8 L absorbed; only 100–200 mL lost in faeces*** — which is why any failure of this system causes such rapid dehydration.
- **Vitamin transport detail: ***THIAMINE, RIBOFLAVIN, NIACIN, PYRIDOXINE, PANTOTHENATE, BIOTIN AND ASCORBATE use SODIUM-COTRANSPORTERS — but FOLATE AND B12 ABSORPTION ARE SODIUM-INDEPENDENT.*** ***FAT-SOLUBLE VITAMINS are ingested as esters, need CHOLESTEROL ESTERASE and micelle incorporation*** — hence deficiency in any cause of fat malabsorption.
- **Iron: absorbed as ***Fe2+ via DMT1 (aided by gastric acid and vitamin C), exported by FERROPORTIN, and REGULATED BY HEPCIDIN, which binds ferroportin and degrades it (hence the iron-restricted anaemia of chronic disease). ELEMENTAL IRON ITSELF STIMULATES FERRITIN SYNTHESIS*** by releasing the iron-regulatory protein from ferritin mRNA, allowing its translation — a classic example of translational control.
- ***BILE SALTS ARE ESSENTIAL FOR FAT ABSORPTION BUT NOT FOR SHORT- AND MEDIUM-CHAIN FATTY ACIDS***, which diffuse directly into the portal blood — the reason MCT oil is used in fat malabsorption and lymphangiectasia.
...
## 32. Basal Ganglia Circuitry and Movement Disorders
...
**PEARL:** ***THE OVARY CONTAINS ITS MAXIMUM NUMBER OF OOGONIA — ABOUT 7 MILLION — AT THE 5TH MONTH OF INTRAUTERINE LIFE***, falling to ~2 million at birth, <400,000 at puberty and only ~400–500 ever ovulated. ***MEIOSIS I (the REDUCTIONAL division) OCCURS FROM PRIMARY TO SECONDARY SPERMATOCYTE*** (and from primary to secondary oocyte); meiosis II is equational. Spermatogenesis takes ***~74 days, plus ~14 days of epididymal transit — so a semen analysis after hormonal treatment is repeated at 80–90 days (about 3 months).***

**UNDERSTAND:** The female germ cell timeline is unique and clinically decisive: ***OOGONIA ARE NEVER MADE AFTER FETAL LIFE — the entire stock is fixed before birth and only declines.*** Primary oocytes arrest in ***PROPHASE I (dictyate stage) from fetal life until ovulation***, and the secondary oocyte arrests again in ***METAPHASE II until fertilisation.*** That decades-long arrest is the accepted explanation for the ***RISING RISK OF NON-DISJUNCTION AND ANEUPLOIDY WITH MATERNAL AGE.*** Spermatogenesis, by contrast, is continuous from puberty — which is why male fertility declines slowly, and why the 74-day cycle dictates that ***ANY INTERVENTION MUST BE JUDGED ONLY AFTER ~3 MONTHS.***

**MUST KNOW:**
- **Spermiogenesis (spermatid → spermatozoon) comprises ***ACROSOME formation, NUCLEAR CONDENSATION, formation of the neck, midpiece and tail, and SHEDDING OF CYTOPLASM as residual bodies***; ***MOTILITY IS ACQUIRED IN THE EPIDIDYMIS.*** About 100 million sperm are produced daily.
- **Hormonal control: ***LH → LEYDIG cells → TESTOSTERONE; FSH → SERTOLI cells → androgen-binding protein and INHIBIN B (the negative feedback on FSH).*** ***SERTOLI CELLS FORM THE BLOOD–TESTIS BARRIER***, protecting haploid germ cells from immune recognition — its breach (trauma, vasectomy) can cause antisperm antibodies. ***THE TESTIS SITS 2 °C BELOW CORE TEMPERATURE — hence infertility in cryptorchidism and varicocele.***
- **Female axis: ***FSH drives granulosa cells (aromatase, converting thecal androgen to oestrogen — the TWO-CELL, TWO-GONADOTROPIN theory); the LH SURGE (triggered by sustained high oestrogen — POSITIVE feedback) causes ovulation ~36 hours later and resumption of meiosis I.***
- **Capacitation and fertilisation: ***CAPACITATION takes about 6–8 HOURS in the female tract (removal of the glycoprotein coat and seminal plasma proteins) and is REQUIRED before the ACROSOME REACTION, which is triggered by binding to the ZONA PELLUCIDA (ZP3) and releases ACROSIN.*** ***FERTILISATION OCCURS IN THE AMPULLA OF THE FALLOPIAN TUBE***, and the ***CORTICAL REACTION (zona reaction) blocks polyspermy.***
- **Semen analysis (WHO): volume ≥1.4 mL, concentration ≥16 million/mL, total motility ≥42%, normal forms ≥4%; ***AZOOSPERMIA must be confirmed on a repeat sample after centrifugation.***
...
## 34. Implantation and the Decidua
...
## 35. Central Control of Appetite
...
**MUST KNOW:**
- **Peripheral signals: ***LEPTIN (adipose, long-term adiposity signal — inhibits NPY/AgRP, stimulates POMC), INSULIN (adiposity signal), GHRELIN (stomach, rises BEFORE meals — the only orexigenic gut hormone), CCK, PYY, GLP-1 and oxyntomodulin (satiety after meals).***
- ***OREXIN DEFICIENCY (loss of lateral hypothalamic orexin neurones, HLA-DQB1*06:02 associated) CAUSES NARCOLEPSY WITH CATAPLEXY*** — the same peptide regulates arousal and feeding, which is why narcolepsy is associated with obesity.
- **Other hypothalamic functions worth listing by nucleus: ***SUPRAOPTIC and PARAVENTRICULAR — ADH and oxytocin; SUPRACHIASMATIC — circadian rhythm; ANTERIOR — heat loss (parasympathetic); POSTERIOR — heat conservation (sympathetic); MAMMILLARY BODIES — memory and feeding reflexes such as licking and swallowing; ARCUATE — appetite and hypophysiotropic hormones.***
- **Bariatric physiology confirms the model: ***GASTRIC BYPASS produces weight loss partly by raising GLP-1 and PYY and lowering ghrelin — not merely by restriction.***
- ***THE SET-POINT/ADAPTIVE THERMOGENESIS RESPONSE (falling energy expenditure and rising ghrelin after weight loss) IS WHY DIETARY WEIGHT LOSS IS SO OFTEN REGAINED*** — physiology defends fat stores far more vigorously than it resists their gain.
...
## 36. Protein Digestion and Peptide Absorption

**PEARL:** ***PROENZYMES ARE ACTIVATED BY SELECTIVE PROTEOLYSIS — THE REMOVAL OF A FEW AMINO ACIDS*** (not by phosphorylation or dimerisation): ***ENTEROKINASE (enteropeptidase) on the duodenal brush border cleaves TRYPSINOGEN to TRYPSIN, and trypsin then activates all the others (including more trypsinogen).*** ***CARBOXYPEPTIDASE IS AN EXOPEPTIDASE — trypsin, chymotrypsin, elastase and pepsin are ENDOPEPTIDASES.*** ***DI- AND TRIPEPTIDES ARE ABSORBED INTACT BY PepT1, a PROTON-COUPLED transporter***, and are hydrolysed to amino acids inside the enterocyte.

**UNDERSTAND:** Two design principles are being tested. First, ***DIGESTIVE ENZYMES MUST BE MADE AS INACTIVE ZYMOGENS AND ACTIVATED ONLY AT THE TARGET SITE*** — otherwise the pancreas digests itself; the single gate-keeping step is enterokinase, located in the *duodenum*, not the pancreas. Second, ***PEPTIDES ARE ABSORBED FASTER THAN FREE AMINO ACIDS***, because PepT1 is a broad, high-capacity carrier — which is why peptide-based (semi-elemental) feeds are absorbed better than amino-acid formulas in short bowel syndrome, and why ***beta-lactam antibiotics, ACE inhibitors and valaciclovir are absorbed by hijacking PepT1.***
...
## 37. Regulation of Gastric Emptying

**PEARL:** ***GASTRIC EMPTYING IS REGULATED MAINLY BY ENTERIC (duodenal) REFLEXES*** — the ***ENTEROGASTRIC REFLEX***, conducted chiefly through the ***ENTERIC NERVOUS SYSTEM within the gut wall (major route)***, with sympathetic and vagal routes contributing less, plus hormonal feedback from ***CCK.*** ***GASTRIC DISTENSION INCREASES emptying*** (mechanoreceptor-driven antral contraction), and ***GASTRIN also PROMOTES emptying — whereas SECRETIN, CCK, GIP, FAT and ACID IN THE DUODENUM DELAY it.***

**UNDERSTAND:** The stomach empties at a rate set almost entirely by ***WHAT THE DUODENUM CAN COPE WITH***, not by what the stomach contains. Acid must be neutralised, fat emulsified, hypertonic chyme diluted — so each of these triggers feedback inhibition. That is why ***FAT IS THE MOST POTENT INHIBITOR OF GASTRIC EMPTYING***, and why ***LIQUIDS (needing no processing) EMPTY FASTER THAN SOLIDS.*** The single most sensitive trigger of the enterogastric reflex is ***ACID (and irritation) in the duodenum.***

**MUST KNOW:**
- ***RECEPTIVE RELAXATION AND GASTRIC ACCOMMODATION*** (vagally mediated, NO/VIP) allow the fundus to hold a meal without a pressure rise; loss of this after vagotomy or fundoplication causes early satiety and bloating.
- ***DUMPING SYNDROME*** is the consequence of losing this control: after gastrectomy or pyloroplasty, hypertonic chyme floods the jejunum → ***EARLY dumping (15–30 min: abdominal pain, diarrhoea, flushing, tachycardia, hypotension from fluid shift and vasoactive peptide release) and LATE dumping (1–3 h: reactive HYPOGLYCAEMIA from an exaggerated insulin response).*** Treated by small dry meals, avoiding simple sugars, and octreotide.
- **Delayed emptying (***GASTROPARESIS***): diabetes (autonomic neuropathy with loss of interstitial cells of Cajal), post-vagotomy, opioids, anticholinergics, GLP-1 agonists, hypothyroidism, scleroderma — diagnosed by ***gastric emptying SCINTIGRAPHY*** and treated with prokinetics and dietary modification.
- **Physiological modifiers: ***emptying is faster for liquids, carbohydrate > protein > fat, and slower with high osmolality, low pH, cold meals, pain, anxiety and exercise.***
- **Autonomic rule for the whole gut: ***PARASYMPATHETIC (vagus, pelvic splanchnics) — CONTRACTS the wall, RELAXES sphincters, INCREASES secretion; SYMPATHETIC — RELAXES the wall, CONTRACTS sphincters, DECREASES secretion and blood flow.*** ***ENKEPHALIN (and opioids) INHIBIT gastric motility***, while gastrin, motilin, substance P, substance K and angiotensin stimulate it.
...
## 38. Intestinal and Colonic Motility

**PEARL:** ***SECRETIN (and glucagon) INHIBIT small intestinal motility; GASTRIN, CCK, MOTILIN, INSULIN and SEROTONIN — and the GASTROENTERIC and GASTROILEAL reflexes and parasympathetic stimulation — INCREASE it.*** ***THE COLON HAS THE LONGEST TRANSIT TIME OF ANY GUT SEGMENT (up to 24–48 hours; the oesophagus takes 2–3 SECONDS), and the SIGMOID COLON SHOWS THE GREATEST MOTILITY.*** Distending the rectum (as with gas at sigmoidoscopy) increases peristalsis in the ***DISTAL COLON.***

**UNDERSTAND:** The colon is built for ***storage and salvage, not transport*** — slow, segmenting HAUSTRAL contractions maximise contact time for water and short-chain fatty acid absorption. Propulsion happens only in brief bursts (***MASS MOVEMENTS, 1–3 times a day, typically after a meal***), which is why defaecation is episodic. The rectal distension response is a ***local myenteric reflex*** — it spreads only through the descending colon, sigmoid and rectum, not the whole gut.

**MUST KNOW:**
- **The named reflexes: ***GASTROCOLIC — a meal (gastric distension) triggers mass movement and the urge to defaecate (exaggerated in irritable bowel syndrome); GASTROILEAL — a meal empties the ileum through the ileocaecal valve; ENTEROGASTRIC — duodenal contents inhibit the stomach; ILEOGASTRIC — ileal distension inhibits gastric motility; INTESTINO-INTESTINAL — distension or injury of one segment inhibits the whole gut (the mechanism of PARALYTIC ILEUS).***
...
**PEARL:** **Alpha waves (8–13 Hz)** appear in the **awake, relaxed adult with eyes CLOSED**
— best over the **occipital** region — and are **abolished by opening the eyes or mental
activity (alpha block/desynchronisation), which replaces them with beta.**

**UNDERSTAND:** EEG frequency is an index of **cortical synchronisation**. When the cortex
is idle, large populations of neurons oscillate together with the thalamic pacemaker,
producing **slow, high-amplitude, synchronised** waves. When the cortex is actively
processing, different columns do different things, so the summed signal becomes **fast,
low-amplitude and desynchronised**. So frequency rises and amplitude falls as alertness and
mental work increase — that single rule orders the whole table.

**MUST KNOW:**

| Wave | Frequency | State |
|---|---|---|
| **Beta** | **>13–30 Hz** | Alert, **eyes open**, mental activity; also seen with **benzodiazepines and barbiturates** |
| **Alpha** | **8–13 Hz** | **Awake, relaxed, EYES CLOSED**, occipital; meditation |
...
**EXAM CONNECTION:** The state described in the stem is the whole question. **Eyes closed
and relaxed → alpha, 8–13 Hz.** The distractors always attach the wrong frequency to the
right wave.
...
## 40. The Migrating Motor Complex

**PEARL:** The **migrating motor complex (MMC)** is the **FASTING/INTERDIGESTIVE** motility
pattern — it is **active between meals, ABOLISHED by eating**, recurs every **90–120
minutes**, sweeps **aborally from stomach to terminal ileum**, and is initiated by
**MOTILIN** from **M cells** of the duodenum and jejunum.

**UNDERSTAND:** The MMC is the gut's **housekeeper**. Its job is to sweep residual food,
secretions, desquamated cells and bacteria distally so the small bowel is not colonised —
which is precisely why it must run when the gut is **empty** and must stop when food
arrives (you do not want to sweep a meal through before it is absorbed). Loss of the MMC —
in **diabetic autonomic neuropathy, scleroderma, blind loops** — leads to **small
...
- **Mass movements** in the colon occur 1–3 times daily, often after a meal (**gastrocolic reflex**).
- Prokinetics: **metoclopramide and domperidone (D2 antagonists), erythromycin (motilin agonist), prucalopride (5-HT4 agonist)**.

**EXAM CONNECTION:** Both statements in the assertion–reason format are false —
**90–120 minutes, not 60**, and **active during fasting, not inactive**. Learn both numbers
...
## 41. Zones of the Adrenal Cortex

**PEARL:** **Zona GLOMERULOSA (outermost) → ALDOSTERONE → sodium and water retention,
potassium excretion.** **Zona FASCICULATA → cortisol → gluconeogenesis and
immunosuppression.** **Zona RETICULARIS (innermost) → androgens (DHEA, androstenedione).**
Medulla → catecholamines.

**UNDERSTAND:** The zones are functionally separated by **which enzymes they express and
which signal drives them**. Only the glomerulosa has **aldosterone synthase (CYP11B2)**,
and it is controlled by **angiotensin II and potassium — NOT by ACTH** — which is why
**aldosterone secretion is preserved in secondary adrenal insufficiency (pituitary
failure)** while cortisol collapses. Only the fasciculata and reticularis have **17α-
hydroxylase**, the gateway to cortisol and androgens, and both are **ACTH-driven** — which
is why ACTH excess causes hypercortisolism *and* hirsutism.
...
- **Congenital adrenal hyperplasia — 21-hydroxylase deficiency (>90%)**: cortisol and aldosterone LOW, **androgens HIGH** → **salt-wasting crisis with hyponatraemia, hyperkalaemia and virilised female genitalia**; raised **17-hydroxyprogesterone**. **11β-hydroxylase deficiency** → hypertension with virilisation (deoxycorticosterone excess); **17α-hydroxylase deficiency** → hypertension **without** virilisation.
- **Addison disease (primary):** all three zones fail — **hyperpigmentation (high ACTH/POMC), hyponatraemia, hyperkalaemia, hypotension**; diagnose with the **short Synacthen test**. **Secondary:** no pigmentation, aldosterone intact, no hyperkalaemia.
- The **medulla** is modified sympathetic neural tissue; **cortisol from the cortex induces PNMT**, converting noradrenaline to adrenaline — hence the portal blood supply flowing cortex → medulla.

**EXAM CONNECTION:** A histology image with a labelled zone. **Outermost = glomerulosa =
salt.** Match the layer to the hormone and then the hormone to the function.
...
## 42. Muscle Spindle and Gamma Motor Neurons

**PEARL:** **INTRAFUSAL fibres are innervated by GAMMA motor neurons**; extrafusal
(force-generating) fibres by **alpha** motor neurons. Sensory supply is from **group Ia
(primary/annulospiral) and group II (secondary/flower-spray)** endings.

**UNDERSTAND:** The spindle is a **length detector wired in parallel** with the working
muscle. If it only had passive fibres, then whenever the muscle shortened the spindle would
go slack and stop signalling — precisely when feedback is most needed. **Gamma motor
neurons contract the polar ends of the intrafusal fibre**, taking up that slack and keeping
the central (nuclear) region taut and sensitive at every muscle length. That is **gamma
loop / alpha–gamma co-activation**: the CNS orders both together so the sensor tracks the
motor command.
...
**EXAM CONNECTION:** An image of a spindle with the intrafusal fibres marked. **Intrafusal =
...
## 43. Sodium Transporters of the Proximal Convoluted Tubule

**PEARL:** In the **EARLY PCT**: **Na⁺/K⁺-ATPase on the basolateral membrane** (the engine),
plus apical **Na⁺–glucose (SGLT2) cotransport** and the **Na⁺/H⁺ antiporter (NHE3)**.
**Na⁺–HCO₃⁻ symport (NBC1) is BASOLATERAL and belongs with bicarbonate exit, not the apical
early-PCT sodium entry set.**

**UNDERSTAND:** All proximal reabsorption is **secondary active transport powered by one
primary pump**. The basolateral **Na⁺/K⁺-ATPase** keeps intracellular sodium low, creating
the electrochemical gradient that every apical carrier exploits: glucose, amino acids,
phosphate and lactate all ride **inward with sodium**, while H⁺ rides **outward against it**.
Knowing that single fact lets you reconstruct the whole tubule.

**MUST KNOW:**
- **PCT reabsorbs ~65–70% of filtered Na⁺ and water, 100% of glucose and amino acids, 80–90% of bicarbonate, and most phosphate** — it is **isosmotic** reabsorption.
- **Early PCT** — sodium is reabsorbed with **bicarbonate, glucose, amino acids, phosphate and lactate**. **Late PCT** — the luminal chloride concentration has risen, so **NaCl is reabsorbed down the chloride gradient**, largely paracellularly.
- **Bicarbonate reclamation:** secreted H⁺ (via NHE3) + filtered HCO₃⁻ → H₂CO₃ → **CO₂ + H₂O by luminal CARBONIC ANHYDRASE IV**; inside the cell the reverse reaction (CA II) regenerates HCO₃⁻, which exits basolaterally on **NBC1**. **Acetazolamide blocks carbonic anhydrase → proximal RTA-like bicarbonate wasting.**
- **Site-specific drugs:** **PCT — acetazolamide, SGLT2 inhibitors (dapagliflozin), mannitol**; **thick ascending limb — furosemide (NKCC2)**; **DCT — thiazides (NCC)**; **collecting duct — amiloride (ENaC) and spironolactone (mineralocorticoid receptor)**.
- **Fanconi syndrome** = generalised PCT dysfunction → **glycosuria with normal blood glucose, aminoaciduria, phosphaturia, bicarbonaturia (type 2 proximal RTA)**; causes include **Wilson disease, cystinosis, myeloma, tenofovir, expired tetracycline, lead**.
- **Glomerulotubular balance** — the PCT reabsorbs a **constant fraction** of whatever is filtered, so a rise in GFR does not flood the distal nephron.

**EXAM CONNECTION:** Multi-select transporter lists. Anchor on the **apical trio of the early
PCT (SGLT, NHE3) plus the basolateral Na⁺/K⁺-ATPase**, and remember that **Na–HCO₃ symport
...
## 44. Baroreceptors of the Carotid Sinus

**PEARL:** Carotid sinus baroreceptors are **mechanoreceptors in the ADVENTITIA** (not the
smooth muscle layer) of a **thin-walled, elastic dilatation of the internal carotid artery
just above the bifurcation**. Their afferents run in the **carotid sinus nerve of Hering — a
branch of the GLOSSOPHARYNGEAL nerve (CN IX)** — to the **nucleus tractus solitarius**.

**UNDERSTAND:** The receptors are **stretch-activated**, so they must sit where they can feel
the vessel expand — in the **outermost, collagen-rich adventitia**, which is deformed as the
whole wall stretches. Placing them in the muscular media would make them report muscle tone
rather than transmural pressure. Their nerve endings express **non-selective cation
channels (Piezo1/2, ENaC/ASIC family)** that open on membrane deformation, converting
stretch directly into depolarisation.
...
**EXAM CONNECTION:** "Which statement is INCORRECT?" — the false one misplaces the receptors
in the media/smooth muscle. Everything about **adventitia, CN IX and cation channels** is
true.
...
## 45. Regulation of Glycogen Phosphorylase

**PEARL:** In **muscle**, **Ca²⁺ released during contraction binds CALMODULIN (the δ subunit
of phosphorylase kinase) and activates phosphorylase kinase WITHOUT phosphorylation** — so
glycogenolysis is switched on by the very signal that initiates contraction. **Insulin and
glucose-6-phosphate INHIBIT** glycogen phosphorylase; **cAMP-dependent protein kinase A
(not "cAMP-independent") activates it hormonally.**

**UNDERSTAND:** The cell needs glucose exactly when it starts to contract, and it would be
wasteful to wait for a hormone to arrive from elsewhere. Calcium is already the contraction
signal, so linking it directly to phosphorylase kinase gives **instantaneous local coupling
of fuel supply to demand**. The hormonal route (adrenaline/glucagon → cAMP → PKA →
phosphorylase kinase → phosphorylase *a*) is the slower, systemic overlay for whole-body
needs.
...
**EXAM CONNECTION:** The question tests activators versus inhibitors. **Ca²⁺ activates
(muscle); insulin and G6P inhibit.** Watch for the word "cAMP-**independent**", which
inverts a true statement.
...
## 46. The SA Node Action Potential

**PEARL:** **Phase 0 of the SA (and AV) nodal action potential is due to CALCIUM influx
through L-type channels ONLY** — there is **no fast sodium current**, no phase 1 and no
plateau. The **unstable phase 4 "prepotential" is driven by the funny current (If, a
sodium influx through HCN channels), decaying K⁺ efflux, and T-type calcium channels**.

**UNDERSTAND:** The nodal resting potential is only about **−55 to −60 mV**, at which
voltage the **fast sodium channels are permanently inactivated** — they simply cannot open.
The cell therefore has to depolarise using the slower L-type calcium current, which is why
**nodal upstrokes are slow, conduction through the AV node is slow, and the node has no
stable resting potential**. Everything about pacemaker behaviour and about the drugs that
act on it follows from that single fact.

**MUST KNOW:**
- **Nodal (slow-response) tissue: phases 0, 3, 4 only.** **Ventricular/Purkinje (fast-response) tissue: phase 0 = fast Na⁺ influx; phase 1 = transient K⁺ efflux; phase 2 = plateau (Ca²⁺ influx balancing K⁺ efflux); phase 3 = K⁺ efflux; phase 4 = stable resting potential (−90 mV).**
- **Rate of phase 4 depolarisation sets the heart rate: SYMPATHETIC stimulation (β1 → cAMP) increases If and steepens phase 4 → tachycardia; VAGAL stimulation (M2 → ↑K⁺ efflux) hyperpolarises and flattens it → bradycardia.**
- **Intrinsic rates: SA node 100–110/min (resting rate is ~70 because of vagal tone), AV node 40–60, bundle of His/Purkinje 15–40.** The **fastest pacemaker dominates** (overdrive suppression).
...
**EXAM CONNECTION:** "Phase 0 in the SA node is due to…" — the answer is **calcium alone**.
...
**PEARL:** **ONE row of INNER hair cells and THREE rows of OUTER hair cells.** The **INNER
hair cells are the true sensory transducers — they receive ~95% of the AFFERENT fibres of
the spiral ganglion** and transmit the auditory signal. The **OUTER hair cells are mainly
EFFERENTLY innervated (olivocochlear bundle) and act as a cochlear AMPLIFIER**, modulating
basilar membrane motion.

**UNDERSTAND:** The division of labour is a sensor/amplifier arrangement. Passive travelling
waves on the basilar membrane are too small and too broadly tuned to explain human hearing,
so the **outer hair cells contract and elongate (electromotility, via the protein PRESTIN)
in time with the sound**, mechanically amplifying the vibration ~40–60 dB and sharpening
frequency tuning. That amplified motion is then **detected by the inner hair cells**, which
report it to the brain. Damage to the outer cells therefore causes **loss of sensitivity and
of frequency discrimination — and abolishes otoacoustic emissions**.
...
**EXAM CONNECTION:** The statement set inverts the roles. Anchor: **inner cells SEND the
signal (afferent), outer cells AMPLIFY it (efferent).**
...
## 48. Potassium Recycling in the Thick Ascending Limb

**PEARL:** The **lumen-POSITIVE transepithelial potential of the thick ascending limb is
generated by POTASSIUM recycling back into the lumen through the ROMK channel** — and that
positive charge is what drives the **paracellular reabsorption of Ca²⁺ and Mg²⁺**.

**UNDERSTAND:** The **NKCC2 cotransporter** brings in 1 Na⁺, 1 K⁺ and 2 Cl⁻, but there is
far less potassium in tubular fluid than sodium or chloride — so without replenishment the
transporter would stall. **ROMK leaks K⁺ back into the lumen**, both keeping NKCC2 supplied
and leaving the lumen **electrically positive (about +8 mV)**. That positive lumen then
**repels cations across the tight junctions**, reabsorbing calcium and magnesium
**passively, for free**. One recycling loop powers two entirely separate transport jobs.
...
**EXAM CONNECTION:** "Which cation is recirculated?" — **potassium**, via ROMK. The follow-up
is usually the consequence: **calcium and magnesium reabsorption**.
...
## 49. Phases of the Nerve Action Potential

**PEARL:** **Repolarisation (the falling phase from the peak back toward baseline) is due to
K⁺ EFFLUX** through voltage-gated potassium channels. The **upstroke is Na⁺ influx**, and
the **after-hyperpolarisation** is continued K⁺ efflux while the slow potassium channels
close.

**UNDERSTAND:** The action potential is entirely explained by the **different kinetics of two
channels responding to the same depolarisation**. Sodium channels open **fast and then
inactivate automatically** (a self-limiting spike); potassium channels open **slowly and stay
open** — so by the time sodium is shutting down, potassium conductance is peaking, driving
the membrane back toward (and briefly past) the potassium equilibrium potential. The
**overshoot beyond the resting potential is the after-hyperpolarisation**, and the
**inactivated sodium channels constitute the refractory period**.

**MUST KNOW:**
- **Absolute refractory period = sodium channels INACTIVATED — no stimulus of any strength can trigger a second action potential**; this sets the **maximum firing frequency** and ensures **unidirectional propagation**. **Relative refractory period = some channels reset plus continuing K⁺ efflux — a stronger-than-normal stimulus can fire.**
- The **resting membrane potential (−70 mV)** is set mainly by **K⁺ permeability (Goldman equation)** and maintained by the **Na⁺/K⁺-ATPase (3 Na⁺ out, 2 K⁺ in — electrogenic)**.
- **Action potentials are ALL-OR-NONE and do not summate** — information is coded by **frequency**, not amplitude. **Local/graded potentials** (EPSPs, receptor potentials) do summate and decay with distance.
- **Conduction velocity increases with myelination (saltatory conduction between nodes of Ranvier) and with fibre diameter.**
- **Hyperkalaemia depolarises the resting potential → initially more excitable, then INEXCITABLE (sodium channels inactivate) → cardiac arrest**; **hypocalcaemia lowers the threshold → TETANY**; **local anaesthetics block the sodium channel from inside in its open state (use-dependence)**; **tetrodotoxin blocks Na⁺ channels, tetraethylammonium blocks K⁺ channels.**

**EXAM CONNECTION:** A labelled action potential curve — match the segment to the ion.
...
## 50. Cell Signalling: Autocrine, Paracrine, Endocrine

**PEARL:** **PARACRINE — a mediator acts on NEIGHBOURING cells** (e.g. endothelin-1,
histamine, nitric oxide, prostaglandins). **AUTOCRINE — the cell acts on ITSELF** (IL-2 on
the activated T cell, growth factors in tumours). **ENDOCRINE — the hormone travels in the
BLOODSTREAM to distant targets.** **Merocrine is a mode of SECRETION, not a signalling
class.**

**UNDERSTAND:** The categories are defined purely by **how far the signal travels before it
binds its receptor**, and that distance dictates the signal's properties. **Paracrine
mediators are short-lived and rapidly degraded** (nitric oxide has a half-life of seconds) —
they have to be, or they would spill into the circulation and act systemically. **Endocrine
hormones must be stable and are often protein-bound** so they survive the journey.
**Autocrine loops are the mechanism of self-amplification** — and their dysregulation is a
classic route to malignant proliferation.

**MUST KNOW:**
- **Other modes: JUXTACRINE/contact-dependent — signalling through direct membrane contact or gap junctions (Notch–Delta, antigen presentation); NEUROCRINE/synaptic — a neurotransmitter across a synaptic cleft; NEUROENDOCRINE — a neuron secreting into the blood (ADH, oxytocin from the posterior pituitary); INTRACRINE — the mediator acts within the cell that made it.**
- **Modes of glandular SECRETION (a different classification): MEROCRINE/eccrine — exocytosis with no loss of cell material (salivary glands, ECCRINE sweat glands, pancreas); APOCRINE — the apical cytoplasm is pinched off (mammary gland, apocrine sweat glands); HOLOCRINE — the whole cell disintegrates to become the secretion (SEBACEOUS glands).**
...
**EXAM CONNECTION:** A diagram showing a cell releasing a mediator onto its **immediate
neighbours** = paracrine. "Merocrine" in the options is a category error and is always
wrong.
...
## 51. Pulse Pressure and Arterial Compliance

**PEARL:** A **widened pulse pressure (here 90 mmHg) with isolated systolic hypertension in
an elderly patient is caused by DECREASED ARTERIAL WALL COMPLIANCE** — arterial stiffening.
Stroke volume, contractility and cardiac output are **increased or unchanged**, not
decreased.

**UNDERSTAND:** **Pulse pressure = stroke volume ÷ arterial compliance.** A compliant aorta
acts as a **WINDKESSEL**: it expands to accommodate the stroke volume during systole (so
systolic pressure stays low) and recoils during diastole (so diastolic pressure is
maintained). With age, **elastin fragments and is replaced by collagen**, so the aorta
cannot expand — the same stroke volume now generates a **much higher systolic peak** — and
cannot recoil, so **diastolic pressure falls or stays flat**. Both ends widen the gap.
...
**EXAM CONNECTION:** "What has DECREASED?" — everything else on the list would **narrow** the
pulse pressure if reduced. Only **compliance** widens it by falling.
...
## 52. TENS and the Gate Control Theory

**PEARL:** **Transcutaneous electrical nerve stimulation (TENS) relieves pain by the GATE
CONTROL THEORY** — stimulating **large-diameter Aβ (touch) fibres**, which activate
inhibitory interneurons in the **substantia gelatinosa (Rexed lamina II)** and "close the
gate" on pain transmission from small **Aδ and C** fibres.

**UNDERSTAND:** Melzack and Wall's insight was that the dorsal horn is not a relay but a
**gate whose opening is set by the BALANCE of large- and small-fibre input**. Small pain
fibres **inhibit the inhibitory interneuron** (opening the gate); large touch fibres
**excite it** (closing the gate). This is why **rubbing an injured area genuinely reduces
pain**, why TENS works, and why **loss of large-fibre input in a peripheral neuropathy
produces spontaneous burning pain** — the gate is left permanently open.
...
**EXAM CONNECTION:** The mechanism named for **peripheral, segmental, large-fibre**
modulation is the **gate control theory**; anything involving the brainstem and opioids is
...
## 53. Ending a Muscle Contraction

**PEARL:** Contraction ends when **cytosolic calcium falls** — the **SERCA pump returns Ca²⁺ into the sarcoplasmic reticulum**. Separately, the **cross-bridge detaches when a NEW ATP binds the myosin head**.

**UNDERSTAND:** Two distinct ATP-requiring steps are often confused. **ATP binding (not hydrolysis) breaks the actin–myosin bond**; **ATP hydrolysis then re-cocks the head**. Meanwhile **SERCA uses ATP to lower cytosolic calcium**, allowing troponin-tropomyosin to re-cover the actin binding sites. This is why **rigor mortis** occurs: with no ATP, the heads cannot detach and the muscle stays locked.

**MUST KNOW:**
- Cross-bridge cycle: **Ca²⁺ binds troponin C → tropomyosin shifts → myosin head (carrying ADP+Pi) binds actin → Pi release triggers the POWER STROKE → ADP released → NEW ATP binds and the head DETACHES → ATP hydrolysis re-cocks the head.**
- **Calcium binds troponin C** (not troponin T, which binds tropomyosin; troponin I inhibits the actin–myosin interaction).
- Relaxation requires **removal of calcium (SERCA, ~70% of the ATP used in relaxation) and the sodium–calcium exchanger**.
...
## 54. Renal Clearance vs GFR

**PEARL:** **Clearance > GFR ⇒ net tubular SECRETION. Clearance < GFR ⇒ net tubular REABSORPTION. Clearance = GFR ⇒ freely filtered, neither secreted nor reabsorbed.**

**UNDERSTAND:** Clearance is the **volume of plasma completely cleared of a substance per minute**. If the kidney removes more than filtration alone could account for, it must be **adding the substance to the tubular fluid** — that is secretion. If it removes less, some filtered load has been **taken back**. The logic requires no memorisation once you see that filtration sets the baseline.

**MUST KNOW:**
- **Inulin: clearance = GFR** — the gold standard for measuring GFR (freely filtered, not secreted or reabsorbed). **Creatinine slightly overestimates GFR** because of a small amount of tubular secretion.
- **PAH: clearance = renal PLASMA FLOW** — it is filtered *and* almost completely secreted, so it is cleared in a single pass.
- **Filtration fraction = GFR/RPF ≈ 20%.**
- **Glucose: clearance is normally ZERO** (completely reabsorbed) until the **transport maximum (~375 mg/min, threshold ~180 mg/dL)** is exceeded.
- **Charge selectivity of the glomerular barrier**: the **basement membrane (lamina rara interna and externa) carries negatively charged heparan sulfate**, repelling anions. Hence filterability is **POLYCATIONIC > NEUTRAL > POLYANIONIC** for molecules of equal size. Loss of this charge barrier explains the **selective albuminuria of minimal change disease**.
- **Sodium deficit = 0.6 × body weight (kg) × (target Na − actual Na)** — e.g. 70 kg with Na 120 → **0.6 × 70 × 20 = 840 mEq**.
...
## 55. Organelle Marker Enzymes

**PEARL:** **Mitochondria — ATP synthase (also glutamate dehydrogenase, creatine kinase). Peroxisome — CATALASE. Golgi — galactosyl transferase. Endoplasmic reticulum — glucose-6-phosphatase. Lysosome — acid phosphatase. Cytosol — LDH. Plasma membrane — 5′-NUCLEOTIDASE, Na⁺/K⁺-ATPase, adenylyl cyclase.**

**UNDERSTAND:** Marker enzymes are chosen because each is **exclusive to one compartment and easy to assay**. After differential centrifugation separates organelles by density, measuring these enzymes tells you **which fraction is which and how pure it is** — the practical reason the list exists.
...
## 56. The Four Types of Hypoxia

**PEARL:** **ANAEMIC hypoxia: PaO₂ NORMAL but oxygen CONTENT reduced.** Hypoxic hypoxia: PaO₂ low. Stagnant: flow low. Histotoxic: utilisation blocked.

**UNDERSTAND:** The key is the distinction between **partial pressure (PO₂) and content (mL O₂/dL)**. **Content depends almost entirely on haemoglobin** (1.34 mL per gram) and only trivially on dissolved oxygen. So halving the haemoglobin halves the content while leaving PaO₂ and saturation entirely normal — which is exactly why **pulse oximetry is normal in anaemia and in carbon monoxide poisoning**.

**MUST KNOW:**
- **Hypoxic hypoxia** — low PaO₂: high altitude, hypoventilation, V/Q mismatch, shunt, diffusion defect. **Responds to oxygen** (except a true shunt).
- **Anaemic hypoxia** — normal PaO₂, low content: **anaemia, carbon monoxide poisoning, methaemoglobinaemia**. Compensated at rest by **increased 2,3-DPG**, but exercise unmasks it.
- **Stagnant (circulatory) hypoxia** — normal content, low delivery: **heart failure, shock, local vascular occlusion**; the **arteriovenous oxygen difference is WIDE**.
- **Histotoxic hypoxia** — **cyanide, hydrogen sulfide**: tissues cannot use oxygen, so **venous oxygen content is HIGH** and the AV difference is narrow (arterialised venous blood).
- **Cyanosis requires > 5 g/dL of reduced haemoglobin** — so a severely anaemic hypoxic patient may never look blue, while a polycythaemic patient looks blue easily.

**EXAM CONNECTION:** The graph plots **content against PO₂**. A curve of normal shape but **halved height** = anaemic hypoxia.
...
## 57. Nerve Fibre Susceptibility

**PEARL:** **PRESSURE blocks the largest fibres first: A > B > C.** **LOCAL ANAESTHETIC blocks the smallest first: C > B > A.** **HYPOXIA blocks B first: B > A > C.**

**UNDERSTAND:** Each agent acts by a different mechanism, so each has a different order. **Pressure mechanically deforms and ischaemically starves the large, heavily myelinated fibres**, which have the highest metabolic demand per unit length. **Local anaesthetics diffuse fastest into small, thin or unmyelinated fibres** with the shortest internodal distances — hence pain goes before motor power.
...
- **Neurapraxia** — the reversible conduction block produced by compression, with the axon intact.
...
## 58. Muscle Spindle vs Golgi Tendon Organ

**PEARL:** **Muscle spindle senses LENGTH (stretch)** and lies **in parallel** with extrafusal fibres; **Golgi tendon organ senses TENSION** and lies **in series**, in the tendon.

**UNDERSTAND:** The geometry determines the function. A receptor **in parallel** is stretched whenever the muscle lengthens but is unloaded when the muscle contracts — so it reports **length**. A receptor **in series** bears the full force whether the muscle shortens or lengthens — so it reports **tension**. This is why the tendon organ still fires during an isometric contraction, when the spindle falls silent.

**MUST KNOW:**
- **Muscle spindle**: **intrafusal fibres (nuclear bag and nuclear chain)** supplied by **GAMMA motor neurons**; sensory endings **Ia (primary, dynamic and static)** and **II (secondary, static)**. **Gamma co-activation** keeps the spindle sensitive during contraction. Afferent → **monosynaptic stretch reflex** (knee jerk).
- **Golgi tendon organ**: **Ib afferents**, **DISYNAPTIC inverse stretch reflex** producing **relaxation (the lengthening reaction)** — a protective mechanism against tendon avulsion.
- **CLONUS** — rhythmic contractions on sustained stretch — reflects **INCREASED gamma motor neuron discharge** with loss of descending inhibition; **more than 5 beats is abnormal**; the alternating **stretch reflex/inverse stretch reflex** sequence contributes. Occurs with **upper motor neuron lesions and spinal cord injury disrupting input to the glycinergic RENSHAW cells**.
- **Renshaw cells** provide recurrent inhibition using **glycine** — the transmitter blocked by **strychnine and tetanus toxin**.

**EXAM CONNECTION:** The image identifies **intrafusal fibres → gamma innervation**. The clonus question tests that gamma discharge is **increased**, not decreased.
...
**UNDERSTAND:** The fourth-power dependence on radius dominates everything else in the circulation. It explains why **arterioles are the resistance vessels** (small changes in their calibre transform flow), why a **large-bore short cannula infuses far faster than a long central line**, and why **atherosclerotic narrowing becomes clinically significant so abruptly**.
...
## 60. Surfactant, Surface Tension and Compliance
...
**MUST KNOW:**
- **Compliance = ΔV/ΔP**; reduced in **RDS, pulmonary fibrosis, oedema, atelectasis, and by loss of surfactant**; **increased in emphysema and ageing** (loss of elastic recoil).
- **Surfactant is dipalmitoyl phosphatidylcholine (lecithin)** from **type II pneumocytes**; appears ~24 weeks, adequate after **34–35 weeks**; **L:S ratio ≥ 2** indicates maturity.
- Surfactant also **reduces the work of breathing, keeps alveoli dry (opposing transudation), and stabilises alveoli of different sizes**.
- Related respiratory calculations: **alveolar ventilation = RR × (tidal volume − dead space)**; with RR 10, VT 600 mL and VD 150 mL → **4.5 L/min**. **Anatomical dead space ≈ 150 mL (2 mL/kg)**.
- **Flow-volume loops**: **variable EXTRATHORACIC obstruction truncates the INSPIRATORY limb** (negative airway pressure collapses the extrathoracic airway on inspiration); **variable INTRAthoracic obstruction truncates the EXPIRATORY limb**; **fixed obstruction flattens BOTH**.
...
## 61. SGLT1 and Oral Rehydration

**PEARL:** **ORS works through SGLT1 — the sodium-GLUCOSE co-transporter of the small intestinal brush border.** Glucose absorption **drags sodium, and sodium drags water**.

**UNDERSTAND:** In secretory diarrhoea (cholera, rotavirus) the **crypt cells secrete chloride and water**, but **SGLT1 on the villous cells remains functional**. Because sodium entry through SGLT1 is **coupled to glucose and is independent of the toxin-affected pathways**, providing glucose creates an absorptive route that bypasses the disease — the single most important discovery in the history of diarrhoea management.

**MUST KNOW:**
- **The glucose-to-sodium ratio must be roughly 1:1** — this is why sugary drinks and plain water do not work.
- **SGLT1 — small intestine and renal tubule (S3 segment); SGLT2 — proximal renal tubule only** (the target of gliflozins, responsible for ~90% of renal glucose reabsorption).
- **GLUT transporters (facilitated diffusion): GLUT1 — brain, red cells, placenta, blood-brain barrier (basal uptake); GLUT2 — liver, pancreatic β cell, basolateral gut and kidney (high Km, bidirectional sensor); GLUT3 — neurons (high affinity); GLUT4 — INSULIN-DEPENDENT, in muscle and adipose tissue; GLUT5 — FRUCTOSE, in intestine and sperm.**
- **Low-osmolarity ORS (245 mOsm/L)**: Na 75, glucose 75, K 20, citrate 10 mmol/L.
- **Zinc for 14 days** is given alongside ORS in childhood diarrhoea.
...
## 62. The Pressure–Volume Loop and Contractility
...
**MUST KNOW:**
- **Increased contractility (sympathetic stimulation, digoxin, dobutamine) → steeper ESPVR → smaller end-systolic volume and larger stroke volume.**
- **Increased preload** widens the loop to the right along the **same** ESPVR (Frank–Starling); **increased afterload** raises the pressure at which the aortic valve opens, reducing stroke volume and raising end-systolic volume.
- **Ejection fraction = stroke volume / end-diastolic volume**; stroke volume is the **width of the loop**; the **area within the loop is stroke work**.
- **Diastolic dysfunction** shifts the **EDPVR upward and leftward** (a stiffer ventricle) — the pressure–volume signature of restrictive and hypertensive heart disease.
- **SA node phase 0 is due to CALCIUM influx through L-type channels only** (there is no fast sodium current), with **phase 4 pacemaker potential from the funny current (If, Na⁺) plus T-type calcium**; **phase 3 is potassium efflux**. This is why **calcium channel blockers slow the sinus node** while sodium channel blockers do not.
...
## 63. The Cough Reflex Pathway

**PEARL:** The **afferent limb** of the cough reflex runs in the **internal laryngeal branch of the vagus** and synapses in the **NUCLEUS TRACTUS SOLITARIUS** of the medulla.

**UNDERSTAND:** The nucleus tractus solitarius is the **general visceral afferent nucleus of the brainstem** — every visceral sensory input from cranial nerves VII, IX and X arrives there (taste, baroreceptor, chemoreceptor, airway and gut afferents). So any question about **where a visceral reflex begins centrally** has the same answer.

**MUST KNOW:**
- Efferents: **nucleus ambiguus** (motor to laryngeal adductors via the recurrent laryngeal nerve) plus **spinal motor neurons to the diaphragm, intercostals and abdominal muscles**.
- Cough sequence: **deep inspiration → glottic closure → forceful expiratory effort against a closed glottis (up to 100 mmHg) → sudden glottic opening → explosive expulsion**.
- **Alcohol depresses the cough and gag reflexes** — by CNS depression and direct chemical irritation of the internal laryngeal nerve — hence the aspiration risk.
- Other NTS-mediated reflexes: **baroreceptor (carotid sinus and aortic arch), chemoreceptor, Hering–Breuer, gag (IX afferent), vomiting and taste**.
...
## 64. Thyroid Hormone Storage and Serotonin's GI Role
...
**UNDERSTAND:** No other endocrine gland stores its product extracellularly in bulk. The colloid reservoir means that **if synthesis stops completely, euthyroidism persists for weeks to months** — which explains why hypothyroidism after antithyroid drugs or thyroidectomy is delayed, why **iodine deficiency takes months to manifest**, and why thyroid hormone replacement is dosed once daily despite hour-to-hour demand.

**MUST KNOW:**
- Steps of synthesis: **iodide trapping (NIS, inhibited by thiocyanate and perchlorate) → oxidation and organification and coupling by THYROID PEROXIDASE (inhibited by carbimazole/methimazole and PTU) → storage in colloid → endocytosis and proteolysis → release**. **PTU additionally blocks peripheral 5′-deiodinase.**
- **T4 is the main secreted hormone; T3 is the active one**, mostly generated peripherally by **deiodinase**; **reverse T3** is inactive and rises in **sick euthyroid syndrome, starvation, and with propranolol, amiodarone and steroids**.
- **Wolff–Chaikoff effect** — large iodide loads transiently inhibit organification (used pre-operatively in Graves); **Jod–Basedow** — iodine-induced thyrotoxicosis in a nodular goitre.
- **Gastric acid secretion is stimulated by acetylcholine (M3), gastrin (CCK-B) and histamine (H2)** — histamine from ECL cells is the **final common amplifier**, which is why H2 blockers work so well. **SEROTONIN INHIBITS gastric acid secretion**, stimulates mucus production and **increases intestinal motility**.
...
## 65. Basic Electrical Rhythm (Slow Waves)
...
## 67. Smooth Endoplasmic Reticulum
